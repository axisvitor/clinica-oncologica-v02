...
| 5 | `flows_taskiq.py:1781` had stray `ise` causing NameError on import of any module touching `app.tasks` | Likely truncated `raise` from a previous edit/merge | Removed the stray token; just keep `raise` on preceding line | backend/M009/S06 |
