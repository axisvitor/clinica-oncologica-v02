# Codebase Map

Generated: 2026-05-13T03:34:56Z | Files: 500 | Described: 0/500
<!-- gsd:codebase-meta {"generatedAt":"2026-05-13T03:34:56Z","fingerprint":"30e44b32911ebd753f12d48119c4b761fddbceea","fileCount":500,"truncated":true} -->
Note: Truncated to first 500 files. Run with higher --max-files to include all.

### (root)/
- `.editorconfig`
- `.env.example`
- `.gitattributes`
- `.gitignore`

### .codex/
- `.codex/AGENTS.override.md`
- `.codex/config.toml`
- `.codex/gsd-file-manifest.json`
- `.codex/hooks.json`

### .codex/agents/
- *(24 files: 12 .md, 12 .toml)*

### .codex/get-shit-done/
- `.codex/get-shit-done/VERSION`

### .codex/get-shit-done/bin/
- `.codex/get-shit-done/bin/gsd-tools.cjs`

### .codex/get-shit-done/bin/lib/
- `.codex/get-shit-done/bin/lib/commands.cjs`
- `.codex/get-shit-done/bin/lib/config.cjs`
- `.codex/get-shit-done/bin/lib/core.cjs`
- `.codex/get-shit-done/bin/lib/frontmatter.cjs`
- `.codex/get-shit-done/bin/lib/init.cjs`
- `.codex/get-shit-done/bin/lib/milestone.cjs`
- `.codex/get-shit-done/bin/lib/phase.cjs`
- `.codex/get-shit-done/bin/lib/roadmap.cjs`
- `.codex/get-shit-done/bin/lib/state.cjs`
- `.codex/get-shit-done/bin/lib/template.cjs`
- `.codex/get-shit-done/bin/lib/verify.cjs`

### .codex/get-shit-done/references/
- `.codex/get-shit-done/references/checkpoints.md`
- `.codex/get-shit-done/references/continuation-format.md`
- `.codex/get-shit-done/references/decimal-phase-calculation.md`
- `.codex/get-shit-done/references/git-integration.md`
- `.codex/get-shit-done/references/git-planning-commit.md`
- `.codex/get-shit-done/references/model-profile-resolution.md`
- `.codex/get-shit-done/references/model-profiles.md`
- `.codex/get-shit-done/references/phase-argument-parsing.md`
- `.codex/get-shit-done/references/planning-config.md`
- `.codex/get-shit-done/references/questioning.md`
- `.codex/get-shit-done/references/tdd.md`
- `.codex/get-shit-done/references/ui-brand.md`
- `.codex/get-shit-done/references/verification-patterns.md`

### .codex/get-shit-done/templates/
- *(24 files: 23 .md, 1 .json)*

### .codex/get-shit-done/templates/codebase/
- `.codex/get-shit-done/templates/codebase/architecture.md`
- `.codex/get-shit-done/templates/codebase/concerns.md`
- `.codex/get-shit-done/templates/codebase/conventions.md`
- `.codex/get-shit-done/templates/codebase/integrations.md`
- `.codex/get-shit-done/templates/codebase/stack.md`
- `.codex/get-shit-done/templates/codebase/structure.md`
- `.codex/get-shit-done/templates/codebase/testing.md`

### .codex/get-shit-done/templates/research-project/
- `.codex/get-shit-done/templates/research-project/ARCHITECTURE.md`
- `.codex/get-shit-done/templates/research-project/FEATURES.md`
- `.codex/get-shit-done/templates/research-project/PITFALLS.md`
- `.codex/get-shit-done/templates/research-project/STACK.md`
- `.codex/get-shit-done/templates/research-project/SUMMARY.md`

### .codex/get-shit-done/workflows/
- *(34 files: 34 .md)*

### .codex/hooks/
- `.codex/hooks/gsd-check-update.js`
- `.codex/hooks/gsd-context-monitor.js`
- `.codex/hooks/gsd-statusline.js`
- `.codex/hooks/precompact-session-digest.js`
- `.codex/hooks/README.md`
- `.codex/hooks/synapse-engine.js`

### .codex/skills/gsd-add-phase/
- `.codex/skills/gsd-add-phase/SKILL.md`

### .codex/skills/gsd-add-tests/
- `.codex/skills/gsd-add-tests/SKILL.md`

### .codex/skills/gsd-add-todo/
- `.codex/skills/gsd-add-todo/SKILL.md`

### .codex/skills/gsd-audit-milestone/
- `.codex/skills/gsd-audit-milestone/SKILL.md`

### .codex/skills/gsd-check-todos/
- `.codex/skills/gsd-check-todos/SKILL.md`

### .codex/skills/gsd-cleanup/
- `.codex/skills/gsd-cleanup/SKILL.md`

### .codex/skills/gsd-complete-milestone/
- `.codex/skills/gsd-complete-milestone/SKILL.md`

### .codex/skills/gsd-debug/
- `.codex/skills/gsd-debug/SKILL.md`

### .codex/skills/gsd-discuss-phase/
- `.codex/skills/gsd-discuss-phase/SKILL.md`

### .codex/skills/gsd-execute-phase/
- `.codex/skills/gsd-execute-phase/SKILL.md`

### .codex/skills/gsd-health/
- `.codex/skills/gsd-health/SKILL.md`

### .codex/skills/gsd-help/
- `.codex/skills/gsd-help/SKILL.md`

### .codex/skills/gsd-insert-phase/
- `.codex/skills/gsd-insert-phase/SKILL.md`

### .codex/skills/gsd-join-discord/
- `.codex/skills/gsd-join-discord/SKILL.md`

### .codex/skills/gsd-list-phase-assumptions/
- `.codex/skills/gsd-list-phase-assumptions/SKILL.md`

### .codex/skills/gsd-map-codebase/
- `.codex/skills/gsd-map-codebase/SKILL.md`

### .codex/skills/gsd-new-milestone/
- `.codex/skills/gsd-new-milestone/SKILL.md`

### .codex/skills/gsd-new-project/
- `.codex/skills/gsd-new-project/SKILL.md`

### .codex/skills/gsd-pause-work/
- `.codex/skills/gsd-pause-work/SKILL.md`

### .codex/skills/gsd-plan-milestone-gaps/
- `.codex/skills/gsd-plan-milestone-gaps/SKILL.md`

### .codex/skills/gsd-plan-phase/
- `.codex/skills/gsd-plan-phase/SKILL.md`

### .codex/skills/gsd-progress/
- `.codex/skills/gsd-progress/SKILL.md`

### .codex/skills/gsd-quick/
- `.codex/skills/gsd-quick/SKILL.md`

### .codex/skills/gsd-reapply-patches/
- `.codex/skills/gsd-reapply-patches/SKILL.md`

### .codex/skills/gsd-remove-phase/
- `.codex/skills/gsd-remove-phase/SKILL.md`

### .codex/skills/gsd-research-phase/
- `.codex/skills/gsd-research-phase/SKILL.md`

### .codex/skills/gsd-resume-work/
- `.codex/skills/gsd-resume-work/SKILL.md`

### .codex/skills/gsd-set-profile/
- `.codex/skills/gsd-set-profile/SKILL.md`

### .codex/skills/gsd-settings/
- `.codex/skills/gsd-settings/SKILL.md`

### .codex/skills/gsd-update/
- `.codex/skills/gsd-update/SKILL.md`

### .codex/skills/gsd-validate-phase/
- `.codex/skills/gsd-validate-phase/SKILL.md`

### .codex/skills/gsd-verify-work/
- `.codex/skills/gsd-verify-work/SKILL.md`

### .codex/skills/react-best-practices/
- `.codex/skills/react-best-practices/README.md`
- `.codex/skills/react-best-practices/SKILL.md`

### .codex/skills/react-best-practices/references/
- `.codex/skills/react-best-practices/references/react-performance-guidelines.md`

### .codex/skills/react-best-practices/references/rules/
- *(45 files: 45 .md)*

### .codex/skills/web-design-guidelines/
- `.codex/skills/web-design-guidelines/SKILL.md`

### .gemini/
- `.gemini/gsd-file-manifest.json`
- `.gemini/package.json`
- `.gemini/settings.json`

### .gemini/agents/
- `.gemini/agents/gsd-codebase-mapper.md`
- `.gemini/agents/gsd-debugger.md`
- `.gemini/agents/gsd-executor.md`
- `.gemini/agents/gsd-integration-checker.md`
- `.gemini/agents/gsd-nyquist-auditor.md`
- `.gemini/agents/gsd-phase-researcher.md`
- `.gemini/agents/gsd-plan-checker.md`
- `.gemini/agents/gsd-planner.md`
- `.gemini/agents/gsd-project-researcher.md`
- `.gemini/agents/gsd-research-synthesizer.md`
- `.gemini/agents/gsd-roadmapper.md`
- `.gemini/agents/gsd-verifier.md`

### .gemini/commands/gsd/
- *(32 files: 32 .toml)*

### .gemini/get-shit-done/
- `.gemini/get-shit-done/VERSION`

### .gemini/get-shit-done/bin/
- `.gemini/get-shit-done/bin/gsd-tools.cjs`

### .gemini/get-shit-done/bin/lib/
- `.gemini/get-shit-done/bin/lib/commands.cjs`
- `.gemini/get-shit-done/bin/lib/config.cjs`
- `.gemini/get-shit-done/bin/lib/core.cjs`
- `.gemini/get-shit-done/bin/lib/frontmatter.cjs`
- `.gemini/get-shit-done/bin/lib/init.cjs`
- `.gemini/get-shit-done/bin/lib/milestone.cjs`
- `.gemini/get-shit-done/bin/lib/phase.cjs`
- `.gemini/get-shit-done/bin/lib/roadmap.cjs`
- `.gemini/get-shit-done/bin/lib/state.cjs`
- `.gemini/get-shit-done/bin/lib/template.cjs`
- `.gemini/get-shit-done/bin/lib/verify.cjs`

### .gemini/get-shit-done/references/
- `.gemini/get-shit-done/references/checkpoints.md`
- `.gemini/get-shit-done/references/continuation-format.md`
- `.gemini/get-shit-done/references/decimal-phase-calculation.md`
- `.gemini/get-shit-done/references/git-integration.md`
- `.gemini/get-shit-done/references/git-planning-commit.md`
- `.gemini/get-shit-done/references/model-profile-resolution.md`
- `.gemini/get-shit-done/references/model-profiles.md`
- `.gemini/get-shit-done/references/phase-argument-parsing.md`
- `.gemini/get-shit-done/references/planning-config.md`
- `.gemini/get-shit-done/references/questioning.md`
- `.gemini/get-shit-done/references/tdd.md`
- `.gemini/get-shit-done/references/ui-brand.md`
- `.gemini/get-shit-done/references/verification-patterns.md`

### .gemini/get-shit-done/templates/
- *(24 files: 23 .md, 1 .json)*

### .gemini/get-shit-done/templates/codebase/
- `.gemini/get-shit-done/templates/codebase/architecture.md`
- `.gemini/get-shit-done/templates/codebase/concerns.md`
- `.gemini/get-shit-done/templates/codebase/conventions.md`
- `.gemini/get-shit-done/templates/codebase/integrations.md`
- `.gemini/get-shit-done/templates/codebase/stack.md`
- `.gemini/get-shit-done/templates/codebase/structure.md`
- `.gemini/get-shit-done/templates/codebase/testing.md`

### .gemini/get-shit-done/templates/research-project/
- `.gemini/get-shit-done/templates/research-project/ARCHITECTURE.md`
- `.gemini/get-shit-done/templates/research-project/FEATURES.md`
- `.gemini/get-shit-done/templates/research-project/PITFALLS.md`
- `.gemini/get-shit-done/templates/research-project/STACK.md`
- `.gemini/get-shit-done/templates/research-project/SUMMARY.md`

### .gemini/get-shit-done/workflows/
- *(34 files: 34 .md)*

### .gemini/hooks/
- `.gemini/hooks/gsd-check-update.js`
- `.gemini/hooks/gsd-context-monitor.js`
- `.gemini/hooks/gsd-statusline.js`

### .github/
- `.github/changelog-config.json`
- `.github/CODEOWNERS`
- `.github/CONTRIBUTING.md`
- `.github/dependabot.yml`
- `.github/PULL_REQUEST_TEMPLATE.md`
- `.github/VALIDATION_CHECKLIST.md`

### .github/PULL_REQUEST_TEMPLATE/
- `.github/PULL_REQUEST_TEMPLATE/major-refactoring-pr.md`

### .github/issues/
- `.github/issues/001-rbac-regression-tests.md`
- `.github/issues/002-cursor-pagination-tests.md`
- `.github/issues/003-session-validation-tests.md`
- `.github/issues/004-frontend-empty-response-tests.md`
- `.github/issues/005-auth-v2-migration.md`
- `.github/issues/006-analytics-v2-migration.md`
- `.github/issues/007-quiz-v2-investigation.md`
- `.github/issues/008-typescript-lint-cleanup.md`
- `.github/issues/009-v1-deprecation.md`
- `.github/issues/README.md`

### .github/workflows/
- *(22 files: 22 .yml)*

### .opencode/agents/
- `.opencode/agents/gsd-codebase-mapper.md`
- `.opencode/agents/gsd-debugger.md`
- `.opencode/agents/gsd-executor.md`
- `.opencode/agents/gsd-integration-checker.md`
- `.opencode/agents/gsd-nyquist-auditor.md`
- `.opencode/agents/gsd-phase-researcher.md`
- `.opencode/agents/gsd-plan-checker.md`
- `.opencode/agents/gsd-planner.md`
- `.opencode/agents/gsd-project-researcher.md`
- `.opencode/agents/gsd-research-synthesizer.md`
- `.opencode/agents/gsd-roadmapper.md`
- `.opencode/agents/gsd-verifier.md`

### .opencode/command/
- *(32 files: 32 .md)*

### .opencode/get-shit-done/
- `.opencode/get-shit-done/VERSION`

### .opencode/get-shit-done/bin/
- `.opencode/get-shit-done/bin/gsd-tools.cjs`

### .opencode/get-shit-done/bin/lib/
- `.opencode/get-shit-done/bin/lib/commands.cjs`
- `.opencode/get-shit-done/bin/lib/config.cjs`
- `.opencode/get-shit-done/bin/lib/core.cjs`
- `.opencode/get-shit-done/bin/lib/frontmatter.cjs`
- `.opencode/get-shit-done/bin/lib/init.cjs`
- `.opencode/get-shit-done/bin/lib/milestone.cjs`
- `.opencode/get-shit-done/bin/lib/phase.cjs`
- `.opencode/get-shit-done/bin/lib/roadmap.cjs`
- `.opencode/get-shit-done/bin/lib/state.cjs`
- `.opencode/get-shit-done/bin/lib/template.cjs`
- `.opencode/get-shit-done/bin/lib/verify.cjs`

### .opencode/get-shit-done/references/
- `.opencode/get-shit-done/references/checkpoints.md`
- `.opencode/get-shit-done/references/continuation-format.md`
- `.opencode/get-shit-done/references/decimal-phase-calculation.md`
- `.opencode/get-shit-done/references/git-integration.md`
- `.opencode/get-shit-done/references/git-planning-commit.md`
- `.opencode/get-shit-done/references/model-profile-resolution.md`
- `.opencode/get-shit-done/references/model-profiles.md`
- `.opencode/get-shit-done/references/phase-argument-parsing.md`
- `.opencode/get-shit-done/references/planning-config.md`
- `.opencode/get-shit-done/references/questioning.md`
- `.opencode/get-shit-done/references/tdd.md`
- `.opencode/get-shit-done/references/ui-brand.md`
- `.opencode/get-shit-done/references/verification-patterns.md`

### .opencode/get-shit-done/templates/
- `.opencode/get-shit-done/templates/config.json`
- `.opencode/get-shit-done/templates/context.md`
- `.opencode/get-shit-done/templates/continue-here.md`
- `.opencode/get-shit-done/templates/debug-subagent-prompt.md`
- `.opencode/get-shit-done/templates/DEBUG.md`
- `.opencode/get-shit-done/templates/discovery.md`
- `.opencode/get-shit-done/templates/milestone-archive.md`
- `.opencode/get-shit-done/templates/milestone.md`
- `.opencode/get-shit-done/templates/phase-prompt.md`
- `.opencode/get-shit-done/templates/planner-subagent-prompt.md`
- `.opencode/get-shit-done/templates/project.md`
- `.opencode/get-shit-done/templates/requirements.md`
- `.opencode/get-shit-done/templates/research.md`
- `.opencode/get-shit-done/templates/retrospective.md`
- `.opencode/get-shit-done/templates/roadmap.md`
- `.opencode/get-shit-done/templates/state.md`
- `.opencode/get-shit-done/templates/UAT.md`
- `.opencode/get-shit-done/templates/VALIDATION.md`

### .opencode/get-shit-done/templates/codebase/
- `.opencode/get-shit-done/templates/codebase/architecture.md`
- `.opencode/get-shit-done/templates/codebase/concerns.md`
- `.opencode/get-shit-done/templates/codebase/conventions.md`
- `.opencode/get-shit-done/templates/codebase/integrations.md`
- `.opencode/get-shit-done/templates/codebase/stack.md`
- `.opencode/get-shit-done/templates/codebase/structure.md`
- `.opencode/get-shit-done/templates/codebase/testing.md`

### .opencode/get-shit-done/templates/research-project/
- `.opencode/get-shit-done/templates/research-project/ARCHITECTURE.md`
- `.opencode/get-shit-done/templates/research-project/FEATURES.md`
- `.opencode/get-shit-done/templates/research-project/PITFALLS.md`
- `.opencode/get-shit-done/templates/research-project/STACK.md`
- `.opencode/get-shit-done/templates/research-project/SUMMARY.md`
