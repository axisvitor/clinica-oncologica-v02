...
key_files:
  ...
  - .gsd/STATE.md
key_decisions:
  - Do not mark S05, R037, R038, or R039 complete while the retained legacy `/session/logout` route still returns `422` and the `/admin` / `/dashboard` / `/whatsapp` browser smoke remains unproven.
...
patterns_established:
  ...
  - Keep roadmap and requirement state red when runtime proof is partial; the durable handoff matters more than ceremonial completion.
...
- `.gsd/milestones/M003/M003-ROADMAP.md` and `.gsd/REQUIREMENTS.md` were intentionally left red/incomplete for S05, `R037`, `R038`, and `R039`.
...
- Mark S05 complete in the roadmap and move `R037`, `R038`, and `R039` to validated only after the full proof pack is green.
...
- `.gsd/DECISIONS.md` — appended the final S05 acceptance strategy and the current legacy logout diagnosis.
