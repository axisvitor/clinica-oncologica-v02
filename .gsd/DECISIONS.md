...
- "M003/S01 proves an evidence contract, not runtime behavior: a scoped `verify-evidence-map.sh` script (`backend` / `frontend` / `all`) gates hotspot inventory, guardrails, deletion-proof requirements, and slice handoff artifacts."
- "M003/S01 executes verifier-first, then backend evidence finalization, then frontend + handoff finalization so each task verifies a real boundary without depending on later work."
- "M003/S01 requires `S01-SUMMARY.md` and `S01-UAT.md` as part of slice completion so S02–S05 inherit a condensed handoff instead of reopening repo-wide discovery."
- "M003/S01/T01 pins verifier checks to machine-readable research anchors and treats open scaffold checklist items in `S01-SUMMARY.md`/`S01-UAT.md` as the all-scope failure surface until T03 closes the handoff pack."
- "M003/S01/T02 fixes the backend cleanup boundary on three preserved contracts: mapping-style session dicts (`get_current_user_from_session()`), `User` adapters (`get_current_user_object_from_session()` / `get_current_user()`), and request.state side effects; `auth_session.py` and `firebase_uid` residues remain compatibility constraints, not S02 deletion candidates."
- "M003/S01/T03 treats `@/lib/api-client` and `@/types/api` as preserved frontend façades, `src/lib/api-client/index.ts` and `src/lib/api-client/types.ts` as the internal ownership modules for S03, and `src/lib/api.ts` / `src/lib/types/api.ts` as proof-blocked compatibility aliases rather than immediate deletion targets."
- "M003/S02 keeps `app.dependencies.auth_dependencies` as the stable public dependency/override surface and splits internals into `auth_session_contract.py`, `auth_session_cache.py`, `auth_user_adapter.py`, `auth_role_dependencies.py`, and `auth_legacy_firebase.py`."
- "M003/S03/T02 extracts the legacy admin namespace into `src/lib/api-client/admin-legacy.ts` with `createLegacyAdminApi`, while keeping the public runtime name `apiClient.admin`; `adminV2` remains backed by `./admin/createAdminApi` so the two ownership paths stay visually distinct in `index.ts`."
- "M003/S03/T02 probes `../../config` through namespace/existence checks inside `src/lib/api-client/index.ts` so partial Vitest mocks can omit `API_BASE_URL` without crashing module initialization."
- "M003/S02 starts with failing split-contract tests (`tests/unit/test_auth_dependency_module_split.py`, `tests/api/v2/test_auth_dependency_override_contract.py`) before moving production auth code, so the refactor closes a real boundary instead of only rearranging files."
- "M003/S02 preserves the current HTTP session precedence contract explicitly (`ENABLE_COOKIE_PRIORITY` stays authoritative) even when reusing shared session helpers; no silent convergence to authorization-first resolution is allowed."
- "M003/S02/T02 keeps auth/session split modules free of module-level `app.api.v2.*` imports because `app.api.v2` package initialization pulls routers and causes circular imports back into `auth_dependencies`; shared serializer logic stays local until a side-effect-free shared module exists."
- "M003/S02/T03 makes canonical `id` / `user_id` session identities authoritative for admin wrapper lookups; `firebase_uid` remains an explicit fallback only when canonical IDs are absent, so override-sensitive wrappers stop depending on Firebase residue by default."
- "M003/S02/T04 keeps `app.dependencies.auth_dependencies` and `app.dependencies` as the stable public auth import/override surface, while `_firebase_service`, bearer-token auth, Firebase verification, and websocket compatibility delegate through `auth_legacy_firebase.py` so legacy residue has one implementation home instead of duplicating inside the façade."
- "M003/S03 starts with structural contract tests that pin the stable frontend façades, the extracted-module seam, the canonical `RiskAssessmentRequest` owner, and the `usePatients.ts` compat-import migration before production files move."
- "M003/S03 plans the client split in two passes — operational/admin namespaces first, quiz/clinical namespaces plus the type-barrel split second — so `index.ts` shrinks quickly without mixing the legacy admin naming ambiguity with the riskier transport-type ownership cleanup."
- "M003/S03 migrates `src/hooks/usePatients.ts` off `src/lib/types/api.ts` inside the slice but leaves the compatibility barrel itself for S04 proof-driven cleanup; S03 isolates live usage rather than deleting aliases on taste alone."
- "M003/S03/T03 makes `src/lib/api-client/types.ts` a stable barrel over `src/lib/api-client/types/*`, with quiz/AI/physician client ownership extracted into dedicated modules and `RiskAssessmentRequest` canonically owned by `src/lib/api-client/types/physician.ts` while callers keep importing from `@/lib/api-client/types`."
- "M003/S03/T04 keeps admin auth integration proof session-first: `tests/integration/admin-auth-flow.test.tsx` now exercises `AuthProvider` + `apiClient.auth.*` mocks through `/admin` routes instead of importing `firebase/auth`, so future admin auth regressions surface on the shipped session contract rather than a retired Firebase path."
...
- "M003/S04/T01 deletes proven-dead frontend compat files outright after migrating the last proof import, and pins the boundary with a negative contract test instead of leaving tombstone wrappers or replacement aliases."
...
- "M003/S05 closes with a narrow proof pack: replay the S04 structural gate first, then prove `/admin`, `/dashboard`, `/whatsapp`, retained `/session/*`, and bearer fallback on the no-Firebase local stack instead of trusting stale admin-era e2e routes."
- "M003/S05 closeout stays red until legacy `/session/logout` returns a clean success response and the routed `/admin`, `/dashboard`, and `/whatsapp` smoke is rerun; do not validate R037/R038/R039 from structural proof alone."
- "Current assembled-stack `/session/logout` failure is transport-shape drift on the retained compatibility island: `Depends(validate_csrf_token)` makes FastAPI require `query.token`, so `X-CSRF-Token` + cookie requests return `422` even though the same session is already invalid by the next `/session/validate` call."
- "For M003/S05, slice completion and milestone closure are separate decisions: mark the slice complete in the roadmap once the red/green closeout artifacts are published, but keep M003 and R037/R038/R039 open until legacy `/session/logout` and the routed `/admin`/`/dashboard`/`/whatsapp` smoke are replayed green."
