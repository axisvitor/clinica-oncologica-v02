...
- O verificador de S01 voltou a ser um gate honesto para o backend: `--report`/`--check backend` ficam verdes sem `root_legacy_session` nem narrativa Firebase backend aprovada, restando só o inventário reduzido de `firebase_uid` adjacente e alguns anchors de rejeição/detecção (`x_session_id`, bearer fallback, websocket query).
- O próximo foco é M004/S05: remover o resíduo funcional adjacente de Firebase (`firebase_uid`, cache/auditoria/adapters e narrativa operacional restante) sem reabrir a fronteira canônica recém-fechada; S06 continua responsável pela prova montada do stack local sem Firebase.
