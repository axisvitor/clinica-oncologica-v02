...
- M009 concluído: migração completa do Celery para Taskiq async-native — 72 tasks em 13 módulos *_taskiq.py, 47/47 schedule parity via LabelScheduleSource, SmartRetryMiddleware com exponential backoff + jitter, ListRedisScheduleSource para ETA dispatch, AsyncSession via TaskiqDepends substituindo sync bridging. ~900 linhas de bridge code eliminadas (async_context_manager.py, run_async_in_celery, async_helpers.py, event_loop_manager.py), 30 arquivos Celery deletados, celery/kombu/amqp/billiard/flower/asgiref removidos de requirements.txt. Helpers extraídos para app/tasks/helpers/ (9 módulos, 40+ funções). Test suite migrada: 29 arquivos atualizados, 8 deletados, 4796 testes coletam sem erros Celery. AST scan zero imports Celery em app/ e tests/. Todos os 10 requisitos (R077–R086) validados.

## Architecture / Key Patterns

- Backend FastAPI com AsyncSession nas rotas API e Session síncrona em serviços ORM internos dos workers Taskiq (via get_scoped_session() dentro de async task bodies).
...
- [x] M009: Substituição do Celery por Taskiq — migração completa do task queue para async-native. 72 tasks em 13 módulos Taskiq, 47/47 schedule parity, ~900 linhas de bridge code removidas, 30 files Celery deletados, test suite migrada (29 files atualizados, 8 deletados). Todos os 10 requisitos (R077–R086) validados. 4796 testes coletam sem erros Celery.
