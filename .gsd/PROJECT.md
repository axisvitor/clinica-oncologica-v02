...
- ✓ Sequential gate recupera contexto mismatch com retry/reset em vez de espera silenciosa (FLOW-01) — v1.9
- ✓ Envios de mensagem falhos sao re-tentados automaticamente via Celery com backoff exponencial (FLOW-02) — v1.9
- ✓ Follow-ups adiados com falha de envio sao re-tentados via task queue (FLOW-03) — v1.9
- ✓ Avanco de dia apos day_complete e atomico e verificado (sem skip silencioso) (FLOW-04) — v1.9
- ✓ Template day_config validado no inicio do fluxo; config malformada falha rapido com erro claro (FLOW-05) — v1.9
- ✓ Detector de fluxo travado (awaiting_response > N horas) roda como task periodica Celery Beat (RECV-01) — v1.9
- ✓ Auto-recuperacao de fluxo travado tenta re-envio ou avanco baseado no estado (RECV-02) — v1.9
- ✓ Admin pode resetar/avancar/destravar fluxo de paciente via API (RECV-03) — v1.9
- ✓ Operacoes de fluxo falhas visiveis no admin (DLQ ou similar) (RECV-04) — v1.9
- ✓ Endpoint de saude do fluxo mostra contagens de fluxos ativos/travados/falhos/completos (OBS-01) — v1.9
- ✓ Alerta de fluxo travado dispara quando paciente nao progrediu em tempo configuravel (OBS-02) — v1.9
- ✓ Taxa de fallback de personalizacao AI rastreada via metrica (OBS-03) — v1.9
- ✓ Correlation ID propagado do webhook por toda cadeia de processamento do fluxo (OBS-04) — v1.9
- ✓ Testes de integracao cobrem pipeline completo webhook -> gate -> continuacao -> envio (TEST-01) — v1.9
- ✓ Testes de integracao cobrem deteccao de fluxo travado -> caminho de recuperacao (TEST-02) — v1.9
- ✓ Testes de integracao cobrem mecanica de retry para envios falhos (TEST-03) — v1.9

### Active

- None currently assigned — v1.9 was completed and the next milestone has not been selected yet.
...
## Current Milestone: v1.9 Bulletproof Flow Pipeline — complete

**Completed goal:** Tornar o pipeline de fluxo WhatsApp operacional e resiliente ponta a ponta, eliminando stalls silenciosos, adicionando recovery/observability operacional e comprovando o comportamento com testes de integracao.

**Delivered in v1.9:**
- Correcao do sequential gate para recuperar de context mismatches com bounded retry/reset
- Retry automatico de envios falhos e de follow-ups adiados via Celery
- Deteccao e auto-recuperacao de fluxos travados com task periodica e idempotencia Redis
- Observabilidade do fluxo: endpoint de saude, alertas de stall, metricas de fallback AI e correlation IDs
- Testes de integracao cobrindo pipeline end-to-end, recovery path e retry mechanics

## Current State

- **Latest shipped milestone:** v1.9 Bulletproof Flow Pipeline (2026-03-06)
- **Production posture:** O pipeline de fluxo agora possui mismatch recovery bounded, retry tasks para envios falhos, detector de stalled flows com auto-recovery, flow health/alert surfaces, metricas de fallback AI e correlation IDs ponta a ponta.
...
- **ADK posture:** ADK runtime controls, tool safety guardrails, deterministic error classification, and Prometheus observability remain live on `/api/v2/adk/run`; CI smoke gate still blocks deploy on oncology trajectory regressions.
- **Codebase snapshot:** ~434k LOC Python + ~163k LOC TypeScript/TSX em arquitetura brownfield madura (DDD + Saga + circuit breaker)
- **Flow posture:** Webhook ingress, continuation, retry, recovery e admin operator surfaces estao cobertos por testes focados e integracoes E2E de fase.

<details>
<summary>Archived Milestone Brief: v1.9 Bulletproof Flow Pipeline</summary>
...
**Delivered:**
- Sequential gate mismatch recovery com bounded reset semantics
- Retry automatico de outbound send e deferred follow-up send
- Day advancement atomico e verificado
- Detector de stuck flow + auto-recovery + admin flow ops
- Flow health/alerting, AI fallback metrics e correlation ID propagation
- Testes de integracao cobrindo pipeline, recovery e retry
...
- v1.9 shipped: bulletproof flow pipeline with retry, recovery, observability, and end-to-end verification (net positive; see milestone summary)
...
_Last updated: 2026-03-11 after M001 completion_
