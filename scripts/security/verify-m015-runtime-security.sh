...
setup_workspace() {
  mkdir -p "$CERT_DIR" "$LOG_DIR" "$EVIDENCE_ROOT"
...
database_url_policy=sslmode=verify-full,sslrootcert=/m015-certs/ca.crt,sslminversion=TLSv1.2
...
postgres_tls_ready() {
  compose_cmd run --rm -T db-probe sh -ec \
    'psql "$M015_DATABASE_PSQL_CONN" -Atc "select current_user" | grep -qx hormonia_app'
}
...
run_db_seam() {
  ...
  compose_up
  wait_for_readiness
  log_phase "evidence" "ready" "substrate readiness evidence captured; DB migrations/RLS proof intentionally deferred to later S01 tasks"
}
