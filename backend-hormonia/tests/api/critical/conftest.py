...
def _ensure_sessions_session_token_column(engine):
    """Ensure Postgres critical-suite schemas expose required sessions auth columns."""
    if engine.dialect.name != "postgresql":
        return

    inspector = sa_inspect(engine)
    if not inspector.has_table("sessions"):
        print("[critical.conftest] sessions table missing; skipping session_token guard")
        return
    ...
    print("[critical.conftest] Applying schema patch: align sessions auth columns")
    with engine.begin() as connection:
