...
class StructuredErrorLogger:
    ...
    def log_error(
        ...
        # Check rate limiting
        if self.rate_limiter.should_log(log_key, log_message, getattr(logging, level.value)):
    ...
    def log_structured(
        ...
        **context_data
        ...
        if self.rate_limiter.should_log(log_key, message, getattr(logging, level.value)):
            log_level = getattr(logging, level.value)
            self.logger.log(
                log_level,
                message,
    ...
    def _format_error_message(self, error_context: ErrorContext) -> str:
        ...
        if error_context.endpoint:
            parts.append(f"{error_context.method or 'UNKNOWN'} {error_context.endpoint}")
    ...
    def _aggregate_error(self, error_context: ErrorContext) -> None:
        ...
        with self.aggregation_lock:
            ...
            error_pattern = f"{error_context.error_type}_{error_context.endpoint or 'unknown'}"
    ...
    def _categorize_error(self, error_type: str) -> str:
        ...
        if any(db_term in error_type_lower for db_term in ['sql', 'database', 'connection', 'integrity']):
            return 'database_errors'
        elif any(ws_term in error_type_lower for ws_term in ['websocket', 'connection']):
            return 'websocket_errors'
        elif any(auth_term in error_type_lower for auth_term in ['auth', 'permission', 'token']):
    ...
    def get_aggregation_stats(self) -> Dict[str, Any]:
        ...
        with self.aggregation_lock:
            stats = {
                'total_patterns': len(self.error_aggregations),
                'patterns': {}
            }
    ...
    def cleanup_old_aggregations(self) -> None:
        ...
        with self.aggregation_lock:
            cutoff_time = datetime.utcnow() - timedelta(minutes=self.aggregation_window_minutes * 2)
