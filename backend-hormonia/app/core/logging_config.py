...
class RateLimitedLogger:
    ...
    def __init__(
        ...
        self.message_hashes: Dict[
            str, Dict
        ] = {}  # hash -> {count, first_seen, last_seen}
    ...
    def should_log(
        self, log_key: str, message: str = "", level: int = logging.INFO
    ) -> bool:
        """
...
class OptimizedRequestLogger:
    ...
    def log_request_start(
        self, method: str, path: str, client_ip: str, correlation_id: str
    ) -> None:
    ...
    def log_request_complete(
        ...
        message = (
            f"Request completed: {method} {path} - {status_code} ({process_time:.3f}s)"
        )
    ...
    def log_request_error(
        ...
        if self.rate_limiter.should_log(log_key, message, level):
            ...
            if level == logging.ERROR:
                self.logger.error(
                    message, extra=extra_data, exc_info=include_stacktrace
                )
            else:
                self.logger.warning(
                    message, extra=extra_data, exc_info=include_stacktrace
