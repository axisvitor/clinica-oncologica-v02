...
class RLSConnectionManager:
    ...
    def _initialize_engines(self):
        ...
        rls_pool_size = max(
            10, settings.DATABASE_POOL_SIZE // 3
        )  # Dynamic: 1/3 of service pool
        rls_engine = create_optimized_engine(
            settings.DATABASE_URL,
            poolclass=QueuePool,
            pool_size=rls_pool_size,  # Dynamic sizing based on service pool
            max_overflow=max(
                10, settings.DATABASE_POOL_MAX_OVERFLOW // 2
            ),  # 1/2 of service overflow
            pool_pre_ping=settings.DATABASE_POOL_PRE_PING,  # True - prevents SSL errors
        ...
        def handle_service_role_error(exception_context):
            ...
            if isinstance(exception_context.original_exception, OperationalError):
                ...
                if (
                    ...
                    logger.warning(
                        f"SSL connection lost on service_role engine: {error_msg[:100]}... Pool pre-ping will reconnect automatically"
                    )
        ...
        def handle_rls_error(exception_context):
            ...
            if isinstance(exception_context.original_exception, OperationalError):
                ...
                if (
                    ...
                    logger.warning(
                        f"SSL connection lost on rls engine: {error_msg[:100]}... Pool pre-ping will reconnect automatically"
                    )
                    # Retorna None para permitir que o pool pre-ping reconecte
                    return None
        ...
        def set_rls_context(dbapi_connection, connection_record):
            ...
            try:
                ...
                with dbapi_connection.cursor() as cursor:
                    ...
                    cursor.execute(
                        "SELECT set_config('app.current_user_role', 'authenticated', false)"
                    )
    ...
    def create_rls_session(
        self, jwt_token: Optional[str] = None, user_id: Optional[str] = None
    ) -> Session:
        """
    ...
    def _inject_rls_context(
        ...
        try:
            if jwt_token:
                ...
                try:
                    decoded_token = jwt.decode(
                        ...
                        },  # SECURITY FIX: Changed from False to True
                    )
            ...
            if jwt_token:
                session.execute(
                    text("SELECT set_config('request.jwt.token', :token, true)"),
                    {"token": jwt_token},
                )
                session.execute(
            ...
            if getattr(settings, "RLS_ENABLE_AUDIT_LOGGING", True):
                session.execute(
                    text("SELECT set_config('app.audit_enabled', 'true', true)")
                )
...
def get_db(
    jwt_token: Optional[str] = None, user_id: Optional[str] = None
    ...
    try:
        with get_scoped_session() as session:
            ...
            if not use_service_role:
                try:
                    user_id_result = session.execute(
                        text("SELECT current_setting('app.current_user_id', true)")
                    ).fetchone()
                    user_role_result = session.execute(
                        text("SELECT current_setting('app.current_user_role', true)")
                    ).fetchone()
                    rls_context = {
                        "current_user_id": user_id_result[0]
                        if user_id_result
                        else None,
                        "current_user_role": user_role_result[0]
                        if user_role_result
                        else None,
                    }
                except Exception as e:
            ...
            # Test pool status
            engine_instance = connection_manager.get_engine(
...
def is_pool_healthy(use_service_role: bool = True) -> bool:
    ...
    try:
        ...
        return (
            ...
            < pool_status["pool_size"] + pool_status["overflow"]
        )
    except Exception:
        return False
