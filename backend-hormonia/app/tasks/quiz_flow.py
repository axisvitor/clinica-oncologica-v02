...
def send_quiz_question_task(self, patient_id: str, quiz_session_id: str, question_index: int) -> dict[str, Any]:
    """
...
def process_quiz_response_task(self, patient_id: str, message_id: str) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.error(f"Quiz response processing failed after {self.max_retries} retries: {exc}")
            return {
                'success': False,
                'error': str(exc),
                'final_failure': True
...
def check_quiz_triggers_task(self, limit: int = 100) -> dict[str, Any]:
    ...
    try:
        with next(get_db()) as db:
            ...
            for flow_state in monthly_flows:
                try:
                    ...
                    logger.error(f"Error checking quiz trigger for patient {flow_state.patient_id}: {e}")
                    results['errors'] += 1
                    results['details'].append({
                        'patient_id': str(flow_state.patient_id),
                        'error': str(e)
            ...
            logger.info(f"Quiz trigger check completed: {results['quizzes_triggered']} triggered, "
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.error(f"Quiz trigger check failed after {self.max_retries} retries: {exc}")
            return {
                'success': False,
                'error': str(exc),
                'final_failure': True
...
def cleanup_expired_quiz_sessions_task(self, max_age_hours: int = 48) -> dict[str, Any]:
    ...
    try:
        with next(get_db()) as db:
            ...
            expired_sessions = quiz_session_repo.get_expired_incomplete_sessions(cutoff_time)
            ...
            for session in expired_sessions:
                try:
                    ...
                    quiz_session_repo.update(session, {
                        'is_completed': True,
            ...
            logger.info(f"Quiz session cleanup completed: {cleanup_count} sessions cleaned, {errors} errors")
...
@celery_app.task(bind=True, max_retries=2, default_retry_delay=30)
def send_quiz_progress_update_task(self, patient_id: str, quiz_session_id: str, progress_data: dict[str, Any]) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.error(f"Quiz progress update failed after {self.max_retries} retries: {exc}")
            return {
                'success': False,
                'error': str(exc),
                'final_failure': True
...
def generate_quiz_report_task(self, patient_id: str, quiz_session_id: str) -> dict[str, Any]:
    """
    ...
    try:
        with next(get_db()) as db:
            ...
            responses = quiz_response_service.get_session_responses(UUID(quiz_session_id))
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.error(f"Quiz report generation failed after {self.max_retries} retries: {exc}")
            return {
                'success': False,
                'error': str(exc),
                'final_failure': True
...
def _notify_providers_of_quiz_completion(patient_id: str, quiz_session_id: str, report_id: str):
    """
...
@celery_app.task(bind=True, max_retries=3, default_retry_delay=60)
def send_quiz_link_reminder_task(self, quiz_session_id: str, hours_before_expiry: int = 24) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.error(f"Quiz reminder failed after {self.max_retries} retries: {exc}")
            return {
                'success': False,
                'error': str(exc),
                'final_failure': True
...
def monitor_quiz_links_task(self) -> dict[str, Any]:
    ...
    try:
            ...
            for session in active_sessions:
                ...
                if session_age.total_seconds() > 24 * 3600:  # 24 hours
                    ...
                    try:
                        ...
                        logger.error(f"Error handling expired session {session.id}: {e}")
        ...
        if self.request.retries < self.max_retries:
            ...
            logger.error(f"Quiz link monitoring failed after {self.max_retries} retries: {exc}")
            return {
                'success': False,
                'error': str(exc),
                'final_failure': True
    ...
    try:
        with next(get_db()) as db:
            ...
            if flow_state and session.quiz_template_id:
                ...
                from app.services.quiz import QuizTemplateService
                template_service = QuizTemplateService(db)
                template = template_service.template_repository.get(session.quiz_template_id)
                ...
                if fallback_result.get('success'):
                    ...
                    logger.warning(f"Failed to trigger fallback: {fallback_result.get('error')}")
