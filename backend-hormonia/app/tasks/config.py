...
class TaskConfigurations:
    ...
    def update_from_env(self):
        ...
        if os.getenv("MONITORING_CLEANUP_INTERVAL"):
            self.monitoring.cleanup_interval = int(
                os.getenv("MONITORING_CLEANUP_INTERVAL")
            )
        if os.getenv("MONITORING_DATA_RETENTION_DAYS"):
            self.monitoring.data_retention_days = int(
                os.getenv("MONITORING_DATA_RETENTION_DAYS")
            )

        # Quiz config
        if os.getenv("QUIZ_SESSION_TIMEOUT_HOURS"):
            self.quiz.session_timeout_hours = int(
                os.getenv("QUIZ_SESSION_TIMEOUT_HOURS")
            )
        if os.getenv("QUIZ_PROGRESS_THRESHOLD"):
            self.quiz.progress_update_threshold = int(
                os.getenv("QUIZ_PROGRESS_THRESHOLD")
            )

        # Flow config
        if os.getenv("FLOW_STEP_TIMEOUT_MINUTES"):
            self.flows.step_timeout_minutes = int(
                os.getenv("FLOW_STEP_TIMEOUT_MINUTES")
            )
