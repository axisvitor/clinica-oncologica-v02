...
from app.services.alerts.escalation import AlertEscalation as AlertEscalationService
    ...
    def run(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_keys = (
                len(old_metric_keys)
                + len(old_alert_keys)
                + len(old_escalation_keys)
                + len(old_bottleneck_keys)
            )
...
system_health_check_task = celery_app.task(
    bind=True, name="monitoring.system_health_check"
)(SystemHealthCheckTask().run)
performance_metrics_collection_task = celery_app.task(
    bind=True, name="monitoring.performance_metrics_collection"
)(PerformanceMetricsCollectionTask().run)
bottleneck_detection_task = celery_app.task(
    bind=True, name="monitoring.bottleneck_detection"
)(BottleneckDetectionTask().run)
alert_monitoring_task = celery_app.task(bind=True, name="monitoring.alert_monitoring")(
    AlertMonitoringTask().run
)
escalation_check_task = celery_app.task(bind=True, name="monitoring.escalation_check")(
    EscalationCheckTask().run
)
automated_recovery_task = celery_app.task(
    bind=True, name="monitoring.automated_recovery"
)(AutomatedRecoveryTask().run)
cleanup_old_monitoring_data_task = celery_app.task(
    bind=True, name="monitoring.cleanup_old_data"
)(CleanupOldMonitoringDataTask().run)
