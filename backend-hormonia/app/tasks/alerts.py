            ...
            for patient in patients:
                try:
                    ...
                    for alert in alerts:
                        ...
                        self.get_task_logger().info(f"Processed alert for patient {patient.id}: {result}")
                        total_alerts_generated += 1
                        
                except Exception as e:
                    self.get_task_logger().error(f"Error checking alerts for patient {patient.id}: {e}")
                    continue
...
def generate_alert_metrics(self):
    ...
    try:
        with get_db_session() as db:
            ...
            recent_alerts = (
                db.query(Alert)
                .filter(Alert.created_at >= yesterday)
                .all()
            )
            
            stats["recent_metrics"] = {
                "alerts_last_24h": len(recent_alerts),
                "critical_last_24h": len([a for a in recent_alerts if a.severity == AlertSeverity.CRITICAL]),
                "avg_response_time_minutes": 0,  # Would calculate from acknowledgment times
                "escalation_rate": len([a for a in recent_alerts if a.data and a.data.get("escalation", {}).get("current_escalation", 0) > 0]) / len(recent_alerts) if recent_alerts else 0
            }
        ...
        with get_db_session() as db:
            ...
            for alert in alerts_needing_escalation:
                ...
                if next_escalation_str:
                    next_escalation = datetime.fromisoformat(next_escalation_str.replace('Z', '+00:00'))
