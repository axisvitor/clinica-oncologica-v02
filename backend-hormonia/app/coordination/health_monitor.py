...
class AgentHealthMonitor:
    ...
    async def update_metrics(
        ...
        try:
            ...
            if len(self.metrics_history) > self.config["metrics_history_limit"]:
                self.metrics_history = self.metrics_history[-self.config["metrics_history_limit"]:]
    ...
    def _determine_health_status(
        ...
        if (response_time_ms > self.config["response_time_threshold"] * 2 or
            error_rate > self.config["error_rate_threshold"] * 3 or
            cpu_usage > 95.0 or
            memory_usage > 95.0):
            return HealthStatus.CRITICAL
        
        # Check for warning conditions
        if (response_time_ms > self.config["response_time_threshold"] or
        ...
        if self.current_metrics:
            time_since_heartbeat = (datetime.utcnow() - self.current_metrics.last_heartbeat).total_seconds()
    ...
    async def _check_health_alerts(self):
        ...
        if metrics.response_time_ms > self.config["response_time_threshold"]:
            await self._create_alert(
                AlertSeverity.WARNING if metrics.response_time_ms < self.config["response_time_threshold"] * 2 else AlertSeverity.CRITICAL,
                "High Response Time",
        ...
        if metrics.error_rate > self.config["error_rate_threshold"]:
            await self._create_alert(
                AlertSeverity.WARNING if metrics.error_rate < self.config["error_rate_threshold"] * 2 else AlertSeverity.CRITICAL,
                "High Error Rate",
        ...
        if metrics.cpu_usage > self.config["cpu_threshold"]:
            await self._create_alert(
                AlertSeverity.WARNING if metrics.cpu_usage < 95.0 else AlertSeverity.CRITICAL,
                "High CPU Usage",
        ...
        if metrics.memory_usage > self.config["memory_threshold"]:
            await self._create_alert(
                AlertSeverity.WARNING if metrics.memory_usage < 95.0 else AlertSeverity.CRITICAL,
                "High Memory Usage",
...
class SystemHealthMonitor:
    ...
    async def update_agent_metrics(
        ...
        try:
            ...
            await monitor.update_metrics(
                response_time_ms, tasks_completed, tasks_failed,
        ...
        try:
            ...
            for monitor in self.agent_monitors.values():
                monitor.active_alerts = [
                    alert for alert in monitor.active_alerts
    ...
    def get_system_overview(self) -> Dict[str, Any]:
        ...
        overview = {
            ...
            "active_system_alerts": len([a for a in self.system_alerts if a.resolved_at is None]),
