...
class ConsensusManager:
    ...
    async def propose_decision(
        ...
        self.logger.info(f"Created consensus proposal {proposal_id} of type {proposal_type}")
    ...
    async def cast_vote(
        ...
        if proposal_id not in self.active_proposals:
            self.logger.warning(f"Vote rejected: proposal {proposal_id} not found or expired")
            return False
        
        # Check if agent already voted
        existing_votes = [v for v in self.proposal_votes[proposal_id] if v.voter_id == voter_id]
        if existing_votes:
            self.logger.warning(f"Vote rejected: agent {voter_id} already voted on {proposal_id}")
            return False
        ...
        self.logger.info(f"Recorded vote from {voter_id} for proposal {proposal_id}: {vote_type.value}")
    ...
    async def _check_consensus_completion(self, proposal_id: str):
        ...
        if proposal.required_consensus == ConsensusType.SIMPLE_MAJORITY:
            ...
            self.logger.error(f"Unsupported consensus type: {proposal.required_consensus}")
            return
    ...
    async def _apply_simple_majority(self, proposal: ConsensusProposal, votes: List[Vote]) -> ConsensusResult:
        ...
        if consensus_reached:
            ...
            confidence_score = sum(v.confidence for v in winning_votes) / len(winning_votes)
        else:
        ...
        return ConsensusResult(
            ...
            reasoning=f"Simple majority: {vote_counts[decision]}/{total_votes}" if consensus_reached else "No majority reached"
        )
    
    async def _apply_weighted_voting(self, proposal: ConsensusProposal, votes: List[Vote]) -> ConsensusResult:
        """Apply weighted voting consensus."""
        weighted_scores = {VoteType.APPROVE: 0.0, VoteType.REJECT: 0.0, VoteType.ABSTAIN: 0.0}
        total_weight = 0.0
        ...
        consensus_reached = (winning_score > 0.5 and 
                           winning_score >= self.min_confidence_threshold)
    ...
    async def _apply_byzantine_consensus(self, proposal: ConsensusProposal, votes: List[Vote]) -> ConsensusResult:
        ...
        if consensus_reached:
            ...
            confidence_score = sum(v.confidence for v in agreeing_votes) / len(agreeing_votes)
        else:
            confidence_score = 0.0
        ...
        return ConsensusResult(
            ...
            completed_at=datetime.utcnow(),
            reasoning=f"Byzantine consensus: {vote_counts[decision]}/{n} agree (required: {required_agreement})" if consensus_reached else "Insufficient agreement for Byzantine consensus"
        )
    
    async def _apply_quorum_consensus(self, proposal: ConsensusProposal, votes: List[Vote]) -> ConsensusResult:
    ...
    async def _complete_consensus(self, proposal_id: str, result: ConsensusResult):
        ...
        if proposal:
            ...
            self.logger.info(f"Consensus completed for {proposal_id}: {result.decision.value} (confidence: {result.confidence_score:.2f})")
    ...
    async def _handle_proposal_timeout(self, proposal_id: str, timeout_seconds: int):
        ...
        if proposal_id in self.active_proposals:
            ...
            self.logger.warning(f"Proposal {proposal_id} timed out with {len(votes)} votes")
    ...
    def cleanup_old_consensus(self, days_old: int = 7):
        ...
        to_remove = [
            proposal_id for proposal_id, result in self.completed_consensus.items()
    ...
    def get_consensus_stats(self) -> Dict[str, Any]:
        ...
        if completed_count > 0:
            successful_consensus = sum(1 for result in self.completed_consensus.values() 
                                     if result.consensus_reached)
            success_rate = successful_consensus / completed_count
            
            avg_participants = sum(result.participants for result in self.completed_consensus.values()) / completed_count
            avg_confidence = sum(result.confidence_score for result in self.completed_consensus.values()) / completed_count
        else:
            success_rate = 0.0
