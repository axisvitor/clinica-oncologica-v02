...
class QuizTemplate(BaseModel):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "LENGTH(version) >= 1", name="ck_quiz_template_version_not_empty"
        ),
        CheckConstraint(
            "questions IS NOT NULL", name="ck_quiz_template_questions_not_null"
        ),
...
class QuizSession(BaseModel):
    ...
    quiz_template_id = Column(
        UUID(as_uuid=True),
        ForeignKey("quiz_templates.id", ondelete="RESTRICT"),
        nullable=False,
    )
    ...
    )  # Auto-calculated: started_at + 48 hours
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "current_question >= 0", name="ck_quiz_session_question_positive"
        ),
    ...
    def is_expired(self) -> bool:
        ...
        if (
            ...
            and datetime.now(self.expiration_date.tzinfo) >= self.expiration_date
        ):
            return True
        return False
...
class QuizResponse(BaseModel):
    ...
    __table_args__ = (
        ...
        # Check constraints
        CheckConstraint(
            "LENGTH(question_id) >= 1", name="ck_quiz_response_question_id_not_empty"
        ),
        CheckConstraint(
            "LENGTH(question_text) >= 1",
            name="ck_quiz_response_question_text_not_empty",
        ),
        CheckConstraint(
            "LENGTH(response_value) >= 1", name="ck_quiz_response_value_not_empty"
        ),
        CheckConstraint(
