...
from sqlalchemy import Column, String, DateTime, Integer, Boolean, ForeignKey, Index, Text, Float
...
class WebhookEndpoint(Base):
    ...
    status = Column(String(20), default="active", nullable=False, comment="active, inactive, paused, error")
    
    # Configuration
    secret = Column(String(255), nullable=True, comment="HMAC secret key")
    events = Column(JSONB, nullable=False, default=list, comment="List of subscribed events")
    headers = Column(JSONB, nullable=True, default=dict, comment="Custom headers")
    
    # Reliability settings
    timeout = Column(Integer, default=30, nullable=False, comment="Request timeout in seconds")
...
class WebhookDelivery(Base):
    ...
    # Delivery details
    status = Column(String(20), nullable=False, comment="pending, success, failed, retrying")
...
class WebhookLog(Base):
    ...
    event_type = Column(String(100), nullable=False, comment="e.g., created, updated, secret_rotated")
