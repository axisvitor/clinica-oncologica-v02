...
"""
from sqlalchemy import Column, String, Text, Date, ForeignKey, Enum, DateTime, LargeBinary
...
class Report(BaseModel):
    ...
    pdf_data = Column(LargeBinary, nullable=True) # Store PDF bytes directly or use path
    status = Column(Enum(ReportStatus), default=ReportStatus.PENDING)
    generated_at = Column(DateTime, default=datetime.utcnow)
    report_metadata = Column("metadata", JSONB, nullable=True) # metadata is reserved in Base? No, usually metadata in sqlalchemy is Base.metadata. 
