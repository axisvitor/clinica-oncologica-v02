...
class MessageStatusEvent(BaseModel):
    ...
    whatsapp_timestamp = Column(
        DateTime(timezone=True), nullable=True
    )  # WhatsApp event timestamp

    # Error tracking
    error_code = Column(
        String(50), nullable=True, index=True
    )  # Error code from Evolution API
    ...
    # Evolution API data
    evolution_event_type = Column(
        String(100), nullable=True
    )  # Raw Evolution API event type
    evolution_payload = Column(
        JSONB, nullable=True
    )  # Full Evolution API payload for debugging
...
class EvolutionWebhookEvent(BaseModel):
    ...
    )  # message.sent, message.delivered, etc.
    source = Column(
        String(100), nullable=False, index=True
    )  # evolution_api, whatsapp, system
    ...
    )  # Related message if identified
    related_patient_id = Column(
        UUID(as_uuid=True), nullable=True, index=True
    )  # Related patient if identified
