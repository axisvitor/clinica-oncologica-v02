...
class LGPDAuditLog(BaseModel):
    ...
    )  # Denormalized for when user is deleted
...
class DataAccessRequest(BaseModel):
    ...
    patient = relationship(
        "Patient", back_populates="data_access_requests", lazy="select"
    )
    assigned_to = relationship("User", foreign_keys=[assigned_to_id], lazy="select")

    __table_args__ = (Index("ix_dsar_status_deadline", "status", "deadline_at"),)
...
__all__ = ["LGPDActionType", "LGPDDataCategory", "LGPDAuditLog", "DataAccessRequest"]
