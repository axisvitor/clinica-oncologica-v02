...
class Alert(BaseModel):
    ...
    # Quiz session reference (for quiz-response alerts)
    quiz_session_id = Column(UUID(as_uuid=True), ForeignKey("quiz_sessions.id", ondelete="SET NULL"), nullable=True)

    # Alert details
    alert_type = Column(String(100), nullable=False)  # 'urgency', 'adherence', 'symptom', 'quiz_response'
