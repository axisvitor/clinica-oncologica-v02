...
class PatientReportData(BaseModel):
    ...
    generated_date: datetime = Field(
        default_factory=datetime.utcnow, description="Report generation date"
    )

    # Report sections
    executive_summary: str = Field(..., description="Executive summary")
    treatment_progress: ReportSection = Field(
        ..., description="Treatment progress section"
    )
    symptoms_analysis: ReportSection = Field(
        ..., description="Symptoms analysis section"
    )
    medication_adherence: ReportSection = Field(
        ..., description="Medication adherence section"
    )
    recommendations: ReportSection = Field(..., description="Recommendations section")

    # Data tables
    quiz_responses: List[Dict[str, Any]] = Field(
        default_factory=list, description="Quiz response data"
    )
    message_statistics: Dict[str, Any] = Field(
        default_factory=dict, description="Message statistics"
    )
    alert_summary: List[Dict[str, Any]] = Field(
        default_factory=list, description="Alert summary"
    )
...
class MedicalReportGenerator:
    ...
    def _create_header(self, report_data: PatientReportData) -> List:
        ...
        patient_info = [
            ...
            [
                "Generated:",
                report_data.generated_date.strftime("%B %d, %Y at %I:%M %p"),
            ],
        ]
    ...
    def _create_section(
        self, section: ReportSection, style_name: str = "SectionHeader"
    ) -> List:
    ...
    def _create_quiz_responses_table(
        ...
        elements = []
        elements.append(
            Paragraph("Quiz Responses Summary", self.styles["SectionHeader"])
        )
        ...
        # Create table
        quiz_table = Table(
            table_data, colWidths=[1.5 * inch, 2 * inch, 1 * inch, 2.5 * inch]
        )
        quiz_table.setStyle(
            TableStyle(
                [
    ...
    def _create_message_statistics(self, message_stats: Dict[str, Any]) -> List:
        ...
        elements = []
        elements.append(
            Paragraph("Communication Statistics", self.styles["SectionHeader"])
        )
    ...
    def _create_alert_summary(self, alerts: List[Dict[str, Any]]) -> List:
        ...
        for alert in alerts:
            alert_data.append(
                [
                    ...
                    if len(alert.get("description", "")) > 50
        ...
        alert_table = Table(
            alert_data, colWidths=[1 * inch, 1 * inch, 1.5 * inch, 2.5 * inch, 1 * inch]
        )
    ...
    def generate_patient_report(
        self, report_data: PatientReportData, output_path: Optional[str] = None
    ) -> bytes:
        """
        ...
        try:
            ...
            elements.extend(
                self._create_quiz_responses_table(report_data.quiz_responses)
            )

            # Message statistics
            elements.extend(
                self._create_message_statistics(report_data.message_statistics)
            )
