...
class QueueManager:
    ...
    def __init__(self, default_instance: str = "primary"):
        ...
        self._stats = {"messages_sent": 0, "messages_failed": 0, "last_activity": None}
        logger.info(
            f"QueueManager initialized with default instance: {default_instance}"
        )
    ...
    async def get_queue_status(
        ...
        processing = len(
            [m for m in self._processing.values() if m.instance_name == queue_name]
        )
        failed = len(
            [m for m in self._failed.values() if m.instance_name == queue_name]
        )
