...
class MessagePersonalizationRequest(BaseModel):
    ...
    patient_context: Dict[str, Any] = Field(
        default_factory=dict, description="Patient context data"
    )
    treatment_day: int = Field(..., description="Current treatment day")
    previous_responses: List[str] = Field(
        default_factory=list, description="Recent patient responses"
    )
...
class SentimentAnalysisRequest(BaseModel):
    ...
    patient_context: Dict[str, Any] = Field(
        default_factory=dict, description="Patient context"
    )
...
class PersonalizationResponse(BaseModel):
    ...
    personalization_notes: List[str] = Field(
        default_factory=list, description="Applied personalizations"
    )
...
class SentimentAnalysisResponse(BaseModel):
    ...
    key_phrases: List[str] = Field(
        default_factory=list, description="Important phrases detected"
    )
    medical_concerns: List[str] = Field(
        default_factory=list, description="Medical concerns if any"
    )


class GeminiClientError(ExternalServiceError):
...
class LangChainOrchestrator:
    ...
    def __init__(
        ...
        if not self.api_key:
            raise GeminiClientError("Gemini API key is required but not provided")
    ...
    async def humanize_message(
        ...
        Raises:
            GeminiClientError: On AI service failures
        """
        try:
            # Format the prompt
            ...
            raise GeminiClientError(f"Failed to humanize message: {str(e)}")
    ...
    async def analyze_sentiment(
        ...
        try:
            ...
            messages = self.sentiment_template.format_messages(
                message=request.message, patient_context=str(request.patient_context)
            )
            ...
            raise GeminiClientError(f"Failed to analyze sentiment: {str(e)}")
    ...
    async def generate_text(self, prompt: str) -> str:
        ...
        try:
            ...
            raise GeminiClientError(f"Failed to generate text: {str(e)}")
    ...
    async def generate_contextual_response(
        ...
        try:
            ...
            raise GeminiClientError(f"Failed to generate response: {str(e)}")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        Raises:
            GeminiClientError: If health check fails
        """
        try:
            # Simple test prompt
            test_messages = [
...
def get_gemini_orchestrator() -> LangChainOrchestrator:  # pragma: no cover
    """Alias for clarity in Gemini-only deployments."""
