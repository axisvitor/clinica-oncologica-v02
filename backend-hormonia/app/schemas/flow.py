...
class FlowTemplateUpdate(BaseModel):
    ...
    duration_days: Optional[int] = Field(
        None, ge=1, le=365, description="Flow duration in days"
    )
...
class PatientFlowStateResponse(PatientFlowStateBase):
    ...
    completed_at: Optional[datetime] = Field(
        None, description="Flow completion timestamp"
    )
    created_at: datetime = Field(..., description="Creation timestamp")
    updated_at: datetime = Field(..., description="Last update timestamp")
...
class FlowProgressionRequest(BaseModel):
    ...
    force_advance: bool = Field(
        default=False, description="Force advancement even if conditions not met"
    )
...
class FlowProgressionResponse(BaseModel):
    ...
    next_actions: List[str] = Field(
        default_factory=list, description="Next scheduled actions"
    )
...
class FlowResetRequest(BaseModel):
    ...
    preserve_history: bool = Field(
        default=True, description="Whether to preserve flow history"
    )
...
class FlowHistoryResponse(BaseModel):
    ...
    patient_id: UUID = Field(..., description="Patient ID")
    flow_states: List[PatientFlowStateResponse] = Field(
        ..., description="Historical flow states"
    )
    total_flows: int = Field(..., description="Total number of flows")
    current_flow: Optional[PatientFlowStateResponse] = Field(
        None, description="Current active flow"
    )
...
class FlowStepDefinition(BaseModel):
    ...
    actions: List[dict[str, Any]] = Field(
        default_factory=list, description="Step actions"
    )
...
class FlowTemplateValidationResult(BaseModel):
    ...
    template_version: Optional[str] = Field(
        None, description="Template version validated"
    )
...
class FlowAnalytics(BaseModel):
    ...
    step_analytics: List[dict[str, Any]] = Field(
        default_factory=list, description="Per-step analytics"
    )
    common_exit_points: List[dict[str, Any]] = Field(
        default_factory=list, description="Common exit points"
    )
...
class FlowTemplateListResponse(BaseModel):
    ...
    templates: List[FlowTemplateResponse] = Field(
        ..., description="List of flow templates"
    )
...
class PatientFlowStateListResponse(BaseModel):
    ...
    flow_states: List[PatientFlowStateResponse] = Field(
        ..., description="List of flow states"
    )
...
class FlowOverrideRequest(BaseModel):
    ...
    override_type: str = Field(
        ..., description="Type of override (pause, resume, skip, restart)"
    )
    ...
    duration_hours: Optional[int] = Field(
        None, description="Duration for pause (in hours)"
    )
...
class FlowOverrideResponse(BaseModel):
    ...
    expires_at: Optional[datetime] = Field(
        None, description="When override expires (for temporary overrides)"
    )
...
class FlowStateResponse(BaseModel):
    ...
    flow_state: Optional[FlowStateData] = Field(
        None, description="Current flow state data"
    )
...
class FlowAdvancementResponse(BaseFlowResponse):
    ...
    advancement_result: dict[str, Any] = Field(
        ..., description="Advancement operation result"
    )
...
class FlowPauseResponse(BaseFlowResponse):
    ...
    auto_resume_at: Optional[datetime] = Field(
        None, description="Auto-resume timestamp"
    )
...
class FlowHistoryItem(BaseModel):
    ...
    state_data: FlowStateData = Field(
        default_factory=dict, description="Flow state data"
    )
...
class FlowHistoryPaginatedResponse(BaseModel):
    """Response model for flow history endpoints with pagination."""

    patient_id: UUID = Field(..., description="Patient ID")
    flow_history: List[FlowHistoryItem] = Field(..., description="Flow history items")
    current_flow: Optional[FlowHistoryItem] = Field(
        None, description="Current active flow"
    )
    total_flows: int = Field(..., description="Total number of flows")
...
class FlowCustomizationRequest(BaseModel):
    ...
    conditions: Optional[dict[str, Any]] = Field(
        None, description="Conditions for applying customization"
    )
    expires_at: Optional[datetime] = Field(
        None, description="Expiration date for customization"
    )
...
# NOTE: FlowAdvanceRequest, FlowPauseRequest, FlowPauseResponse, FlowResumeResponse,
# FlowHistoryItem, FlowHistoryResponse, FlowAnalytics are defined earlier in this file.
# Duplicates removed to avoid F811 errors.
