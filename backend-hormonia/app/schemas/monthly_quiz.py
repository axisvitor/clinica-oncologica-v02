...
class MonthlyQuizLinkCreate(BaseModel):
    ...
    delivery_method: DeliveryMethod = Field(
        ...
        description="Delivery method for the quiz link"
    )
    expiry_hours: Optional[int] = Field(
        default=72,
        description="Link expiry in hours (default: 72)"
    )
...
class MonthlyQuizLinkResponse(BaseModel):
    ...
    patient_phone: Optional[str] = Field(None, description="Patient phone for dashboard")
    template_name: Optional[str] = Field(None, description="Template name for dashboard")
    template_version: Optional[str] = Field(None, description="Template version for dashboard")
...
class MonthlyQuizSubmitResponse(BaseModel):
    ...
    response_value: Any = Field(..., description="Response value (string or list for multiple choice)")
...
class BulkQuizLinkCreate(BaseModel):
    ...
    delivery_method: DeliveryMethod = Field(
        default=DeliveryMethod.WHATSAPP,
        description="Delivery method for all links"
    )
    expiry_hours: Optional[int] = Field(
        default=72,
...
class BulkQuizLinkResponse(BaseModel):
    ...
    failures: List[Dict[str, Any]] = Field(
        ...
        description="Failed creations with reasons"
    )
