...
class UserUpdate(BaseModel):
    ...
    name: Optional[str] = Field(
        None, min_length=2, max_length=255, description="Full name of the user"
    )
    email: Optional[EmailStr] = Field(
        None, description="Email address (must be unique)"
    )
...
class UserResponse(BaseModel):
    ...
    total_patients: Optional[int] = Field(
        None, description="Total patients for doctors"
    )
...
class PasswordReset(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"new_password": "NewSecurePass123"}}
    )
...
class UserFilter(BaseModel):
    ...
    has_patients: Optional[bool] = Field(
        None, description="Filter doctors by whether they have patients"
    )
