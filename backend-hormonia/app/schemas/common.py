...
class UnauthorizedErrorResponse(BaseModel):
    ...
    message: str = Field("Authentication required", description="Human readable error message")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Error timestamp")
...
class ForbiddenErrorResponse(BaseModel):
    ...
    message: str = Field("Insufficient permissions", description="Human readable error message")
...
class RateLimitErrorResponse(BaseModel):
    ...
    message: str = Field("Rate limit exceeded", description="Human readable error message")
...
class ServiceUnavailableErrorResponse(BaseModel):
    ...
    message: str = Field("Service temporarily unavailable", description="Human readable error message")
    retry_after: Optional[int] = Field(None, description="Seconds to wait before retrying")
    maintenance_mode: bool = Field(False, description="Whether service is in maintenance mode")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Error timestamp")
