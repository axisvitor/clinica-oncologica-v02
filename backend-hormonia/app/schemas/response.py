...
class SentimentAnalysisResult(BaseModel):
    ...
    emotional_indicators: List[str] = Field(default_factory=list, description="Emotional indicators")
...
class StructuredResponseData(BaseModel):
    ...
    sentiment_analysis: SentimentAnalysisResult = Field(..., description="Sentiment analysis")
    medical_concerns: List[str] = Field(default_factory=list, description="Medical concerns")
...
class ResponseProcessingResult(BaseModel):
    ...
    patient_id: UUID = Field(..., description="Patient ID")
    structured_response: StructuredResponseData = Field(..., description="Structured response data")
    flow_actions: List[FlowActionData] = Field(..., description="Flow actions to execute")
...
class PatientResponseSummary(BaseModel):
    ...
    avg_response_time_minutes: Optional[float] = Field(None, description="Average response time")
    sentiment_distribution: dict[str, int] = Field(..., description="Sentiment distribution")
    concern_levels: dict[str, int] = Field(..., description="Concern level distribution")
...
class ResponseAnalyticsRequest(BaseModel):
    ...
    concern_level: Optional[ConcernLevelEnum] = Field(None, description="Concern level filter")
    response_type: Optional[ResponseTypeEnum] = Field(None, description="Response type filter")
...
class ResponseAnalyticsResult(BaseModel):
    ...
    total_responses: int = Field(..., description="Total responses")
    response_summaries: List[PatientResponseSummary] = Field(..., description="Patient summaries")
    sentiment_trends: dict[str, List[dict[str, Any]]] = Field(..., description="Sentiment trends")
    concern_trends: dict[str, List[dict[str, Any]]] = Field(..., description="Concern trends")
...
class TextPatternExtractionResult(BaseModel):
    ...
    medication_mentioned: Optional[bool] = Field(None, description="Medication mentioned")
...
class MedicalConcernAlert(BaseModel):
    ...
    requires_immediate_attention: bool = Field(..., description="Immediate attention flag")
...
class ResponseProcessingHealthCheck(BaseModel):
    ...
    processing_stats: ResponseProcessingStats = Field(..., description="Processing statistics")
...
class BulkResponseProcessingResult(BaseModel):
    ...
    results: List[ResponseProcessingResult] = Field(..., description="Individual results")
