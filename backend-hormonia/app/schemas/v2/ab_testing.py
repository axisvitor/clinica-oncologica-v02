...
class VariantConfig(BaseModel):
    ...
    traffic_weight: float = Field(
        ..., gt=0, le=1, description="Traffic allocation (0-1)"
    )
    configuration: Dict[str, Any] = Field(
        default_factory=dict, description="Variant-specific config"
    )
...
class StatisticalTestResult(BaseModel):
    ...
    effect_size_interpretation: Optional[
...
class Segment(BaseModel):
    ...
    is_exclusive: bool = Field(
        False, description="Mutually exclusive with other segments"
    )
...
class ExperimentCreate(BaseModel):
    ...
    auto_declare_threshold: Optional[float] = Field(
        None, ge=0.9, le=0.99, description="Confidence threshold for auto winner"
    )
...
class VariantAssignmentRequest(BaseModel):
    ...
    user_attributes: Optional[Dict[str, Any]] = Field(
        None, description="User attributes for segmentation"
    )
    force_variant: Optional[VariantType] = Field(
        None, description="Force specific variant (testing only)"
    )
