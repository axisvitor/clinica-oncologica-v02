...
class HealthConcern(BaseModel):
    ...
    detected_date: Optional[date] = Field(
        None, description="When the concern was detected"
    )
    source: Optional[str] = Field(
        None, description="Source of detection (quiz, message, etc.)"
    )
...
class QuizFindings(BaseModel):
    ...
    key_findings: List[str] = Field(
        default_factory=list, description="Key findings from responses"
    )
    symptom_trends: Dict[str, str] = Field(
        default_factory=dict, description="Symptom trends (symptom: trend)"
    )
    concerning_responses: List[str] = Field(
        default_factory=list, description="Responses that need attention"
    )
...
class EngagementMetrics(BaseModel):
    ...
    avg_response_time_minutes: float = Field(
        0.0, ge=0, description="Average response time in minutes"
    )
    total_messages_sent: int = Field(
        0, ge=0, description="Total messages sent to patient"
    )
    total_messages_received: int = Field(
        0, ge=0, description="Total messages received from patient"
    )
    engagement_score: float = Field(
        0.0, ge=0, le=100, description="Overall engagement score (0-100)"
    )
...
class TreatmentCompliance(BaseModel):
    ...
    missed_interactions: int = Field(
        0, ge=0, description="Number of missed interactions"
    )
...
class SummaryContent(BaseModel):
    ...
    treatment_compliance: TreatmentCompliance = Field(
        default_factory=TreatmentCompliance
    )
    recommendations: List[str] = Field(
        default_factory=list, description="3-5 actionable recommendations"
    )
...
class GenerateSummaryRequest(BaseModel):
    ...
    include_sections: Optional[List[str]] = Field(
        None, description="Specific sections to include (all if not specified)"
    )
