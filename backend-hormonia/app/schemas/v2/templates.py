...
class FlowTemplateV2Base(BaseModel):
    ...
    steps: Dict[str, Any] = Field(..., description="Template steps/messages configuration")
...
class FlowTemplateV2Create(FlowTemplateV2Base):
    ...
    kind_key: Optional[str] = Field(None, max_length=100, description="Flow kind key (for new kinds)")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "description": "Standard flow for hormonal treatment patients",
                "steps": {
                    "day_1": {"message": "Welcome", "type": "greeting"}
                },
...
class QuizQuestionSchema(BaseModel):
    ...
    type: str = Field(..., description="Question type (multiple_choice, open_text, scale, etc.)")
...
class QuizTemplateV2Base(BaseModel):
    ...
    randomize_questions: Optional[bool] = Field(None, description="Randomize question order")
    is_active: Optional[bool] = Field(None, description="Active status")
...
class FlowKindV2Base(BaseModel):
    ...
    kind_key: str = Field(..., min_length=1, max_length=100, description="Unique kind key")
...
class FlowKindV2Response(BaseModel):
    ...
    published_versions: Optional[int] = Field(None, description="Published version count")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "template_id": "123e4567-e89b-12d3-a456-426614174000",
                "rendered_content": {
                    "message": "Hello João Silva, welcome to day 7!"
                },
