    ...
    model_config = ConfigDict(
        ...
        json_schema_extra = {
            "example": {
                ...
                "personalization_notes": ["Added patient name", "Used encouraging tone"],
                ...
                "cache_info": {"hit": False, "key": "ai:humanize:xyz", "ttl_seconds": 7200},
...
class SentimentAnalysisRequest(BaseModel):
    ...
    include_medical_concerns: bool = Field(
        True,
        description="Detect medical concerns"
    )
    include_urgency: bool = Field(
        True,
...
class SentimentAnalysisResponse(BaseModel):
    ...
    medical_concerns: List[str] = Field(
        default_factory=list,
        description="Medical concerns detected"
    )
    ...
    emotion_scores: Dict[str, float] = Field(
        default_factory=dict,
        description="Emotion scores (0-1)"
    )
...
class ResponseQualityResponse(BaseModel):
    ...
    readability_score: float = Field(ge=0.0, le=100.0, description="Readability (0-100)")
    empathy_score: float = Field(ge=0.0, le=1.0, description="Empathy score (0-1)")
    professionalism_score: float = Field(
        ge=0.0, le=1.0,
        description="Professionalism (0-1)"
    )
...
class CacheStatsResponse(BaseModel):
    ...
    by_endpoint: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict,
        description="Stats by endpoint"
    )
    oldest_entry_age_seconds: Optional[int] = Field(
        None,
        description="Age of oldest entry"
    )
    generated_at: datetime = Field(default_factory=datetime.utcnow)
