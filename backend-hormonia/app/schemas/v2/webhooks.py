...
class WebhookCreate(BaseModel):
    ...
    secret: Optional[str] = Field(
        ...
        min_length=16,
        max_length=256,
        description="Custom HMAC secret (auto-generated if not provided)",
    )
...
class WebhookUpdate(BaseModel):
    ...
    events: Optional[List[WebhookEventType]] = Field(
        None, min_length=1, description="Events to subscribe to"
    )
    description: Optional[str] = Field(
        None, max_length=500, description="Webhook description"
    )
...
class WebhookTestRequest(BaseModel):
    ...
    payload: Optional[Dict[str, Any]] = Field(
        default_factory=dict, description="Custom test payload"
    )
...
class WebhookSecretRotate(BaseModel):
    ...
    new_secret: Optional[str] = Field(
        ...
        min_length=16,
        max_length=256,
        description="New secret (auto-generated if not provided)",
    )

    model_config = ConfigDict(json_schema_extra={"example": {"new_secret": None}})
...
class WebhookHealth(BaseModel):
    ...
    last_failure_at: Optional[datetime] = Field(
        None, description="Last failed delivery"
    )
...
class WebhookEventTypeList(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"events": [], "total": 14}}
    )
...
class FailedWebhookList(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"example": {"data": [], "total": 3}})
