...
class TaskV2Base(BaseModel):
    ...
    description: Optional[constr(max_length=1000)] = Field(
        None, description="Detailed task description"
    )
    metadata: Optional[Dict[str, Any]] = Field(
        default=None, description="Additional task metadata"
    )
...
class RetryConfigV2(BaseModel):
    ...
    base_delay: conint(ge=1, le=3600) = Field(
        default=60, description="Base delay in seconds for retry"
    )
    max_delay: conint(ge=60, le=86400) = Field(
        default=3600, description="Maximum delay in seconds between retries"
    )
...
class TaskProgressV2(BaseModel):
    ...
    total: conint(ge=0) = Field(default=100, description="Total work units")
    message: Optional[str] = Field(None, max_length=500, description="Progress message")
...
class TaskV2Create(TaskV2Base):
    ...
    timeout_seconds: Optional[conint(ge=1, le=86400)] = Field(
        None, description="Task execution timeout in seconds"
    )
...
class TaskV2Cancel(BaseModel):
    ...
    force: bool = Field(default=False, description="Force termination of running task")

    model_config = ConfigDict(
        json_schema_extra={
            "example": {"reason": "Task no longer needed", "force": False}
        }
    )
...
class TaskV2Retry(BaseModel):
    ...
    override_retry_limit: bool = Field(
        default=False, description="Retry even if max retries exceeded"
    )
    delay_seconds: Optional[conint(ge=0, le=3600)] = Field(
        None, description="Custom delay before retry"
    )
    notes: Optional[constr(max_length=500)] = Field(
        None, description="Notes about manual retry"
    )
...
class TaskV2Response(TaskV2Base):
    ...
    retry_count: conint(ge=0) = Field(default=0, description="Number of retry attempts")
    retry_config: Optional[RetryConfigV2] = Field(
        None, description="Retry configuration"
    )
    worker_name: Optional[str] = Field(
        None, description="Name of worker executing the task"
    )
    ...
    started_at: Optional[datetime] = Field(
        None, description="When task started executing"
    )
    completed_at: Optional[datetime] = Field(None, description="When task completed")
    scheduled_at: Optional[datetime] = Field(
        None, description="When task is scheduled to run"
    )
...
class TaskV2WithLogs(TaskV2Response):
    ...
    logs: List[TaskLogEntryV2] = Field(
        default_factory=list, description="Task log entries"
    )
...
class TaskStatisticsV2(BaseModel):
    ...
    avg_runtime_seconds: confloat(ge=0) = Field(description="Average task runtime")
    avg_wait_time_seconds: confloat(ge=0) = Field(
        description="Average time tasks wait before execution"
    )
    success_rate: confloat(ge=0, le=100) = Field(description="Success rate percentage")

    tasks_by_type: Dict[str, int] = Field(
        default_factory=dict, description="Task counts by type"
    )
    tasks_by_priority: Dict[str, int] = Field(
        default_factory=dict, description="Task counts by priority"
    )

    slowest_tasks: List[Dict[str, Any]] = Field(
        default_factory=list, description="Slowest tasks in period"
    )

    analysis_period_hours: conint(ge=1) = Field(description="Analysis period in hours")
...
class QueueStatusV2(BaseModel):
    ...
    workers: List[str] = Field(
        default_factory=list, description="Workers consuming from this queue"
    )
    avg_processing_time: Optional[float] = Field(
        None, description="Average task processing time"
    )
...
class BulkTaskOperation(BaseModel):
    ...
    reason: Optional[constr(max_length=500)] = Field(
        None, description="Reason for bulk operation"
    )
...
class BulkTaskResult(BaseModel):
    ...
    failed_ids: List[str] = Field(
        default_factory=list, description="IDs that failed to process"
    )
    errors: Dict[str, str] = Field(
        default_factory=dict, description="Error messages by task ID"
    )
...
class TaskCleanupConfigV2(BaseModel):
    ...
    days_old: conint(ge=1, le=365) = Field(
        default=90, description="Delete tasks older than this many days"
    )
    status_filter: Optional[List[TaskStatus]] = Field(
        None, description="Only clean tasks with these statuses"
    )
    task_types: Optional[List[TaskType]] = Field(
        None, description="Only clean these task types"
    )
    dry_run: bool = Field(default=True, description="Preview cleanup without deleting")
    batch_size: conint(ge=1, le=1000) = Field(
        default=100, description="Process in batches of this size"
    )
