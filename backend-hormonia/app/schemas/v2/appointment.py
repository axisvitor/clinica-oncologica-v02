...
class AppointmentV2Base(BaseModel):
    ...
    status: str = Field(
        default="scheduled", max_length=50, description="Appointment status"
    )
    scheduled_at: Optional[datetime] = Field(
        None, description="Scheduled date and time"
    )
    duration_minutes: Optional[int] = Field(
        None, ge=1, le=480, description="Duration in minutes"
    )
    pre_appointment_notes: Optional[str] = Field(
        None, description="Notes before appointment"
    )
    post_appointment_notes: Optional[str] = Field(
        None, description="Notes after appointment"
    )
...
class AppointmentV2Create(AppointmentV2Base):
    ...
    appointment_type: str = Field(
        ..., description="consultation|followup|treatment|exam|emergency|telemedicine"
    )
    ...
    def validate_appointment_type(cls, v):
        ...
        if v not in valid_types:
            raise ValueError(
                f"appointment_type must be one of: {', '.join(valid_types)}"
            )
        return v
