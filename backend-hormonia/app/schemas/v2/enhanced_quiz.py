...
class BranchingRule(BaseModel):
    ...
    conditions: List[BranchingCondition] = Field(..., description="List of conditions (AND logic)")
    logic: str = Field(default="AND", description="Logic operator for conditions")
    next_question_id: Optional[str] = Field(None, description="Next question if condition matches")
...
class AdvancedQuizTemplate(BaseModel):
    ...
    difficulty: QuizDifficulty = Field(default=QuizDifficulty.BASIC, description="Difficulty level")
    ...
    show_results_immediately: bool = Field(default=True, description="Show results after completion")
...
class QuizAnalyticsResponse(BaseModel):
    ...
    average_score: Optional[float] = Field(None, ge=0, description="Average score")
    average_time_minutes: Optional[float] = Field(None, ge=0, description="Average completion time")
    trends: List[QuizAnalyticsTrend] = Field(default_factory=list, description="Trend data")
    category_breakdown: Dict[str, int] = Field(default_factory=dict, description="Sessions by category")
...
class AdaptiveQuizFlowResponse(BaseModel):
    ...
    next_question: Optional[QuizQuestion] = Field(None, description="Next question to display")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "risk_score": 78.5,
                "risk_factors": ["High pain level", "Poor medication adherence", "Weight loss"],
                "recommendations": [
...
class BulkQuizOperation(BaseModel):
    ...
    patient_ids: List[str] = Field(..., min_items=1, description="List of patient UUIDs")
    template_id: Optional[str] = Field(None, description="Template UUID for assign operation")
...
class QuizExportRequest(BaseModel):
    ...
    patient_ids: Optional[List[str]] = Field(None, description="Filter by patient UUIDs")
    template_ids: Optional[List[str]] = Field(None, description="Filter by template UUIDs")
