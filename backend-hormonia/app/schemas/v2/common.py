...
class PaginationParams(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"cursor": "eyJpZCI6MTIzfQ==", "limit": 20}}
    )
...
class FieldSelection(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"fields": ["id", "name", "email"]}}
    )
...
class EagerLoadParams(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"include": ["doctor", "quizzes"]}}
    )
