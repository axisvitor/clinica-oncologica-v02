...
class RoleListResponse(BaseModel):
    ...
    model_config = ConfigDict(json_schema_extra={"example": {"data": [], "total": 2}})
...
class RoleRevocationRequest(BaseModel):
    ...
    model_config = ConfigDict(
        json_schema_extra={"example": {"reason": "User changed departments"}}
    )
...
class BulkRoleAssignmentResult(BaseModel):
    ...
    successful_users: List[UserRoleInfo] = Field(
        ..., description="Successfully updated users"
    )
    failed_users: List[Dict[str, Any]] = Field(
        ..., description="Failed users with reasons"
    )
...
class RoleStatistics(BaseModel):
    ...
    active_users_by_role: Dict[str, int] = Field(
        ..., description="Active users per role"
    )
    inactive_users_by_role: Dict[str, int] = Field(
        ..., description="Inactive users per role"
    )
    role_changes_last_30_days: int = Field(
        default=0, description="Role changes in last 30 days"
    )
    most_assigned_role: Optional[str] = Field(
        None, description="Most commonly assigned role"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                "total_users": 100,
                "role_distribution": {"admin": 5, "doctor": 95},
                "active_users_by_role": {"admin": 5, "doctor": 90},
                "inactive_users_by_role": {"admin": 0, "doctor": 5},
...
class RolePermissions(BaseModel):
    ...
    permission_groups: Dict[str, List[str]] = Field(
