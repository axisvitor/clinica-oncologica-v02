...
class UploadOptionsRequest(BaseModel):
    ...
    generate_thumbnail: bool = Field(
        False, description="Generate thumbnail for images (128x128)"
    )

    generate_preview: bool = Field(
        False, description="Generate preview for images (800x600)"
    )

    resize_width: Optional[int] = Field(
        None, ge=100, le=4000, description="Resize image width (maintains aspect ratio)"
    )
    ...
    quality: int = Field(
        85, ge=1, le=100, description="Image compression quality (1-100)"
    )

    scan_virus: bool = Field(True, description="Enable virus scanning")

    storage_provider: StorageProvider = Field(
        StorageProvider.LOCAL, description="Storage provider to use"
    )

    public: bool = Field(False, description="Make file publicly accessible")

    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Additional metadata to store with file"
    )
...
class DirectUploadRequest(BaseModel):
    ...
    storage_provider: StorageProvider = Field(
        StorageProvider.S3, description="Target storage provider"
    )
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        None, description="Metadata to associate with upload"
    )
...
class UploadResponse(BaseModel):
    ...
    image_metadata: Optional[ImageMetadata] = Field(
        None, description="Image metadata (for images only)"
    )
...
class DirectUploadResponse(BaseModel):
    ...
    upload_fields: Optional[Dict[str, str]] = Field(
        None, description="Additional form fields for POST upload"
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "uploads_by_category": {"image": 35, "document": 7},
                "uploads_by_month": {"2025-01": 42},
            }
        }
    )
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "details": {"file_size": 15728640, "max_size": 10485760},
            }
        }
    )
