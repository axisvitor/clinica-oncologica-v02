...
class AutomatedRecoveryService:
    ...
    async def run_automated_recovery_cycle(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Automated recovery cycle completed: {len(recovery_results)} actions executed"
            )
            return report
        ...
        for action in recovery_plan:
            try:
                ...
                if result.result == RecoveryResult.SUCCESS:
                    logger.info(
                        f"Recovery action {action.value} completed successfully: {result.description}"
                    )
                elif result.result == RecoveryResult.PARTIAL_SUCCESS:
                    logger.warning(
                        f"Recovery action {action.value} partially successful: {result.description}"
                    )
                else:
                    logger.error(
                        f"Recovery action {action.value} failed: {result.error_message}"
                    )
    ...
    async def _restart_failed_flows(self) -> RecoveryOperation:
        ...
        try:
            ...
            for flow_id in failed_flows[: self.recovery_config["batch_size"]]:
                try:
                    success = await self.error_recovery_service.recover_failed_flow(
                        flow_id
                    )
                    if success:
                        recovered_count += 1
            ...
            result = (
                ...
                if recovered_count == len(failed_flows)
                else RecoveryResult.PARTIAL_SUCCESS
            )
        ...
        try:
            ...
            for corrupted_flow in corruption_report:
                try:
                    ...
                    logger.error(
                        f"Error correcting flow {corrupted_flow['flow_id']}: {e}"
                    )
