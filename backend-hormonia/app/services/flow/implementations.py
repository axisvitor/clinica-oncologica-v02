...
class AITemplateResolver:
    ...
    async def personalize_message(self, template: 'MessageTemplate', context: Dict) -> str:
        ...
        try:
            ...
            return template.base_content.replace('[nome]', context.get('patient_name', ''))
    ...
    async def track_flow_event(self, event: FlowEvent) -> None:
        ...
        try:
            if self.redis:
                ...
                await self.redis.lpush(event_key, {
            ...
            logger.debug(f"Tracked flow event: {event.event_type} for {event.patient_id}")
