...
class FlowProcessor(Protocol):
    ...
    async def process_patient_flows(
...
class MessageScheduler(Protocol):
    ...
    async def schedule_flow_message(
        self, message_request: "FlowMessageRequest"
    ) -> bool: ...
...
class FlowOrchestrator:
    ...
    async def _schedule_patient_message(
        self, patient_id: UUID, flow_data: Dict
    ) -> bool:
        ...
        message_request = FlowMessageRequest(
            ...
            template=template,
            send_time=await self.message_scheduler.calculate_optimal_send_time(
                patient_id
            ),
        )
