...
class FlowDashboardTrendsMixin:
    @staticmethod
    def _engagement_column():
        return getattr(FlowAnalytics, "engagement_score", FlowAnalytics.success_rate)
    ...
    async def _get_engagement_distribution(
        ...
        engagement_column = self._engagement_column()
        timestamp_column = self._timestamp_column()

        scores_stmt = select(engagement_column).where(
            and_(
                engagement_column.is_not(None),
                timestamp_column.between(date_range[0], date_range[1]),
            )
        )

        if flow_type:
            flow_type_column = getattr(
                FlowAnalytics,
                "flow_type",
                FlowAnalytics.flow_template_version_id,
            )
            scores_stmt = scores_stmt.where(flow_type_column == flow_type)

        scores = list((await self.db.execute(scores_stmt)).scalars().all())
