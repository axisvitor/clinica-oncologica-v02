...
class FollowUpRedisStore:
        ...
        try:
            ...
            if redis:
                ...
                if action.status == "pending":
                    ...
                    await redis.zadd("followup:actions:pending", {str(action.action_id): score})
                
                logger.debug(
                    f"Stored action in Redis",
    ...
    async def get_pending_actions(
        ...
        try:
            ...
            if redis:
                ...
                pending = [
                    action for action in self._fallback_storage["actions"].values()
                    if action.get("status") == "pending"
                    and datetime.fromisoformat(action["scheduled_for"]).timestamp() <= now_ts
                ]
    ...
    async def store_alert(self, alert: Any) -> bool:
        ...
        try:
            alert_data = {
                ...
                "escalation_level": alert.escalation_level.value if hasattr(alert.escalation_level, 'value') else str(alert.escalation_level),
                "concern_type": alert.concern_type.value if hasattr(alert.concern_type, 'value') else str(alert.concern_type),
                "description": alert.description,
                "original_message": alert.original_message,
                "recommended_actions": alert.recommended_actions,
                "notification_channels": [ch.value if hasattr(ch, 'value') else str(ch) for ch in alert.notification_channels],
            ...
            if redis:
                ...
                if not alert.resolved_at:
                    level = alert.escalation_level.value if hasattr(alert.escalation_level, 'value') else str(alert.escalation_level)
                    score = ESCALATION_SCORES.get(level.lower(), 2)
                    await redis.zadd("followup:alerts:active", {str(alert.alert_id): score})
                
                logger.debug(
                    f"Stored alert in Redis",
                    extra={"alert_id": str(alert.alert_id), "patient_id": str(alert.patient_id)}
                )
                return True
            else:
    ...
    async def get_active_alerts(
        ...
        try:
            ...
            if redis:
                ...
                if patient_id:
                    alerts = [a for a in alerts if a.get("patient_id") == str(patient_id)]
    ...
    async def store_context(self, context: Any) -> bool:
        ...
        try:
            ...
            if redis:
                ...
                logger.debug(
                    f"Stored context in Redis with 7-day TTL",
                ...
                self._fallback_storage["contexts"][str(context.patient_id)] = {
                    "data": context_data,
                    "expires_at": datetime.utcnow() + timedelta(seconds=CONTEXT_TTL_SECONDS)
                }
                return True
        ...
        try:
            ...
            if redis:
                ...
                return {
                    ...
                    "stats": {
                        "pending_actions": len([
                            a for a in self._fallback_storage["actions"].values()
                            if a.get("status") == "pending"
                        ]),
                        "active_alerts": len([
                            a for a in self._fallback_storage["alerts"].values()
    ...
    async def cleanup_expired(self) -> Dict[str, int]:
        ...
        try:
            ...
            expired_contexts = [
                patient_id for patient_id, stored in self._fallback_storage["contexts"].items()
            ...
            if cleaned["contexts"] > 0:
                logger.info(
                    f"Cleaned up expired fallback data",
