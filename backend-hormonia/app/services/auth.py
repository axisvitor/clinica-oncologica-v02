...
class AuthService:
    ...
    def __init__(self, db: Any, user_repository: UserRepository, redis_client=None):
        ...
        self._failed_attempts: Dict[str, Dict[str, Any]] = defaultdict(lambda: {
            'count': 0,
            'last_attempt': None,
    ...
    def create_user(self, email: str, password: str, full_name: str, role: str = "doctor") -> User:
        """
        Create a new user.
    ...
    async def _is_rate_limited_redis(self, email: str, client_ip: Optional[str] = None) -> bool:
        """
    ...
    def _is_rate_limited_memory(self, email: str, client_ip: Optional[str] = None) -> bool:
        """
    ...
    async def _record_failed_attempt(self, email: str, client_ip: Optional[str] = None) -> None:
        """
    ...
    async def _record_failed_attempt_redis(self, email: str, client_ip: Optional[str] = None) -> None:
        """
        ...
        try:
            ...
            logger.warning(f"Failed attempt recorded for {email} (count: {current_count})")

        except Exception as e:
            logger.error(f"Redis failed attempt recording error: {e}")

    def _record_failed_attempt_memory(self, email: str, client_ip: Optional[str] = None) -> None:
        """
        ...
        logger.warning(f"Failed attempt recorded for {email} (count: {attempts['count']})")
