...
class QuestionHumanizer:
    ...
    async def humanize_question(
        ...
        if question_type not in self.SAFE_QUESTION_TYPES:
            logger.info(f"Unknown question type '{question_type}' - keeping original for safety")
        ...
        try:
            ...
            humanized = response.humanized_message if hasattr(response, 'humanized_message') else question

            # 7. Validate humanized output
            if self._is_too_similar(humanized, recent_questions):
                logger.info("Humanized question too similar to recent ones - using fallback variation")
            ...
            await self._log_telemetry(patient.id, question, humanized, "success", {"intent_pattern": intent})

            return humanized

        except Exception as e:
            logger.error(f"Humanization failed: {e}")
            await self._log_telemetry(patient.id, question, question, f"error: {str(e)}")
    ...
    async def _get_recent_questions(self, patient_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if data:
                ...
                cutoff = (datetime.utcnow() - timedelta(hours=self.history_window_hours)).isoformat()
    ...
    async def _store_question_history(self, patient_id: str, question: str, question_type: str, intent_pattern: Optional[str] = None):
    ...
    def _select_intent_pattern(self, question_type: str, recent_questions: List[Dict[str, Any]]) -> str:
    ...
    async def _log_telemetry(self, patient_id: str, original: str, result: str, status: str, metadata: Optional[Dict] = None):
