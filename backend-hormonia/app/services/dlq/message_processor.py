...
class DLQMessageProcessor:
    ...
    def reprocess_message(self, failed_message: FailedMessage) -> bool:
        ...
        try:
            ...
            if (
                ...
                failure_reason == FailureReason.NOTIFICATION_ERROR
    ...
    def _infer_and_process(
        ...
        if "phone_number" in payload or "whatsapp" in payload:
            ...
            logger.warning(f"Unknown message type for {failed_message.message_id}")
            return False
    ...
    def _reprocess_whatsapp(
        ...
        try:
            ...
            if not phone_number:
                logger.error(
                    f"Missing phone_number in payload: {failed_message.message_id}"
                )
                return False
        ...
        try:
            ...
            if not recipients:
                ...
                if email:
                    ...
                    logger.error(
                        f"Missing recipients in payload: {failed_message.message_id}"
                    )
                    return False
    ...
    def _reprocess_quiz(
        ...
        try:
            ...
            if action == "send_reminder":
                ...
                if phone_number:
                    return self._reprocess_whatsapp(
                        failed_message,
                        {
                            "phone_number": phone_number,
        ...
        try:
            ...
            if result:
                for channel, res in result.items():
                    if res.success:
                        logger.info(
                            f"Notification {failed_message.message_id} reprocessed"
                        )
                        return True

            logger.warning(
                f"Notification reprocess failed: {failed_message.message_id}"
            )
            return False
