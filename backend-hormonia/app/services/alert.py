...
class AlertService:
            ...
            try:
                ...
                if alert:
                    ...
                    if not self._has_recent_alert(patient_id, rule.rule_type, hours=rule.time_window_hours):
                        ...
                        logger.info(f"Generated {rule.rule_type} alert for patient {patient_id}")
                    else:
                        logger.debug(f"Skipping duplicate {rule.rule_type} alert for patient {patient_id}")
                        
            except Exception as e:
                logger.error(f"Error evaluating rule {rule_name} for patient {patient_id}: {e}")
    ...
    def _check_no_response(self, patient: Patient, rule: AlertRule) -> Optional[Alert]:
        ...
        outbound_since_response = (
            ...
            .filter(
                ...
                Message.created_at > (last_response.created_at if last_response else datetime.min)
            )
            .count()
        )
        
        if outbound_since_response > 0 and (not last_response or last_response.created_at < cutoff_time):
            hours_since = (datetime.utcnow() - (last_response.created_at if last_response else patient.created_at)).total_seconds() / 3600
            ...
            return Alert(
                ...
                data={"hours_since_response": hours_since, "last_response_at": last_response.created_at.isoformat() if last_response else None}
            )
    ...
    def _check_negative_sentiment(self, patient: Patient, rule: AlertRule) -> Optional[Alert]:
        ...
        if negative_scores:
            ...
            if avg_negative_score >= rule.threshold:
                return Alert(
                    ...
                    alert_type=rule.rule_type,
                    severity=rule.severity,
                    description=rule.description_template.format(score=round(avg_negative_score, 2)),
                    data={"sentiment_score": avg_negative_score, "message_count": len(negative_scores)}
                )
        
        return None
    
    def _check_treatment_adherence(self, patient: Patient, rule: AlertRule) -> Optional[Alert]:
        ...
        for response in adherence_responses:
            if response.response_metadata and "adherence_score" in response.response_metadata:
        ...
        if adherence_scores:
            ...
            if avg_adherence < rule.threshold:
                return Alert(
                    ...
                    alert_type=rule.rule_type,
                    severity=rule.severity,
                    description=rule.description_template.format(percentage=round(avg_adherence * 100, 1)),
                    data={"adherence_percentage": avg_adherence, "response_count": len(adherence_scores)}
                )
        
        return None
    
    def _check_emergency_keywords(self, patient: Patient, rule: AlertRule) -> Optional[Alert]:
        """Check for emergency keywords in recent messages."""
        cutoff_time = datetime.utcnow() - timedelta(hours=rule.time_window_hours)
        
        emergency_keywords = [
            "emergency", "urgent", "help", "pain", "bleeding", "dizzy", 
            "chest pain", "can't breathe", "suicide", "hurt myself"
        ]
        ...
        if found_keywords:
            return Alert(
                ...
                alert_type=rule.rule_type,
                severity=rule.severity,
                description=rule.description_template.format(keywords=", ".join(set(found_keywords))),
                data={"keywords": list(set(found_keywords)), "message_count": len(recent_messages)}
            )
        
        return None
    
    def _has_recent_alert(self, patient_id: UUID, alert_type: str, hours: int = 24) -> bool:
    ...
    async def create_alert(self, alert: Alert) -> Alert:
        ...
        try:
            ...
            logger.info(f"Created alert {created_alert.id} for patient {alert.patient_id}")
