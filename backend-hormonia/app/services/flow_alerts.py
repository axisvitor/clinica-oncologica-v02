...
class FlowAlertsService:
    ...
    async def _inactive_template_alerts(self) -> List[Alert]:
        templates_stmt = select(FlowTemplateVersion).where(
            FlowTemplateVersion.is_active.is_(True)
        )
        active_templates = (await self.db.execute(templates_stmt)).scalars().all()

        template_ids = [template.id for template in active_templates]
        active_counts: Dict[Any, int] = {}
        if template_ids:
            counts_stmt = (
                select(
                    PatientFlowState.flow_template_version_id,
                    func.count(PatientFlowState.id).label("active_patients"),
                )
                .where(
                    PatientFlowState.flow_template_version_id.in_(template_ids),
                    PatientFlowState.completed_at.is_(None),
                )
                .group_by(PatientFlowState.flow_template_version_id)
            )
            count_rows = (await self.db.execute(counts_stmt)).all()
            active_counts = {
                template_id: active_patients
                for template_id, active_patients in count_rows
            }

        alerts: List[Alert] = []
        for template in active_templates:
            active_patients = active_counts.get(template.id, 0)
