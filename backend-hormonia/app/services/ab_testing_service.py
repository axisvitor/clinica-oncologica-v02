...
class ABTestingService:
    ...
    def _calculate_confidence_interval(
        self, conversion_rate: float, sample_size: int, confidence_level: float = 0.95
    ) -> ConfidenceInterval:
        if sample_size == 0:
        ...
        adjusted_standard_error = (
            np.sqrt((p * (1 - p) / n + z_score**2 / (4 * n**2))) / denominator
        )
    ...
    async def list_experiments(
        ...
        cache_key = self._get_cache_key(
            "list_experiments", cursor=cursor, limit=limit, status=status, search=search
        )
        ...
        query = self.db.query(ABExperiment).options(
        ...
        if status:
            query = query.filter(
                ABExperiment.status == ModelExperimentStatus[status.value.upper()]
            )
        if search:
        ...
        next_cursor = (
            create_cursor(experiments[-1].id) if has_more and experiments else None
        )
    ...
    async def get_experiment(self, experiment_id: UUID) -> Dict[str, Any]:
        cache_key = self._get_cache_key(
            "get_experiment", experiment_id=str(experiment_id)
        )
        cached = await self._get_cached_result(cache_key)
        if cached:
            return cached
    ...
    async def create_experiment(
        ...
        experiment = ABExperiment(
            ...
            traffic_split=data.variants[1].traffic_weight
    ...
    async def update_experiment(
        self, experiment_id: UUID, data: ExperimentUpdate
    ) -> Dict[str, Any]:
        experiment = (
            self.db.query(ABExperiment).filter(ABExperiment.id == experiment_id).first()
        )
    ...
    async def assign_variant(
        ...
        anonymous_id: str,
        force_variant: Optional[VariantType],
    ) -> VariantAssignmentResponse:
        experiment = (
            self.db.query(ABExperiment).filter(ABExperiment.id == experiment_id).first()
        )
        ...
        existing = (
            ...
            .filter(
                and_(
                    ...
                    ABVariantAssignment.anonymous_patient_id == hashed_id,
                )
            )
            .first()
        )
        ...
        if existing:
            variant_data = next(
                (v for v in variants if v["type"] == existing.variant.value), {}
            )
    ...
    async def track_conversion(
        self, data: ConversionEventCreate
    ) -> ConversionEventResponse:
        ...
        self.db.commit()
        await self._invalidate_cache_pattern(
            f"ab_testing:v2:*results*{data.experiment_id}*"
        )
    ...
    async def get_experiment_results(
        self, experiment_id: UUID, confidence_level: ConfidenceLevel
    ) -> ExperimentResults:
        cache_key = self._get_cache_key(
            "get_results",
        ...
        cached = await self._get_cached_result(cache_key)
        if cached:
            return ExperimentResults(**cached)
        ...
        metrics = (
            ...
            .filter(
                and_(
                    ...
                    ABExperimentMetric.included_in_analysis,
                )
            )
            .all()
        )
        ...
        for v_type, stats in variant_stats.items():
            conv_rate = (
                stats["conversions"] / stats["total"] if stats["total"] > 0 else 0
            )
            ci = self._calculate_confidence_interval(
                conv_rate, stats["total"], float(confidence_level.value) / 100
            )
        ...
        if len(variant_stats) >= 2:
            ...
            if ctrl["total"] > 0 and trt["total"] > 0:
                test_result = self._perform_chi_square_test(
                    ctrl["conversions"], ctrl["total"], trt["conversions"], trt["total"]
                )
                winner = (
                    "treatment"
                    if trt["conversions"] / trt["total"]
                    > ctrl["conversions"] / ctrl["total"]
                    else "control"
                )
    ...
    async def get_dashboard(self) -> ExperimentDashboard:
        ...
        cached = await self._get_cached_result(cache_key)
        if cached:
            return ExperimentDashboard(**cached)

        total = self.db.query(ABExperiment).count()
        active = (
            self.db.query(ABExperiment)
            .filter(ABExperiment.status == ModelExperimentStatus.ACTIVE)
            .count()
        )
        completed = (
            self.db.query(ABExperiment)
            .filter(ABExperiment.status == ModelExperimentStatus.COMPLETED)
            .count()
        )
        draft = (
            self.db.query(ABExperiment)
            .filter(ABExperiment.status == ModelExperimentStatus.DRAFT)
            .count()
        )
        ...
        total_participants = (
            self.db.query(func.sum(ABExperiment.total_participants)).scalar() or 0
        )
        total_conversions = (
            self.db.query(ABExperimentMetric)
            .filter(ABExperimentMetric.included_in_analysis)
            .count()
        )
        ...
        result = ExperimentDashboard(
            ...
            avg_conversion_rate=total_conversions / total_participants
