...
class ConnectionWebhookHandler:
    ...
    def __init__(
        self, connection_state_repo: Optional[ConnectionStateRepository] = None
    ):
        """
        ...
        self.connection_state_repo = (
            connection_state_repo or ConnectionStateRepository()
        )
    ...
    async def process_qrcode(
        ...
        try:
            ...
            instance = event_data.get("instance")
            qr_code = event_data.get("qrcode") or event_data.get("data", {}).get(
                "qrcode"
            )
            ...
            await redis_client.setex(
                qr_key, cache_settings.CACHE_QRCODE_TTL_SECONDS, str(qr_data)
            )
