...
class MessageWebhookHandler:
        ...
        try:
            ...
            if not message_data:
                ...
                if webhook_id and webhook_store:
                    await webhook_store.mark_processed(webhook_id, False, "No valid message data")
                return None
    ...
    async def _find_patient_with_security(
        ...
        if should_block:
            logger.warning(
                f"Phone blocked due to repeated unauthorized attempts",
    ...
    async def _send_unauthorized_response(self, phone: str, attempt_count: int = 1) -> None:
        ...
        try:
            ...
            if not client:
                logger.warning(f"Evolution client unavailable")
                return
