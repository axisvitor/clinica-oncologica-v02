    ...
    def find_patient_by_phone(self, phone: str) -> Optional[Patient]:
        ...
        try:
            ...
            if not phone.startswith("+55") and not phone.startswith("55"):
                ...
                logger.info(f"Phone lookup attempt 3: Adding +55 prefix '{with_prefix}'")
                patient = self.patient_repo.get_by_phone(with_prefix)
                if patient:
                    logger.info(f"Patient found with +55 prefix: {with_prefix}")
                    return patient
            
            # Strategy 4: Try last 11 digits (Brazilian mobile with DDD)
            if len(without_plus) > 11:
                local_11 = without_plus[-11:]
                logger.info(f"Phone lookup attempt 4: Local 11 digits '{local_11}'")
            ...
            if len(without_plus) > 10:
                ...
                logger.info(f"Phone lookup attempt 5: Local 10 digits '{local_10}'")
            ...
            logger.warning(
                ...
                f"Original: {phone}, Normalized: {normalized}"
            )
            return None
