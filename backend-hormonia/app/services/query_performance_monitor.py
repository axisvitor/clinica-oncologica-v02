...
class QueryPerformanceMonitor:
    ...
    def track_query_time(self, query: str, duration_ms: float, parameters: Optional[Dict] = None):
        """
        ...
        try:
            ...
            if query_hash in self._query_cache:
                ...
                slow_query.avg_duration_ms = (
                    (slow_query.avg_duration_ms * (slow_query.execution_count - 1) + duration_ms) 
    ...
    def get_performance_metrics(self) -> QueryMetrics:
        ...
        try:
            ...
            if elapsed_seconds > 0:
                self._metrics.queries_per_second = self._metrics.total_queries / elapsed_seconds
    ...
    def get_query_analysis(self, hours_back: int = 1) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "patterns": patterns,
                "recommendations": self._generate_performance_recommendations(recent_queries),
    ...
    def _update_metrics(self, duration_ms: float):
        ...
        if self._metrics.total_queries == 1:
            ...
            self._metrics.avg_duration_ms = (
                (self._metrics.avg_duration_ms * (self._metrics.total_queries - 1) + duration_ms)
    ...
    def _generate_optimization_suggestions(self, query: str) -> List[str]:
        ...
        if "join" not in query_lower and ("patient" in query_lower or "message" in query_lower):
            suggestions.append("Consider using JOINs or eager loading to prevent N+1 queries")
    ...
    def _analyze_query_patterns(self, queries: List[SlowQuery]) -> Dict[str, Any]:
        ...
        if queries:
            patterns["avg_execution_count"] = sum(q.execution_count for q in queries) / len(queries)
        
        return patterns
    
    def _generate_performance_recommendations(self, queries: List[SlowQuery]) -> List[str]:
        ...
        n_plus_one_queries = [
            q for q in queries 
            if "N+1 queries" in " ".join(q.optimization_suggestions)
        ]
