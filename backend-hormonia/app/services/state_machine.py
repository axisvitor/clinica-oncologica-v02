...
class StateMachine:
    ...
    def can_transition(
        ...
        for condition in current_step.conditions:
            result, message = self.condition_evaluator.evaluate_condition(condition, context)
    ...
    def transition(
        ...
        if not force:
            can_transition, conditions_evaluated = self.can_transition(from_step_id, context)
        ...
        for step_id in step_ids:
            if step_id not in visited:
                if has_cycle(step_id, visited, set()):
                    errors.append(f"Circular reference detected starting from step {step_id}")
