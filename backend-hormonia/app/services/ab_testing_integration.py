...
class ABTestingIntegration:
        ...
        try:
            ...
            if not assignment or not assignment.is_eligible:
                ...
                self.audit_service.log_patient_interaction(
                    ...
                    assignment_reason=getattr(
                        assignment, "assignment_reason", "eligibility_criteria_not_met"
                    ),
                )
                return self.message_factory.create_outbound_message(
                    patient_id=patient_id,
            ...
            if variant_type == VariantType.TREATMENT:
                ...
                try:
                    ...
                    if self._contains_medical_keywords(content):
                        logger.warning(
                            f"Medical content detected in A/B test message for patient {patient_id}, using control variant"
                        )
                        ...
                        if hasattr(humanized_result, "humanized_message"):
                            ...
                            metadata["ai_processing"] = {
                                "confidence_score": getattr(
                                    humanized_result, "confidence_score", None
                                ),
                                "personalization_notes": getattr(
                                    humanized_result, "personalization_notes", []
                                ),
                            }
                        else:
                            final_content = (
                                humanized_result if humanized_result else content
                            )
                    ...
                    logger.error(
                        f"AI humanization failed for experiment {active_experiment['id']}: {str(e)}"
                    )
                    final_content = content
            ...
            logger.info(
                f"Created A/B test message for patient {patient_id}, variant: {variant_type.value}"
            )
            return message
    ...
    async def track_message_event(
        ...
        try:
            ...
            logger.debug(
                f"Tracked A/B test event: {event_type} for message {message.id}"
            )
    ...
    async def setup_experiment_for_template(
        ...
        try:
            ...
            result = await self.ab_service.create_experiment(
                experiment_data, user_id=None
            )
            ...
            logger.info(
                f"Created A/B experiment {experiment_id} for template {template.value}"
            )
    ...
    async def get_experiment_performance_summary(
        ...
        try:
            ...
            try:
                results = await self.ab_service.get_experiment_results(
                    UUID(experiment_id), ConfidenceLevel.NINETY_FIVE
                )
...
Contamos com você!"""
    ...
    def generate_experiment_report(
        self, experiment_id: str, report_type: str = "summary"
    ) -> Dict[str, Any]:
    ...
    def _get_active_experiment_for_template(
...
async def create_ab_test_monthly_quiz_invitation(
    db: Any, patient_id: UUID, patient_name: str, link_url: str, quiz_session_id: str
) -> Message:
    """
