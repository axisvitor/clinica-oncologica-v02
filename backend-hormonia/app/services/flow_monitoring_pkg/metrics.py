...
from sqlalchemy import Boolean, DateTime, and_, cast, func, or_
...
class FlowMonitoringMetricsMixin:
    async def collect_performance_metrics(self) -> PerformanceMetrics:
        ...
        try:
            ...
            messages_last_hour = (
                self.db.query(Message).filter(Message.sent_at >= one_hour_ago).count()
            )
    ...
    async def _calculate_corruption_rate(self) -> float:
        ...
        try:
            total_flows = self.db.query(PatientFlowState).count()
            if total_flows == 0:
                return 0.0
