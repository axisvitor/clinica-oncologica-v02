...
class ReportService:
    ...
    async def create_report(self, report_request, user_id: UUID):
        ...
        try:
            ...
            sanitized_title = self.data_protection.sanitize_for_logging(report_request.title)
    ...
    async def generate_patient_report(self, patient_id: UUID, user_id: UUID, include_messages: bool = True,
        ...
        try:
            ...
            patient_query = (
                ...
                .options(
                    ...
                    selectinload(Patient.messages) if include_messages else selectinload(Patient.messages).selectinload(Message.id),
            ...
            if include_messages:
                ...
                report_data["latest_message"] = {
                    "content": self.data_protection.sanitize_for_logging(patient.messages[0].content) if patient.messages else None,
                    "timestamp": patient.messages[0].created_at.isoformat() if patient.messages else None,
                    "direction": patient.messages[0].direction.value if patient.messages else None
                }
    ...
    async def generate_bulk_patient_reports(self, patient_ids: List[UUID], user_id: UUID,
                                          include_messages: bool = True) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.data_protection.log_sensitive_access(
                ...
                entity_id=uuid4(),
                access_reason=AccessReason.MEDICAL_TREATMENT,
                additional_context={'action': 'bulk_patient_reports', 'patient_count': len(patient_ids)}
            )
            ...
            patients_query = (
                ...
                .options(
                    joinedload(Patient.doctor),
                    selectinload(Patient.messages).selectinload(Message.patient) if include_messages else selectinload(Patient.messages).selectinload(Message.id),
                    selectinload(Patient.flow_states)
                )
    ...
    async def get_doctor_dashboard_data(self, doctor_id: UUID, user_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            patient_stats = {row.flow_state.value: row.count for row in patient_stats_result}

            # Message statistics (aggregated query)
            message_stats_query = (
                select(
                    func.count(Message.id).label('total_messages'),
                    func.count(Message.id).filter(Message.direction == 'outbound').label('outbound_count'),
                    func.count(Message.id).filter(Message.direction == 'inbound').label('inbound_count'),
                    func.count(Message.id).filter(Message.status == 'failed').label('failed_count')
                )
                .select_from(
                    Message.join(Patient, Message.patient_id == Patient.id)
                )
                .where(Patient.doctor_id == doctor_id)
            )
    ...
    async def get_system_analytics(self, user_id: UUID, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            analytics_query = (
                select(
                    ...
                    func.count(Patient.id).filter(Patient.created_at >= start_date).label('new_patients'),
                    func.count(Patient.id).filter(Patient.flow_state == 'active').label('active_patients'),
                    func.count(Message.id).label('total_messages'),
                    func.count(Message.id).filter(Message.created_at >= start_date).label('recent_messages'),
                    func.count(Message.id).filter(Message.status == 'failed').label('failed_messages'),
                    func.avg(Patient.current_day).label('avg_treatment_day')
                )
                .select_from(
                    Patient.outerjoin(Message, Patient.id == Message.patient_id)
                )
            )
            ...
            flow_distribution = {row.flow_state.value: row.count for row in flow_distribution_result}
