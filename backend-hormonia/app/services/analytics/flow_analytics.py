...
class FlowAnalyticsService:
    ...
    async def track_message_sent(self, 
                                patient_id: UUID, 
                                message_id: UUID,
                                flow_type: str,
                                flow_day: int,
                                template_id: str,
                                additional_data: Optional[dict[str, Any]] = None) -> FlowAnalytics:
        """
        ...
        try:
            ...
            logger.info(f"Tracked message sent for patient {patient_id} on day {flow_day}")
            return analytics
    ...
    async def track_response_received(self,
                                    patient_id: UUID,
        ...
        try:
            ...
            logger.info(f"Tracked response received for patient {patient_id} on day {flow_day}")
            return analytics
    ...
    async def track_flow_event(self,
                              patient_id: UUID,
                              flow_type: str,
                              flow_day: int,
                              event_type: EventType,
                              event_data: Optional[dict[str, Any]] = None) -> FlowAnalytics:
        """
    ...
    async def calculate_engagement_metrics(self,
                                         ...
                                         date_range: Optional[Tuple[datetime, datetime]] = None) -> EngagementMetrics:
        """
        ...
        try:
            ...
            metrics.response_rate = (responses_received / messages_sent * 100) if messages_sent > 0 else 0.0
            ...
            ).with_entities(FlowAnalytics.response_time_seconds).all()
            ...
            sentiment_data = query.filter(
                and_(
                    FlowAnalytics.event_type == EventType.RESPONSE_RECEIVED,
                    FlowAnalytics.sentiment_score.isnot(None)
                )
            ).with_entities(FlowAnalytics.sentiment_score).all()
            ...
            engagement_scores = query.filter(
                FlowAnalytics.engagement_score.isnot(None)
            ).with_entities(FlowAnalytics.engagement_score).all()
            
            if engagement_scores:
                metrics.engagement_score = sum(es[0] for es in engagement_scores) / len(engagement_scores)
            
            logger.info(f"Calculated engagement metrics: {metrics.response_rate:.1f}% response rate")
            return metrics
    ...
    async def identify_at_risk_patients(self,
                                      flow_type: Optional[str] = None,
                                      lookback_days: int = 7) -> List[PatientRisk]:
        """
        ...
        try:
            ...
            for flow_state in active_patients:
                ...
                last_response = self.db.query(FlowAnalytics).filter(
                    and_(
                        FlowAnalytics.patient_id == patient_id,
                        FlowAnalytics.event_type == EventType.RESPONSE_RECEIVED
                    )
                ).order_by(desc(FlowAnalytics.timestamp)).first()
                
                if not last_response:
                    risk_factors.append("No responses recorded")
                    risk_level = RiskLevel.HIGH
                elif last_response.timestamp < cutoff_date:
                    days_since_response = (datetime.utcnow() - last_response.timestamp).days
                ...
                recent_engagement = self.db.query(FlowAnalytics).filter(
                    and_(
                        FlowAnalytics.patient_id == patient_id,
                ...
                if recent_engagement:
                    avg_engagement = sum(e[0] for e in recent_engagement) / len(recent_engagement)
                    if avg_engagement < 0.3:
                        risk_factors.append(f"Low engagement score: {avg_engagement:.2f}")
                        if risk_level == RiskLevel.LOW:
                            risk_level = RiskLevel.MEDIUM
                
                # Check for negative sentiment patterns
                recent_sentiment = self.db.query(FlowAnalytics).filter(
                    and_(
                        FlowAnalytics.patient_id == patient_id,
                        FlowAnalytics.timestamp >= cutoff_date,
                        FlowAnalytics.sentiment_score.isnot(None)
                ...
                concerning_events = self.db.query(FlowAnalytics).filter(
                    and_(
                        FlowAnalytics.patient_id == patient_id,
                        FlowAnalytics.timestamp >= cutoff_date,
                ...
                if concerning_events > 0:
                    risk_factors.append(f"{concerning_events} concerning events detected")
    ...
    async def get_flow_performance_metrics(self,
                                         flow_type: str,
        ...
        try:
            ...
            unique_patients = query.with_entities(FlowAnalytics.patient_id).distinct().count()
            
            # Message metrics
            messages_sent = query.filter(FlowAnalytics.event_type == EventType.MESSAGE_SENT).count()
            responses_received = query.filter(FlowAnalytics.event_type == EventType.RESPONSE_RECEIVED).count()
            
            # Flow completion metrics
            flows_started = query.filter(FlowAnalytics.event_type == EventType.FLOW_STARTED).count()
            flows_completed = query.filter(FlowAnalytics.event_type == EventType.FLOW_COMPLETED).count()
            
            # Day-by-day breakdown
            day_metrics = {}
            max_day = query.with_entities(func.max(FlowAnalytics.flow_day)).scalar() or 0
            
            for day in range(1, max_day + 1):
                day_query = query.filter(FlowAnalytics.flow_day == day)
                day_messages = day_query.filter(FlowAnalytics.event_type == EventType.MESSAGE_SENT).count()
                day_responses = day_query.filter(FlowAnalytics.event_type == EventType.RESPONSE_RECEIVED).count()
                
                day_metrics[f"day_{day}"] = {
                    'messages_sent': day_messages,
                    'responses_received': day_responses,
                    'response_rate': (day_responses / day_messages * 100) if day_messages > 0 else 0.0
                }
            ...
            return {
                ...
                'overview': {
                    ...
                    'response_rate': (responses_received / messages_sent * 100) if messages_sent > 0 else 0.0,
                    'flows_started': flows_started,
                    'flows_completed': flows_completed,
                    'completion_rate': (flows_completed / flows_started * 100) if flows_started > 0 else 0.0
                },
    ...
    async def get_patient_analytics_summary(self,
                                          patient_id: UUID,
        ...
        try:
            ...
            analytics_data = self.db.query(FlowAnalytics).filter(
                and_(
                    FlowAnalytics.patient_id == patient_id,
                    FlowAnalytics.timestamp >= cutoff_date
                )
            ).order_by(desc(FlowAnalytics.timestamp)).all()
            ...
            # Calculate metrics
            messages_received = sum(1 for a in analytics_data if a.event_type == EventType.MESSAGE_SENT)
            responses_sent = sum(1 for a in analytics_data if a.event_type == EventType.RESPONSE_RECEIVED)
            
            # Sentiment analysis
            sentiment_scores = [a.sentiment_score for a in analytics_data if a.sentiment_score is not None]
            avg_sentiment = sum(sentiment_scores) / len(sentiment_scores) if sentiment_scores else None
            
            # Engagement analysis
            engagement_scores = [a.engagement_score for a in analytics_data if a.engagement_score is not None]
            avg_engagement = sum(engagement_scores) / len(engagement_scores) if engagement_scores else None
            
            # Response time analysis
            response_times = [a.response_time_seconds for a in analytics_data if a.response_time_seconds is not None]
            avg_response_time = sum(response_times) / len(response_times) if response_times else None
            
            # Recent activity
            last_message = next((a for a in analytics_data if a.event_type == EventType.MESSAGE_SENT), None)
            last_response = next((a for a in analytics_data if a.event_type == EventType.RESPONSE_RECEIVED), None)
            ...
            return {
                ...
                'engagement_summary': {
                    ...
                    'response_rate': (responses_sent / messages_received * 100) if messages_received > 0 else 0.0,
                ...
                'recent_activity': {
                    'last_message_sent': last_message.timestamp.isoformat() if last_message else None,
                    'last_response_received': last_response.timestamp.isoformat() if last_response else None,
                    'days_since_last_response': (datetime.utcnow() - last_response.timestamp).days if last_response else None
                },
                'flow_progress': {
                    'current_flow_type': analytics_data[0].flow_type if analytics_data else None,
                    'current_flow_day': analytics_data[0].flow_day if analytics_data else None
                }
            }
    ...
    async def cleanup_old_analytics(self, days_to_keep: int = 90) -> int:
        ...
        try:
            ...
            deleted_count = self.db.query(FlowAnalytics).filter(
                FlowAnalytics.timestamp < cutoff_date
            ).delete()
