...
class DataAggregator:
    ...
    def get_patient_data_summary(
        self, patient_id: UUID, period_start: date, period_end: date
    ) -> dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Aggregating data for patient {patient_id} from {period_start} to {period_end}"
            )
            ...
            message_data = self._aggregate_message_data(
                patient_id, period_start, period_end
            )
            quiz_data = self._aggregate_quiz_data(patient_id, period_start, period_end)
            alert_data = self._aggregate_alert_data(
                patient_id, period_start, period_end
            )
            treatment_data = self._aggregate_treatment_data(
                patient, period_start, period_end
            )
    ...
    def _aggregate_message_data(
        ...
        try:
            ...
            for outbound in outbound_messages:
                ...
                if next_inbound:
                    response_time = (
                        next_inbound.created_at - outbound.created_at
                    ).total_seconds() / 3600
        ...
        try:
            ...
            for template_id, template_responses in responses_by_template.items():
                ...
                if numeric_responses:
                    avg_scores[template_id] = sum(numeric_responses) / len(
                        numeric_responses
                    )
                ...
                recent_responses.append(
                    {
                        ...
                        "question": response.question_text[:100] + "..."
                        if len(response.question_text) > 100
                        else response.question_text,
                        "response": response.response_value[:100] + "..."
                        if len(response.response_value) > 100
            ...
            return {
                ...
                "responses_by_template": {
                    k: len(v) for k, v in responses_by_template.items()
                },
    ...
    def _aggregate_treatment_data(
        self, patient: Patient, period_start: date, period_end: date
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if patient.treatment_start_date:
                treatment_duration_days = (
                    period_end - patient.treatment_start_date
                ).days
    ...
    def get_patient_analytics(
        self, patient_id: UUID, days_back: int = 30
    ) -> PatientAnalytics:
        """
        ...
        try:
            ...
            # Get aggregated data
            data_summary = self.get_patient_data_summary(
    ...
    def get_system_analytics(self) -> SystemAnalytics:
        ...
        try:
            ...
            messages_today = (
                self.db.query(Message)
                .filter(
                    Message.created_at >= today_start, Message.created_at <= today_end
                )
                .count()
            )
            ...
            unresolved_alerts = (
                self.db.query(Alert)
                .filter(Alert.status.in_(["pending", "acknowledged"]))
                .count()
            )
