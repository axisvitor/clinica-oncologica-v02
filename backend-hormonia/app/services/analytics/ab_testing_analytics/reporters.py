...
class ReportGenerator:
    ...
    def generate_executive_report(results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary_metrics": {
                "control_response_rate": results.get("variant_statistics", {}).get("control", {}).get("response_rate"),
                "treatment_response_rate": results.get("variant_statistics", {}).get("treatment", {}).get("response_rate"),
                "improvement": results.get("effect_sizes", {}).get("absolute_difference"),
                "statistical_significance": results.get("statistical_tests", {}).get("overall", {}).get("is_significant")
            },

            "business_recommendation": ReportGenerator._generate_business_recommendation(results),
    ...
    def generate_clinical_report(results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "safety_analysis": {
                "safety_profile": results.get("healthcare_metrics", {}).get("safety_profile"),
            ...
            "conclusions": {
                "primary_conclusion": results.get("statistical_tests", {}).get("overall", {}).get("conclusion"),
                "clinical_significance": results.get("healthcare_metrics", {}).get("clinical_significance"),
                "recommendations": results.get("recommendations")
            },
    ...
    def generate_stopping_recommendation(
        ...
        try:
            ...
            # Bayesian decision
            prob_treatment_better = bayesian_analysis.get("probability_treatment_better", 0.5)
            if prob_treatment_better > 0.95:
                recommendations.append("Bayesian analysis shows >95% probability treatment is better")
                overall_decision = "stop"
                confidence_level = "high"
            elif prob_treatment_better < 0.05:
                recommendations.append("Bayesian analysis shows >95% probability control is better")
                overall_decision = "stop"
                confidence_level = "high"
            elif prob_treatment_better > 0.8 or prob_treatment_better < 0.2:
                recommendations.append("Bayesian analysis shows strong evidence for one variant")
                confidence_level = "medium"

            # Futility assessment
            futility_score = futility_analysis.get("futility_score", 0)
            if futility_score > 0.7:
                recommendations.append("High futility - unlikely to detect significant effect")
                overall_decision = "stop"
    ...
    def _generate_executive_summary(results: Dict[str, Any]) -> str:
        ...
        is_significant = results.get("statistical_tests", {}).get("overall", {}).get("is_significant", False)
        improvement = results.get("effect_sizes", {}).get("absolute_difference", 0)

        if is_significant and improvement > 0:
            return (f"The A/B test showed statistically significant improvement in patient engagement "
            ...
            return (f"The A/B test showed statistically significant decrease in patient engagement "
                   f"with AI-humanized messages. Control group performed better by {abs(improvement)*100:.1f}%.")
        else:
            return ("The A/B test did not show statistically significant differences between "
    ...
    def _extract_key_findings(results: Dict[str, Any]) -> List[str]:
        ...
        # Statistical significance
        if results.get("statistical_tests", {}).get("overall", {}).get("is_significant"):
    ...
    def _generate_business_recommendation(results: Dict[str, Any]) -> str:
        ...
        recommendation = results.get("business_impact", {}).get("recommendation", "do_not_implement")
        roi = results.get("business_impact", {}).get("roi_estimate", 0)
    ...
    def _extract_adverse_events(results: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        error_rate_control = results.get("variant_statistics", {}).get("control", {}).get("error_rate", 0)
        error_rate_treatment = results.get("variant_statistics", {}).get("treatment", {}).get("error_rate", 0)
