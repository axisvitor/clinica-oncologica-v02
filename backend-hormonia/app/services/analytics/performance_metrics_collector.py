...
class PerformanceMetricsCollector:
    ...
    async def record_query_time(
        ...
        labels = {"operation": operation or "unknown", "table": table or "unknown"}
    ...
    async def record_connection_count(
        self, active_connections: int, pool_size: int, connection_type: str = "database"
    ) -> None:
    ...
    async def record_error_rate(
        ...
        labels = {"error_type": error_type, "endpoint": endpoint or "unknown"}

        metadata = {"error_count": error_count, "total_requests": total_requests}
    ...
    async def record_throughput(
        self, endpoint: str, requests_per_second: float, time_window: int = 60
    ) -> None:
        """Record API throughput metrics."""
        labels = {"endpoint": endpoint, "time_window": str(time_window)}
    ...
    async def get_error_analysis(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for metric in error_metrics:
            ...
            error_count = (
                metric.metadata.get("error_count", 0) if metric.metadata else 0
            )
    ...
    async def get_performance_dashboard_data(self) -> Dict[str, Any]:
        ...
        connection_metrics = await self.get_metrics(
            metric_name="connection_pool_utilization", start_time=last_hour
        )

        avg_connection_utilization = 0.0
        if connection_metrics:
            avg_connection_utilization = statistics.mean(
                m.value for m in connection_metrics
            )
        ...
        return {
            ...
            "api_performance": {
                "avg_response_time": response_summary.avg_value
                if response_summary
                else 0.0,
                "p95_response_time": response_summary.p95_value
                if response_summary
                else 0.0,
    ...
    async def _flush_metrics(self) -> None:
        ...
        if self.redis_client:
            ...
            logger.debug(
                f"Flushed {len(metrics_to_flush)} metrics (Redis not available)"
            )
