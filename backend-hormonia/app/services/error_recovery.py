...
class ErrorRecoveryService:
    ...
    async def _handle_message_delivery_error(
        self, error: MessageDeliveryError, context: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if error.retry_count >= max_retries:
                logger.error(
                    f"Max retries exceeded for message delivery to patient {error.patient_id}"
                )
            ...
            logger.info(
                f"Scheduled message retry for patient {error.patient_id} "
    ...
    async def _handle_flow_state_corruption(
        self, error: FlowStateCorruptionError, context: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            recovered_state = await self._recover_flow_state(
                patient_id, error.corruption_type
            )
            ...
            if recovered_state:
                ...
                logger.warning(
                    f"Flow state corruption requires manual intervention for patient {patient_id}"
                )
                return False
    ...
    async def _handle_external_service_error(
        self, error: ExternalServiceError, context: dict[str, Any]
    ) -> bool:
        """Handle external service failures with graceful degradation."""
        try:
            if not error.is_recoverable:
                logger.error(
                    f"Non-recoverable external service error: {error.service_name}"
                )
            ...
            if failure_count >= 5:  # Circuit breaker threshold
                logger.warning(
                    f"Circuit breaker open for service: {error.service_name}"
                )
    ...
    async def _handle_ai_service_error(
        self, error: AIServiceError, context: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if "message_template" in context:
                fallback_message = await self._generate_fallback_message(
                    context["message_template"], context.get("patient_id")
                )
                context["fallback_message"] = fallback_message
                logger.info("Generated fallback message for AI service failure")
                return True

            # For other AI operations, log and continue with degraded functionality
            logger.warning(
                f"AI service unavailable, continuing with reduced functionality: {error.ai_service}"
            )
            return True
    ...
    async def _handle_redis_connection_error(
        self, error: RedisConnectionError, context: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            if error.operation in ["get", "set", "delete"]:
                logger.warning(
                    f"Redis unavailable, using in-memory fallback for {error.operation}"
                )
            ...
            logger.info(
                f"Redis unavailable, skipping non-critical operation: {error.operation}"
            )
            return True
        ...
        try:
            ...
            if retry_func:
                try:
                    await retry_func()
                    logger.info(
                        f"Successfully retried database operation: {error.operation}"
                    )
                    return True
    ...
    async def _handle_flow_processing_error(
        self, error: FlowProcessingError, context: dict[str, Any]
    ) -> bool:
        ...
        try:
            ...
            logger.warning(
                f"Paused flow for manual investigation: patient {patient_id}"
            )
            return False
    ...
    async def _schedule_message_retry(
        self, error: MessageDeliveryError, context: dict[str, Any], delay: int
    ) -> None:
    ...
    async def _create_manual_correction_task(
        self, error: FlowStateCorruptionError, context: dict[str, Any]
    ) -> None:
        ...
        logger.critical(
            f"Manual correction required for patient {error.patient_id}: {error.corruption_type}"
        )
    ...
    async def _activate_fallback_mode(
    ...
    async def _generate_fallback_message(
        self, template: str, patient_id: Optional[UUID]
    ) -> str:
        ...
        if patient_id:
            try:
                ...
                logger.warning(
                    f"Failed to fetch patient data for fallback message (patient_id={patient_id}): {e}",
                    exc_info=True,
                )

        return template

    async def _get_last_good_flow_state(
    ...
    async def _pause_flow_for_investigation(
    ...
    async def _escalate_message_delivery_failure(
        self, error: MessageDeliveryError, context: dict[str, Any]
    ) -> None:
    ...
    async def _escalate_service_failure(
        self, error: ExternalServiceError, context: dict[str, Any]
    ) -> None:
