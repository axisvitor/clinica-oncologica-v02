...
class FlowDashboardService:
    ...
    async def get_dashboard_overview(self, 
                                   ...
                                   custom_range: Optional[Tuple[datetime, datetime]] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                'overview_metrics': {
                    ...
                    'average_response_time': engagement_metrics.average_response_time.total_seconds() if engagement_metrics.average_response_time else None,
                ...
                'at_risk_summary': {
                    ...
                    'critical_risk': len([p for p in at_risk_patients if p.risk_level == RiskLevel.CRITICAL]),
                    'high_risk': len([p for p in at_risk_patients if p.risk_level == RiskLevel.HIGH]),
                    'medium_risk': len([p for p in at_risk_patients if p.risk_level == RiskLevel.MEDIUM])
                },
    ...
    async def get_patient_engagement_trends(self,
                                          timeframe: DashboardTimeframe = DashboardTimeframe.LAST_30_DAYS,
                                          flow_type: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            trend_direction = self._calculate_trend_direction(daily_metrics, 'response_rate')
            
            # Get engagement distribution
            engagement_distribution = await self._get_engagement_distribution(date_range, flow_type)
            
            # Get top performing days/times
            peak_engagement = await self._get_peak_engagement_times(date_range, flow_type)
            ...
            return {
                ...
                'insights': self._generate_engagement_insights(daily_metrics, trend_direction)
            }
    ...
    async def get_at_risk_patient_dashboard(self,
        ...
        try:
            ...
            interventions = await self._generate_intervention_recommendations(at_risk_patients)
            ...
            return {
                'summary': {
                    'total_at_risk': len(at_risk_patients),
                    'by_risk_level': {
                        level.value: len(patients) for level, patients in risk_groups.items()
                    },
    ...
    async def get_flow_optimization_recommendations(self,
                                                  flow_type: str,
        ...
        try:
            ...
            timing_analysis = await self._analyze_message_timing(flow_type, (start_date, end_date))
            
            # Analyze content effectiveness
            content_analysis = await self._analyze_content_effectiveness(flow_type, (start_date, end_date))
            
            # Analyze drop-off points
            dropoff_analysis = await self._analyze_flow_dropoffs(flow_type, (start_date, end_date))
    ...
    async def get_real_time_alerts(self, 
                                 severity_levels: Optional[List[str]] = None) -> Dict[str, Any]:
        """
    ...
    def _get_date_range(self, 
                       ...
                       custom_range: Optional[Tuple[datetime, datetime]] = None) -> Tuple[datetime, datetime]:
    ...
    async def _calculate_trends(self, date_range: Tuple[datetime, datetime]) -> Dict[str, Any]:
        ...
        return {
            'response_rate': {
                ...
                'change_percent': calculate_change(current_metrics.response_rate, previous_metrics.response_rate),
                'direction': self._get_trend_direction(current_metrics.response_rate, previous_metrics.response_rate)
            },
            'engagement_score': {
                'current': current_metrics.engagement_score,
                'previous': previous_metrics.engagement_score,
                'change_percent': calculate_change(current_metrics.engagement_score, previous_metrics.engagement_score),
                'direction': self._get_trend_direction(current_metrics.engagement_score, previous_metrics.engagement_score)
            },
            'messages_sent': {
                'current': current_metrics.total_messages_sent,
                'previous': previous_metrics.total_messages_sent,
                'change_percent': calculate_change(current_metrics.total_messages_sent, previous_metrics.total_messages_sent),
                'direction': self._get_trend_direction(current_metrics.total_messages_sent, previous_metrics.total_messages_sent)
            }
        }
    ...
    async def _get_recent_alerts(self, date_range: Tuple[datetime, datetime]) -> List[Dict[str, Any]]:
        ...
        ).order_by(desc(FlowAnalytics.timestamp)).limit(10).all()
    ...
    async def _get_daily_engagement_metrics(self, 
                                          date_range: Tuple[datetime, datetime], 
                                          flow_type: Optional[str]) -> List[Dict[str, Any]]:
        """Get daily engagement metrics."""
    ...
    def _calculate_trend_direction(self, daily_metrics: List[Dict[str, Any]], metric_key: str) -> TrendDirection:
        """Calculate trend direction from daily metrics."""
        if len(daily_metrics) < 2:
            return TrendDirection.UNKNOWN
        
        values = [day[metric_key] for day in daily_metrics if day[metric_key] is not None]
        if len(values) < 2:
            return TrendDirection.UNKNOWN
    ...
    async def _get_engagement_distribution(self, 
                                         date_range: Tuple[datetime, datetime], 
                                         flow_type: Optional[str]) -> Dict[str, Any]:
    ...
    async def _get_peak_engagement_times(self, 
                                       date_range: Tuple[datetime, datetime], 
                                       flow_type: Optional[str]) -> Dict[str, Any]:
    ...
    def _generate_engagement_insights(self, 
        ...
        if daily_metrics:
            avg_response_rate = sum(d['response_rate'] for d in daily_metrics) / len(daily_metrics)
            if avg_response_rate > 80:
                insights.append("Excellent response rates - current strategy is effective")
            elif avg_response_rate < 50:
                insights.append("Low response rates - consider message timing or content adjustments")
        
        return insights
    
    def _analyze_risk_factors(self, at_risk_patients: List[PatientRisk]) -> Dict[str, Any]:
    ...
    async def _generate_intervention_recommendations(self, at_risk_patients: List[PatientRisk]) -> List[Dict[str, Any]]:
        """Generate intervention recommendations for at-risk patients."""
        recommendations = []
        
        # Group by risk level
        critical_patients = [p for p in at_risk_patients if p.risk_level == RiskLevel.CRITICAL]
        high_risk_patients = [p for p in at_risk_patients if p.risk_level == RiskLevel.HIGH]
    ...
    async def _analyze_message_timing(self, 
                                    flow_type: str, 
        ...
        return {
            ...
            'recommendations': ['Send messages between 10 AM and 4 PM for best response rates']
        }
    
    async def _analyze_content_effectiveness(self, 
                                           flow_type: str, 
                                           date_range: Tuple[datetime, datetime]) -> Dict[str, Any]:
        """Analyze content effectiveness."""
        return {
    ...
    async def _analyze_flow_dropoffs(self, 
                                   flow_type: str, 
                                   date_range: Tuple[datetime, datetime]) -> Dict[str, Any]:
    ...
    def _generate_optimization_recommendations(self, 
                                             ...
                                             dropoff_analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if response_rate < 70:
            recommendations.append({
                ...
                'actions': ['Review message timing', 'A/B test message content', 'Personalize messages more']
            })
    ...
    async def _check_sentiment_alerts(self) -> List[Dict[str, Any]]:
        ...
        recent_negative = self.db.query(FlowAnalytics).filter(
            and_(
