...
class DataIntegrityMonitoringService:
    ...
    async def run_comprehensive_integrity_scan(
        ...
        try:
            ...
            scan_results = {
                ...
                "entities_scanned": {"patients": 0, "flows": 0, "messages": 0},
                "issues_detected": {
                    "total": 0,
                    "by_type": {},
                    "by_severity": {},
            ...
            scan_results["entities_scanned"]["patients"] = patient_results[
                "entities_scanned"
            ]
            ...
            scan_results["entities_scanned"]["messages"] = message_results[
                "entities_scanned"
            ]
            ...
            logger.info(
                f"Integrity scan completed: {total_issues} issues detected in "
    ...
    async def _scan_patient_integrity(
        self, limit: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for flow in flows:
                try:
                    ...
                    ref_issues = (
                        await self.flow_integrity.validate_referential_integrity(flow)
                    )
                    if ref_issues:
    ...
    async def _scan_message_integrity(
        self, limit: Optional[int] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for patient_id in patient_ids:
                try:
                    ...
                    conv_result = (
                        await self.message_integrity.validate_conversation_integrity(
                            patient_id
                        )
                    )
                    ...
                    logger.error(
                        f"Error scanning messages for patient {patient_id}: {e}"
                    )
    ...
    async def _check_patient_duplicates(self, patient: Patient) -> None:
        ...
        try:
            ...
            if patient.cpf:
                cpf_duplicate = await self.patient_integrity._check_duplicate_cpf(
                    patient.cpf
                )
                if cpf_duplicate and cpf_duplicate.id != patient.id:
                    self._add_integrity_issue(
                        ...
                        metadata={
                            "duplicate_patient_id": str(cpf_duplicate.id),
                            "field": "cpf",
                        },
                    )
            ...
            if patient.email:
                email_duplicate = await self.patient_integrity._check_duplicate_email(
                    patient.email
                )
                if email_duplicate and email_duplicate.id != patient.id:
                    self._add_integrity_issue(
                        ...
                        metadata={
                            "duplicate_patient_id": str(email_duplicate.id),
                            "field": "email",
                        },
                    )
    ...
    async def _validate_patient_data_consistency(self, patient: Patient) -> None:
        ...
        try:
            ...
            if patient.treatment_start_date and patient.birth_date:
                if patient.treatment_start_date < patient.birth_date:
                    self._add_integrity_issue(
                        ...
                        metadata={
                            "treatment_start": patient.treatment_start_date.isoformat(),
            ...
            logger.error(
                f"Error validating patient data consistency for {patient.id}: {e}"
            )
    ...
    async def _check_patient_orphaned_relationships(self, patient: Patient) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error checking orphaned relationships for patient {patient.id}: {e}"
            )
    ...
    def _generate_integrity_recommendations(self) -> List[str]:
        ...
        if type_counts.get("patient_duplicate", 0) > 0:
            recommendations.append(
                "Consider implementing automated duplicate patient merging"
            )
        ...
        critical_issues = [
            i for i in self.detected_issues if i.severity == IntegritySeverity.CRITICAL
        ]
        if critical_issues:
    ...
    async def auto_repair_integrity_issues(
        self,
        ...
        try:
            ...
            if issue_types:
                issues_to_repair = [
                    i for i in issues_to_repair if i.type in issue_types
                ]
                ...
                try:
                    if issue.type == IntegrityIssueType.CHECKSUM_MISMATCH:
                        ...
                        repair_results["details"].append(
                            f"Repaired checksum for {issue.entity_type} {issue.entity_id}"
                        )
                        ...
                        repair_results["details"].append(
                            f"Skipped duplicate repair (manual review required): {issue.entity_id}"
                        )

                    else:
                        repair_results["skipped"] += 1
                        repair_results["details"].append(
                            f"No auto-repair available for {issue.type.value}"
                        )

                except Exception as e:
                    repair_results["failed"] += 1
                    repair_results["details"].append(
                        f"Failed to repair {issue.entity_id}: {str(e)}"
                    )
                    logger.error(f"Auto-repair failed for issue {issue.id}: {e}")

            logger.info(
                f"Auto-repair completed: {repair_results['repaired']} repaired, "
