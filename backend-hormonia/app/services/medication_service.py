...
class MedicationService:
    ...
    @with_db_retry()
    def create_medication(self, data: MedicationV2Create, prescribed_by_id: UUID, treatment_id: Optional[UUID] = None) -> Medication:
        create_data = data.dict(exclude={"patient_id", "prescribed_by_id", "treatment_id"})
    ...
    @with_db_retry()
    def update_medication(self, id: UUID, data: MedicationV2Update) -> Optional[Medication]:
        medication = self.repository.get_by_id(id)
        if not medication: return None
    ...
    def delete_medication(self, id: UUID) -> bool:
        medication = self.repository.get_by_id(id)
        if not medication: return False
