...
class ConsentRecord(Base):
    ...
    consent_type = Column(String, nullable=False)  # data_collection, data_processing, marketing
...
class PrivacyService:
    ...
    def revoke_consent(
        ...
        self.logger.info(
            f"Consent revoked: {consent_type}",
            extra={'patient_id': str(patient_id)}
        )
    ...
    def export_patient_data(
        ...
        self.logger.info(
            f"Data export generated for patient",
            extra={'patient_id': str(patient_id)}
        )
    ...
    def delete_patient_data(
        ...
        if scope in ["all", "quiz_only"]:
            ...
            deleted_counts["responses"] = self.db.query(QuizResponse).filter(
                QuizResponse.patient_id == patient_id
            ).delete()

            # Delete quiz sessions
            deleted_counts["sessions"] = self.db.query(QuizSession).filter(
                QuizSession.patient_id == patient_id
            ).delete()

        if scope in ["all", "consents_only"]:
            # Delete consents (but keep audit trail for legal compliance)
            deleted_counts["consents"] = self.db.query(ConsentRecord).filter(
                ConsentRecord.patient_id == str(patient_id)
            ).delete()
    ...
    def anonymize_patient_data(
        ...
        old_responses = self.db.query(QuizResponse).filter(
            QuizResponse.patient_id == patient_id,
        ...
        return {
            "anonymized_responses": anonymized_count
        }
