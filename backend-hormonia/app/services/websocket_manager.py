...
class ConnectionManager:
    ...
    async def authenticate_connection(
        ...
        try:
            ...
            if not subject:
                logger.warning(f"Invalid token for connection {connection_id}: no user ID")
                return None
            
            # Check token expiration
            exp = payload.get("exp")
            if exp is None or datetime.utcnow().timestamp() > exp:
                logger.warning(f"Expired token for connection {connection_id}")
                return None
            ...
            if not user or not user.is_active:
                logger.warning(f"Invalid or inactive user for connection {connection_id}")
                return None
            ...
            logger.info(f"WebSocket connection authenticated: {connection_id} for user {subject}")
            return user
...
class WebSocketManager:
    ...
    async def join_room(self, connection_id: str, room_id: str, room_type: str = "general") -> None:
        """Join a connection to a room."""
        await self.connection_manager.join_room(connection_id, room_id, room_type)

    async def leave_room(self, connection_id: str, room_id: str, room_type: str = "general") -> None:
        """Remove a connection from a room."""
        await self.connection_manager.leave_room(connection_id, room_id, room_type)
    ...
    async def send_to_user(self, message: dict, user_id: str) -> int:
        ...
        return await self.connection_manager.send_to_user(message, user_id)

    async def broadcast_to_room(self, message: dict, room_id: str, room_type: str = "general") -> int:
        """Broadcast a message to all connections in a room."""
        return await self.connection_manager.broadcast_to_room(message, room_id, room_type)
    ...
    def get_user_connection_count(self, user_id: str) -> int:
        ...
        return self.connection_manager.get_user_connection_count(user_id)

    def get_room_connection_count(self, room_id: str, room_type: str = "general") -> int:
        """Get the number of connections in a room."""
        return self.connection_manager.get_room_connection_count(room_id, room_type)
    ...
    def get_connection_metadata(self, connection_id: str) -> Optional[dict]:
        ...
        return self.connection_manager.get_connection_metadata(connection_id)
