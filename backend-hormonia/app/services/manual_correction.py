...
class ManualCorrectionService:
    ...
    async def get_correction_recommendations(self, patient_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            issues = await self.corruption_detector.detect_flow_state_corruption(patient_id)
    ...
    async def apply_correction(self, patient_id: UUID, action: str, 
        ...
        try:
            ...
            # Log correction
            await self._log_correction_applied(patient_id, action, parameters, result, backup_key)
            
            # Verify correction worked
            post_correction_issues = await self.corruption_detector.detect_flow_state_corruption(patient_id)
            ...
            logger.error(f"Error applying correction {action} to patient {patient_id}: {e}")
            return {
                'success': False,
                'error': str(e),
                'action': action,
    ...
    async def apply_bulk_corrections(self, corrections: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
    ...
    async def _generate_recommendation(self, issue: Dict[str, Any], patient_id: UUID) -> Optional[Dict[str, Any]]:
    ...
    async def _reset_to_calculated_day(self, patient_id: UUID, flow_state: PatientFlowState,
                                     parameters: Dict[str, Any]) -> Dict[str, Any]:
        """Reset current day based on enrollment date."""
        try:
            if not flow_state.enrollment_date:
                return {'success': False, 'error': 'No enrollment date to calculate from'}
            
            # Calculate correct day
            days_since_enrollment = (datetime.utcnow() - flow_state.enrollment_date).days + 1
    ...
    async def _update_flow_type(self, patient_id: UUID, flow_state: PatientFlowState,
                              parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _fix_enrollment_date(self, patient_id: UUID, flow_state: PatientFlowState,
                                 parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if new_enrollment_date:
                ...
                new_enrollment_date = datetime.utcnow() - timedelta(days=flow_state.current_day - 1)
            ...
            return {
                ...
                'changes': {
                    'old_enrollment_date': old_enrollment_date.isoformat() if old_enrollment_date else None,
    ...
    async def _repair_state_data(self, patient_id: UUID, flow_state: PatientFlowState,
                               parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'changes': {
                    'repaired_fields': [missing_field] if missing_field else ['message_count', 'monthly_cycle'],
    ...
    async def _rebuild_message_sequence(self, patient_id: UUID, flow_state: PatientFlowState,
                                      parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _reset_temporal_fields(self, patient_id: UUID, flow_state: PatientFlowState,
                                   parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if flow_state.last_message_sent and flow_state.last_message_sent > datetime.utcnow():
                changes['old_last_message_sent'] = flow_state.last_message_sent.isoformat()
                flow_state.last_message_sent = datetime.utcnow() - timedelta(hours=1)
                changes['new_last_message_sent'] = flow_state.last_message_sent.isoformat()
            
            # Reset next_message_due if it's too far overdue
            if flow_state.next_message_due and flow_state.next_message_due < datetime.utcnow() - timedelta(days=2):
                changes['old_next_message_due'] = flow_state.next_message_due.isoformat()
                flow_state.next_message_due = datetime.utcnow() + timedelta(hours=1)
                changes['new_next_message_due'] = flow_state.next_message_due.isoformat()
    ...
    async def _pause_for_investigation(self, patient_id: UUID, flow_state: PatientFlowState,
                                     parameters: Dict[str, Any]) -> Dict[str, Any]:
        """Pause flow for manual investigation."""
        try:
            reason = parameters.get('reason', 'Data corruption detected - requires investigation')
    ...
    async def _reset_flow_completely(self, patient_id: UUID, flow_state: PatientFlowState,
                                   parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            backup_data = {
                'flow_state': {
                    ...
                    'flow_type': flow_state.flow_type,
                    'current_day': flow_state.current_day,
                    'enrollment_date': flow_state.enrollment_date.isoformat() if flow_state.enrollment_date else None,
                    'last_message_sent': flow_state.last_message_sent.isoformat() if flow_state.last_message_sent else None,
                    'next_message_due': flow_state.next_message_due.isoformat() if flow_state.next_message_due else None,
    ...
    async def _create_correction_backup(self, patient_id: UUID, flow_state: PatientFlowState) -> str:
        ...
        await self.redis.setex(backup_key, 86400 * 30, json.dumps(backup_data))  # Keep for 30 days
        
        return backup_key
    
    async def _log_correction_applied(self, patient_id: UUID, action: str,
        ...
        logger.info(f"Applied correction {action} to patient {patient_id}: {result.get('success', False)}")
    ...
    async def restore_from_backup(self, backup_key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            flow_state.enrollment_date = datetime.fromisoformat(backup_flow_state['enrollment_date']) if backup_flow_state['enrollment_date'] else None
            flow_state.last_message_sent = datetime.fromisoformat(backup_flow_state['last_message_sent']) if backup_flow_state['last_message_sent'] else None
            flow_state.next_message_due = datetime.fromisoformat(backup_flow_state['next_message_due']) if backup_flow_state['next_message_due'] else None
