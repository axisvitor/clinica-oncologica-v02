"""
JWT Cache Service - Redis-based JWT validation caching with token blacklist.

This service implements:
- Supabase JWT validation caching (reduces API calls)
- Token blacklist for logout functionality
- Automatic TTL based on JWT exp claim
- Fallback to direct Supabase validation if Redis unavailable
"""
...
class JWTCacheService:
    ...
    Features:
    - Cache Supabase JWT validation results
    ...
    async def validate_token_cached(self, token: str) -> Optional[Dict[str, Any]]:
        ...
        3. If cache miss, validate at Supabase
        ...
        Returns:
            Supabase user data dict if valid, None otherwise
        """
        if not token:
            return None
        ...
        # Cache miss or Redis unavailable - validate at Supabase
        supabase_data = await self._validate_at_supabase(token)
        if not supabase_data:
            return None

        # Cache the validation result
        if redis_available:
            ttl = self._calculate_ttl(supabase_data)
            await self.cache_validation(token_hash, supabase_data, ttl)
    ...
    async def _validate_at_supabase(self, token: str) -> Optional[Dict[str, Any]]:
        """
        Validate JWT token at Supabase Auth API.

        Args:
            token: JWT token string

        Returns:
            Supabase user data dict or None
        """
        try:
            ...
            data = resp.json()
            logger.debug(f"Token validated at Supabase for email: {data.get('email')}")
            return data

        except Exception as e:
            logger.error(f"Error validating token at Supabase: {e}")
            return None
    ...
    async def cache_validation(
        ...
        Args:
            token_hash: SHA256 hash of token
            claims: Supabase user data/claims to cache
