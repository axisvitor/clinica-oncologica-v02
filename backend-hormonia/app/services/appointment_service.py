...
class AppointmentService:
    ...
    def create_appointment(self, data: AppointmentV2Create) -> Appointment:
        ...
        if practitioner_uuid and data.scheduled_at:
            if self._check_conflict(practitioner_uuid, data.scheduled_at, data.duration_minutes):
                raise ValueError("Scheduling conflict detected")
            
        # Create
        create_data = data.dict(exclude={"patient_id", "practitioner_id", "status", "appointment_type"})
        ...
        if data.appointment_type:
            create_data["appointment_type"] = AppointmentType(data.appointment_type.lower())
    ...
    @with_db_retry()
    def update_appointment(self, id: UUID, data: AppointmentV2Update) -> Optional[Appointment]:
        ...
        if "scheduled_at" in update_data or "duration_minutes" in update_data:
            ...
            if new_start and new_duration and practitioner_id:
                if self._check_conflict(practitioner_id, new_start, new_duration, exclude_id=id):
                    raise ValueError("Scheduling conflict detected")
        ...
        if "appointment_type" in update_data:
            update_data["appointment_type"] = AppointmentType(update_data["appointment_type"].lower())
        if "practitioner_id" in update_data and update_data["practitioner_id"]:
    ...
    def cancel_appointment(self, id: UUID) -> Optional[Appointment]:
        ...
        if hasattr(current_status, 'value'): current_status = current_status.value
    ...
    @with_db_retry()
    def complete_appointment(self, id: UUID, notes: Optional[str] = None) -> Optional[Appointment]:
        appt = self.repository.get_by_id(id)
        if not appt: return None
        ...
        if current_status != AppointmentStatus.IN_PROGRESS.value:
            raise ValueError(f"Cannot complete appointment with status {current_status}. Must be in_progress.")
    ...
    def _check_conflict(self, practitioner_id: UUID, start: datetime, duration: int, exclude_id: Optional[UUID] = None) -> bool:
        end = start + timedelta(minutes=duration)
        conflicts = self.repository.find_conflicts(practitioner_id, start, end, exclude_id)
