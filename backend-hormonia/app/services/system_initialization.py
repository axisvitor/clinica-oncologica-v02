...
class SystemInitializationService:
    ...
    async def _initialize_firebase(self) -> None:
        ...
        try:
            ...
            if not all([
                ...
                self.logger.warning("⚠️ Firebase credentials not configured, skipping Firebase initialization")
            ...
            user_list = firebase_auth.list_users(max_results=1)
            ...
            self.initialization_status["components"]["firebase"] = {
                ...
                "project_id": settings.FIREBASE_ADMIN_PROJECT_ID,
                "security_config": {
                    "allowed_domains_count": len(security_config.get("allowed_domains", [])),
                    "require_custom_claims": security_config.get("require_custom_claims"),
                    "audit_logging_enabled": security_config.get("enable_audit_logging")
                },
                "tested_at": datetime.utcnow().isoformat()
            }
    ...
    async def _initialize_security(self) -> None:
        ...
        try:
            ...
            if security_checks:
                self.logger.warning(f"⚠️ Security warnings found: {'; '.join(security_checks)}")
            else:
    ...
    async def _validate_service_dependencies(self) -> None:
        ...
        try:
            ...
            if self.initialization_status["components"].get("database", {}).get("status") == "success":
                dependencies.append("database")
            
            if self.initialization_status["components"].get("redis", {}).get("status") == "success":
                dependencies.append("redis")
            
            if self.initialization_status["components"].get("firebase", {}).get("status") in ["success", "skipped"]:
            ...
            self.logger.info(f"✅ Service dependencies validation successful: {len(dependencies)} core, {len(external_services)} external")
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                health_status["components"]["database"] = {"status": "healthy", "latency_ms": "< 10"}
            except Exception as e:
            ...
            try:
                redis_client = get_redis_client()
                redis_client.ping()
                health_status["components"]["redis"] = {"status": "healthy", "latency_ms": "< 5"}
            except Exception as e:
            ...
            try:
                if all([settings.FIREBASE_ADMIN_PROJECT_ID, settings.FIREBASE_ADMIN_PRIVATE_KEY, settings.FIREBASE_ADMIN_CLIENT_EMAIL]):
                    firebase_auth.list_users(max_results=1)
                    health_status["components"]["firebase"] = {"status": "healthy"}
                else:
                    health_status["components"]["firebase"] = {"status": "not_configured"}
            except Exception as e:
                health_status["components"]["firebase"] = {"status": "unhealthy", "error": str(e)}
                health_status["status"] = "degraded"
            
            # Calculate overall health score
            healthy_components = sum(1 for comp in health_status["components"].values() if comp.get("status") == "healthy")
            total_components = len([comp for comp in health_status["components"].values() if comp.get("status") != "not_configured"])
            
            if total_components > 0:
                health_status["overall_score"] = (healthy_components / total_components) * 100
