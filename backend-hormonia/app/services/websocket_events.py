...
class WebSocketEventService:
    ...
    async def broadcast_flow_event(
        ...
        try:
            ...
            logger.info(
                f"Broadcasted flow event {event_type.value} for patient {patient_id} to {sent_count} connections"
            )
            return sent_count
    ...
    async def broadcast_alert_event(
        ...
        try:
            ...
            logger.info(
                f"Broadcasted alert event {event_type.value} to {sent_count} connections"
            )
            return sent_count
    ...
    async def broadcast_message_event(
        ...
        try:
            ...
            logger.info(
                f"Broadcasted message event {event_type.value} for patient {message_data['patient_id']} to {sent_count} connections"
            )
            return sent_count
    ...
    async def broadcast_quiz_event(
        self, event_type: WebSocketEventType, quiz_data: dict[str, Any]
    ) -> int:
        ...
        try:
            ...
            logger.info(
                f"Broadcasted quiz event {event_type.value} for patient {quiz_data['patient_id']} to {sent_count} connections"
            )
            return sent_count
    ...
    async def broadcast_report_event(
        ...
        try:
            ...
            logger.info(
                f"Broadcasted report event {event_type.value} for patient {report_data['patient_id']} to {sent_count} connections"
            )
            return sent_count
    ...
    async def broadcast_system_event(
        self, event_type: WebSocketEventType, system_data: dict[str, Any]
    ) -> int:
        ...
        try:
            ...
            logger.info(
                f"Broadcasted system event {event_type.value} to {sent_count} connections"
            )
            return sent_count
    ...
    async def notify_flow_progression(
        self, patient_id: UUID, flow_data: dict[str, Any]
    ) -> int:
        """Notify about flow progression."""
        return await self.broadcast_flow_event(
            WebSocketEventType.FLOW_PROGRESSION, patient_id, flow_data
        )

    async def notify_flow_state_changed(
        self, patient_id: UUID, flow_data: dict[str, Any]
    ) -> int:
        """Notify about flow state change."""
        return await self.broadcast_flow_event(
            WebSocketEventType.FLOW_STATE_CHANGED, patient_id, flow_data
        )

    async def notify_message_sent(
        self, patient_id: UUID, message_data: dict[str, Any]
    ) -> int:
        """Notify about message sent."""
        return await self.broadcast_message_event(
            WebSocketEventType.MESSAGE_SENT, message_data
        )
    ...
    async def notify_message_delivered(
        ...
        return await self.broadcast_message_event(
            WebSocketEventType.MESSAGE_DELIVERED, message_data
        )
    ...
    async def notify_message_failed(
        ...
        return await self.broadcast_message_event(
            WebSocketEventType.MESSAGE_FAILED, message_data
        )

    async def notify_quiz_started(
        self, patient_id: UUID, quiz_data: dict[str, Any]
    ) -> int:
        """Notify about quiz started."""
        return await self.broadcast_quiz_event(
            WebSocketEventType.QUIZ_STARTED, quiz_data
        )
    ...
    async def notify_quiz_completed(
        ...
        return await self.broadcast_quiz_event(
            WebSocketEventType.QUIZ_COMPLETED, quiz_data
        )

    async def notify_report_generated(
        self, patient_id: UUID, report_data: dict[str, Any]
    ) -> int:
        """Notify about report generation completion."""
        return await self.broadcast_report_event(
            WebSocketEventType.REPORT_GENERATION_COMPLETED, report_data
        )
    ...
    async def notify_alert_created(self, alert_data: dict[str, Any]) -> int:
        ...
        return await self.broadcast_alert_event(
            WebSocketEventType.ALERT_CREATED, alert_data
        )

    async def notify_alert_resolved(self, alert_data: dict[str, Any]) -> int:
        """Notify about alert resolution."""
        return await self.broadcast_alert_event(
            WebSocketEventType.ALERT_RESOLVED, alert_data
        )

    async def notify_system_maintenance(
        self, message: str, affected_services: List[str] = None
    ) -> int:
        ...
        return await self.broadcast_system_event(
            WebSocketEventType.SYSTEM_MAINTENANCE, system_data
        )

    async def notify_system_notification(
        ...
        return await self.broadcast_system_event(
            WebSocketEventType.SYSTEM_NOTIFICATION, system_data
        )
