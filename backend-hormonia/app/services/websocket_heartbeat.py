...
class WebSocketHeartbeatManager:
    ...
    def __init__(
        ...
        send_ping_callback: Optional[
            Callable[[str, Dict[str, Any]], Awaitable[bool]]
        ] = None,
        on_connection_dead: Optional[Callable[[str], None]] = None,
        on_connection_warning: Optional[
            Callable[[str, "HeartbeatMetrics"], None]
        ] = None,
        ...
        self.pending_pings: Dict[
            str, Dict[str, float]
        ] = {}  # connection_id -> {ping_id -> timestamp}
        self.ping_counter = 0

        # Callbacks
        self.send_ping_callback: Optional[
            Callable[[str, Dict[str, Any]], Awaitable[bool]]
        ] = send_ping_callback
        self.on_connection_dead: Optional[Callable[[str], None]] = on_connection_dead
        self.on_connection_warning: Optional[
            Callable[[str, HeartbeatMetrics], None]
        ] = on_connection_warning
    ...
    def register_connection(self, connection_id: str) -> None:
        ...
        if connection_id not in self.connection_metrics:
            ...
            logger.debug(
                f"Registered connection for heartbeat monitoring: {connection_id}"
            )
    ...
    def unregister_connection(self, connection_id: str) -> None:
        ...
        logger.debug(
            f"Unregistered connection from heartbeat monitoring: {connection_id}"
        )
    ...
    def handle_pong(
        self, connection_id: str, ping_id: str, client_timestamp: Optional[str] = None
    ) -> bool:
        """
        ...
        if ping_id not in self.pending_pings.get(connection_id, {}):
            logger.warning(
                f"Received pong for unknown ping {ping_id} from {connection_id}"
            )
            return False
        ...
        logger.debug(
            f"Received pong from connection {connection_id}: {ping_id} (latency: {latency:.2f}ms)"
        )
            ...
            try:
                ...
                for connection_id, metrics in self.connection_metrics.items():
                    ...
                    # Check for warning status
                        ...
                        metrics.missed_pings >= self.warning_threshold
                        and metrics.status == HeartbeatStatus.HEALTHY
                    ):
                ...
                logger.debug(
                    f"Heartbeat cycle completed. Monitored connections: {len(self.connection_metrics)}"
                )
    ...
    async def _perform_cleanup(self) -> None:
        ...
        for connection_id, pings in self.pending_pings.items():
            expired_pings = [
                ping_id
                for ping_id, timestamp in pings.items()
        ...
        if any(self.pending_pings.values()):
            logger.debug(
                f"Cleaned up expired pings. Total timeouts: {self.total_timeouts}"
            )
    ...
    async def _check_ping_timeout(self, connection_id: str, ping_id: str) -> None:
        ...
        if (
            connection_id in self.pending_pings
            and ping_id in self.pending_pings[connection_id]
        ):
            # Ping timed out
            del self.pending_pings[connection_id][ping_id]
            self.total_timeouts += 1
