...
class AlertProcessor:
        ...
        for channel_name, channel in self.notification_channels.items():
            ...
            try:
                if channel.channel_type == "email":
                    results[channel_name] = self._send_email_notifications(alert, target_users, channel)
                elif channel.channel_type == "websocket":
                    results[channel_name] = self._send_websocket_notifications(alert, channel)
                elif channel.channel_type == "webhook":
                    results[channel_name] = self._send_webhook_notifications(alert, channel)
                else:
                    logger.warning(f"Unknown notification channel type: {channel.channel_type}")
                    
            except Exception as e:
                logger.error(f"Error sending {channel_name} notification for alert {alert.id}: {e}")
    ...
    def _send_email_notifications(self, alert: Alert, users: List[User], channel: NotificationChannel) -> Dict[str, Any]:
        ...
        subject = (
            f"Medical Alert: {alert.alert_type.title()} - {alert.severity.value.title()}"
        )
        ...
        try:
            if provider == "sendgrid":
                ...
                ses = boto3.client(
                    "ses", region_name=channel.config.get("region")
                )
    ...
    def _send_websocket_notifications(self, alert: Alert, channel: NotificationChannel) -> Dict[str, Any]:
    ...
    def _send_webhook_notifications(self, alert: Alert, channel: NotificationChannel) -> Dict[str, Any]:
    ...
    def _schedule_escalation(self, alert: Alert) -> bool:
        ...
        logger.info(f"Scheduling escalation for alert {alert.id} in {escalation_rule.escalation_delay_minutes} minutes")
        ...
        alert.data["escalation"] = {
            ...
            "next_escalation_at": (datetime.utcnow() + timedelta(minutes=escalation_rule.escalation_delay_minutes)).isoformat()
        }
    ...
    def process_escalation(self, alert_id: UUID) -> Dict[str, Any]:
        ...
        if alert.status != AlertStatus.PENDING:
            logger.info(f"Alert {alert_id} already acknowledged/resolved, skipping escalation")
        ...
        if escalation_rule:
            ...
            for channel_name, channel in self.notification_channels.items():
                if channel.enabled:
                    try:
                        if channel.channel_type == "email":
                            notification_results[channel_name] = self._send_email_notifications(alert, escalation_users, channel)
                        elif channel.channel_type == "websocket":
                            notification_results[channel_name] = self._send_websocket_notifications(alert, channel)
                    except Exception as e:
            ...
            if current_escalation + 1 < max_escalations:
                escalation_data["next_escalation_at"] = (
                    datetime.utcnow() + timedelta(minutes=escalation_rule.escalation_delay_minutes)
                ).isoformat()
    ...
    async def acknowledge_alert(self, alert_id: UUID, user_id: UUID, notes: Optional[str] = None) -> Optional[Alert]:
    ...
    async def resolve_alert(self, alert_id: UUID, user_id: UUID, resolution_notes: Optional[str] = None) -> Optional[Alert]:
        """Resolve an alert."""
    ...
    def get_alert_dashboard_data(self) -> Dict[str, Any]:
        ...
        escalated_alerts = [
            ...
            if alert.data and alert.data.get("escalation", {}).get("current_escalation", 0) > 0
        ]
        ...
        return {
            ...
            "recent_alerts": [
                {
                    ...
                    "description": alert.description,
                    "created_at": alert.created_at.isoformat(),
                    "escalation_level": alert.data.get("escalation", {}).get("current_escalation", 0) if alert.data else 0
                }
            ...
            "escalation_summary": {
                ...
                ) / len(escalated_alerts) if escalated_alerts else 0
            },
    ...
    def update_notification_channel(self, channel_name: str, enabled: bool, config: Optional[Dict[str, Any]] = None) -> bool:
