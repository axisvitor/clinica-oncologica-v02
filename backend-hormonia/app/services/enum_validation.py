...
class EnumValidationError(Exception):
    ...
    def __init__(
        self, message: str, enum_type: str, invalid_value: Any, valid_values: List[str]
    ):
...
class EnumValidationService:
    ...
    def validate_enum_value(
        cls, enum_class: Type[Enum], value: Any, allow_none: bool = False
    ) -> Optional[Enum]:
        """
    ...
    @classmethod
    def handle_enum_validation_error(
        cls, error: EnumValidationError, context: Optional[Dict[str, Any]] = None
    ) -> None:
        """
        ...
        logger.error(
            f"Enum validation failed for {error.enum_type}: {error}", extra=log_context
        )
