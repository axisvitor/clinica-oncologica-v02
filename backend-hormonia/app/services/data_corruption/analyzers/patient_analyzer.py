...
class PatientAnalyzer(BaseAnalyzer):
    ...
    async def analyze(self, patient) -> list:
        ...
        try:
            # Analyze name field
            await self.field_analyzer.analyze_text_field(
                patient.name, "patient.name", patient.id
            )

            # Analyze email field
            if patient.email:
                self.format_validator.validate_email(
                    patient.email, "patient.email", patient.id
                )

            # Analyze phone field
            self.format_validator.validate_phone(
                patient.phone, "patient.phone", patient.id
            )

            # Analyze metadata corruption
            if patient.patient_data:
                await self.field_analyzer.analyze_metadata(
                    patient.patient_data, "patient.metadata", patient.id
                )
            ...
            fields_to_check = [("name", patient.name), ("email", patient.email)]
            for field_name, field_value in fields_to_check:
                if field_value:
                    await self.encoding_analyzer.check_encoding(
                        field_value, f"patient.{field_name}", patient.id
                    )
