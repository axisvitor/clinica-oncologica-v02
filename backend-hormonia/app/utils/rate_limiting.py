...
RATE_LIMITS = {
    ...
    RateLimitType.READ: RateLimit(
        calls=100, period=60, burst=20
    ),  # 100 per minute, 20 burst
    RateLimitType.WRITE: RateLimit(
        calls=50, period=60, burst=10
    ),  # 50 per minute, 10 burst
    RateLimitType.WEBHOOK: RateLimit(
        calls=1000, period=60, burst=100
    ),  # 1000 per minute, 100 burst
    RateLimitType.UPLOAD: RateLimit(
        calls=10, period=60, burst=3
    ),  # 10 per minute, 3 burst
}
...
class RateLimiter:
    ...
    async def is_allowed(
        self, key: str, rate_limit: RateLimit, current_time: Optional[float] = None
    ...
    async def _check_local_rate_limit(
        ...
        async with self._cache_lock:
            ...
            # Clean old entries
            while self._local_cache[key] and self._local_cache[key][0] < window_start:
            ...
            # Calculate headers
            remaining = max(
                0, rate_limit.calls - current_count - (1 if is_allowed else 0)
            )
            reset_time = int(current_time + rate_limit.period)
...
async def check_rate_limit(
    request: Request, rate_limit_type: RateLimitType
) -> dict[str, int]:
    """
