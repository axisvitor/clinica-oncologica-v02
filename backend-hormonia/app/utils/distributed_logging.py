...
class DistributedLogger:
    ...
    def _get_current_context(self) -> LogContext:
        ...
        return LogContext(
            trace_id=trace_id, span_id=span_id, agent_id=agent_id, task_id=task_id
        )
    ...
    def get_logs_by_trace(self, trace_id: str) -> List[DistributedLogEntry]:
        ...
        return [
            entry for entry in self.log_buffer if entry.context.trace_id == trace_id
        ]

    def get_logs_by_agent(self, agent_id: str) -> List[DistributedLogEntry]:
        """Get all logs for specific agent."""
        return [
            entry for entry in self.log_buffer if entry.context.agent_id == agent_id
        ]

    def get_logs_by_task(self, task_id: str) -> List[DistributedLogEntry]:
        """Get all logs for specific task."""
        return [entry for entry in self.log_buffer if entry.context.task_id == task_id]
...
class LogAggregator:
        ...
        try:
            ...
            if entry.level in [LogLevel.ERROR, LogLevel.CRITICAL]:
                ...
                self.error_patterns[error_key] = (
                    self.error_patterns.get(error_key, 0) + 1
                )
            ...
            if "response_time" in entry.metadata:
                ...
                self.performance_metrics[component].append(
                    entry.metadata["response_time"]
                )

                # Keep only recent metrics
                if len(self.performance_metrics[component]) > 100:
                    self.performance_metrics[component] = self.performance_metrics[
                        component
                    ][-100:]
    ...
    def export_logs(
        ...
        if format == "json":
            ...
            return "\n".join(
                [f"{log.timestamp} [{log.level.value}] {log.message}" for log in logs]
            )
