...
class ErrorTracker:
    ...
    def _find_duplicate_error(self, error_event: ErrorEvent) -> Optional[ErrorEvent]:
        ...
        for existing_error in reversed(self.recent_errors):
            if (
                existing_error.error_type == error_event.error_type
                and existing_error.message == error_event.message
                and (error_event.timestamp - existing_error.timestamp)
                < timedelta(minutes=1)
            ):
    ...
    def _check_alert_conditions(self, error_event: ErrorEvent) -> None:
        ...
        should_alert = total_count >= self.alert_threshold and (
            error_type not in self.last_alert_time
            or (now - self.last_alert_time[error_type]) > self.alert_window
        )
    ...
    def _send_notification(
        self, message: str, error_event: ErrorEvent, count: int
    ) -> None:
    ...
    def get_error_summary(self, hours: int = 24) -> dict[str, Any]:
        ...
        for error in recent_errors:
            ...
            if (
                summary["last_occurrence"] is None
                or error.timestamp > summary["last_occurrence"]
            ):
        ...
        for error_type, summary in error_summary.items():
            result[error_type] = {
                ...
                if summary["last_occurrence"]
                else None,
                "unique_messages": len(summary["messages"]),
                "sample_messages": list(summary["messages"])[
                    :3
                ],  # First 3 unique messages
            }
        ...
        return {
            ...
            "summary_generated_at": now_sao_paulo().isoformat(),
        }
