...
class HealthMonitor:
    ...
    async def _check_system_resources(self) -> None:
        ...
        try:
            ...
            system_health = ComponentHealth(
                ...
                status=max(cpu_status, memory_status, disk_status, key=lambda x: x.value),
                metrics=[
                    HealthMetric(
                        name='cpu_percent',
                        value=cpu_percent,
    ...
    async def _check_external_services(self) -> None:
        ...
        for service_config in services:
            ...
            try:
                ...
                self.logger.error(f"{service_config['name']} health check failed: {str(e)}", exc_info=True)
                self.components[service_config['name']] = ComponentHealth(
                    name=service_config['name'],
                    status=HealthStatus.CRITICAL,
                    error_message=str(e),
    ...
    async def _check_application_health(self) -> None:
        ...
        try:
            ...
            recent_critical_errors = sum(
                1 for error_type, details in error_summary.get('errors_by_type', {}).items()
            ...
            self.components['application'] = ComponentHealth(
                ...
                metrics=[
                    ...
                    HealthMetric(
                        ...
                        unit='count',
                        status=HealthStatus.CRITICAL if recent_critical_errors > 0 else HealthStatus.HEALTHY
                    ),
                    HealthMetric(
            ...
            self.logger.error(f"Application health check failed: {str(e)}", exc_info=True)
            self.components['application'] = ComponentHealth(
                name='application',
                status=HealthStatus.CRITICAL,
                error_message=str(e)
    ...
    def _build_health_response(self, overall_status: HealthStatus, check_duration: float) -> dict[str, Any]:
        ...
        return {
            ...
            'summary': {
                ...
                'unhealthy_components': len([c for c in self.components.values() if c.status == HealthStatus.UNHEALTHY]),
    ...
    def get_metrics_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            'metrics': all_metrics,
            'last_collection': self.last_full_check.isoformat() + 'Z' if self.last_full_check else None
        }
