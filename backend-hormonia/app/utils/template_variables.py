...
class TemplateVariableProcessor:
    ...
    VARIABLE_MAPPINGS = {
        ...
        "{treatment_start_date}": ["treatment_start_date", "patient_data.treatment_start_date"],
    ...
    @classmethod
    def _get_context_value(cls, context: Dict[str, Any], paths: List[str]) -> Optional[Any]:
        """
    ...
    @classmethod
    def _substitute_computed_variables(cls, content: str, context: Dict[str, Any]) -> str:
        """
        ...
        if "{progress_percentage}" in content:
            current_day = cls._get_context_value(
                context,
                ["current_day", "treatment_day", "patient_data.current_day"]
            )
            if current_day:
                try:
