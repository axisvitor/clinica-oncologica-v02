...
class DatabaseOptimizer:
    ...
    def get_query_stats(self) -> dict[str, Any]:
        ...
        avg_duration = sum(stat.duration_ms for stat in self.query_stats) / total_queries
        slow_queries = sum(1 for stat in self.query_stats if stat.duration_ms > self.slow_query_threshold_ms)
        ...
        for query_type in query_types:
            type_stats = [stat for stat in self.query_stats if self._classify_query(stat.query) == query_type]
            query_types[query_type]["avg_duration_ms"] = sum(stat.duration_ms for stat in type_stats) / len(type_stats)
    ...
    def get_slowest_queries(self, limit: int = 10) -> List[dict[str, Any]]:
        ...
        sorted_stats = sorted(self.query_stats, key=lambda x: x.duration_ms, reverse=True)
