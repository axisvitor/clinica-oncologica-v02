...
class SensitiveDataFilter(logging.Filter):
    ...
    def _filter_string(self, text: str) -> str:
        ...
        # Filter JWT tokens
        text = re.sub(r'Bearer\s+[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+\.[A-Za-z0-9\-_]+', 'Bearer [REDACTED]', text)
        
        # Filter API keys (common patterns)
        text = re.sub(r'["\']?api[_-]?key["\']?\s*[:=]\s*["\']?[A-Za-z0-9\-_]{20,}["\']?', 'api_key: [REDACTED]', text, flags=re.IGNORECASE)
...
def log_function_call(func_name: str, args: tuple = None, kwargs: dict[str, Any] = None, 
...
def log_database_operation(operation: str, table: str, record_id: Optional[str] = None,
                          duration: Optional[float] = None, logger: Optional[logging.Logger] = None) -> None:
...
def log_external_api_call(service: str, endpoint: str, method: str, status_code: Optional[int] = None,
                         duration: Optional[float] = None, logger: Optional[logging.Logger] = None) -> None:
...
def log_business_event(event_type: str, description: str, entity_id: Optional[str] = None,
...
def log_security_event(event_type: str, description: str, user_id: Optional[str] = None,
                      ...
                      severity: str = 'INFO', logger: Optional[logging.Logger] = None) -> None:
    """Log security events."""
    if logger is None:
        logger = logging.getLogger(__name__)
...
def log_performance_metric(metric_name: str, value: Union[int, float], unit: str = 'ms',
                          tags: Optional[dict[str, str]] = None, logger: Optional[logging.Logger] = None) -> None:
