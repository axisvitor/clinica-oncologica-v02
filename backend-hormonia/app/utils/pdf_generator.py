...
try:
    ...
    from reportlab.lib.enums import TA_CENTER, TA_LEFT, TA_RIGHT, TA_JUSTIFY  # noqa: F401
...
class PDFGenerator:
    ...
    def generate_patient_report(
        ...
        try:
            ...
            # Header
            story.append(
                Paragraph("MEDICAL PATIENT REPORT", self.styles["CustomHeader"])
            )
            story.append(Spacer(1, 20))

            # Patient Information Section
            story.append(
                Paragraph("Patient Information", self.styles["CustomSubHeader"])
            )
            ...
            if "treatment_progress" in report_data:
                story.append(
                    Paragraph("Treatment Progress", self.styles["CustomSubHeader"])
                )
                ...
                progress_data = [
                    ...
                    [
                        "Adherence Rate",
                        f"{progress.get('adherence_rate', 0):.1%}",
                        self._get_status_symbol(progress.get("adherence_rate", 0), 0.8),
                    ],
                    [
                        "Response Rate",
                        f"{progress.get('response_rate', 0):.1%}",
                        self._get_status_symbol(progress.get("response_rate", 0), 0.7),
                    ],
                    [
                        "Quiz Completion",
                        f"{progress.get('quiz_completion', 0):.1%}",
                        self._get_status_symbol(
                            progress.get("quiz_completion", 0), 0.8
                        ),
                    ],
                ]

                progress_table = Table(
                    progress_data, colWidths=[2 * inch, 1.5 * inch, 1 * inch]
                )
            ...
            if "alerts" in report_data:
                story.append(
                    Paragraph(
                        "Recent Alerts & Symptoms", self.styles["CustomSubHeader"]
                    )
                )
                ...
                if alerts:
                    ...
                    for alert in alerts[:10]:  # Show last 10 alerts
                        alert_data.append(
                            [
                                ...
                                else alert.get("message", "N/A"),
                            ]
                        )

                    alert_table = Table(
                        alert_data,
                        colWidths=[1 * inch, 1 * inch, 1.5 * inch, 2.5 * inch],
                    )
                    ...
                    story.append(
                        Paragraph(
                            "No alerts recorded in the selected period.",
                            self.styles["MedicalData"],
                        )
                    )
            ...
            if "analytics" in report_data:
                story.append(
                    Paragraph("Analytics Summary", self.styles["CustomSubHeader"])
                )
            ...
            story.append(
                Paragraph("MEDICAL DISCLAIMER", self.styles["CustomSubHeader"])
            )
    ...
    def generate_analytics_report(
        ...
        try:
            ...
            story.append(
                Paragraph(
                    f"ANALYTICS REPORT - {time_period.upper()}",
            ...
            # Summary Statistics
            story.append(
                Paragraph("Summary Statistics", self.styles["CustomSubHeader"])
            )
            ...
            summary_data = [
                ...
                [
                    ...
                    f"{analytics_data.get('avg_response_time', 0):.1f} hours",
                ],
                [
                    "Quiz Completion Rate",
                    f"{analytics_data.get('quiz_completion_rate', 0):.1%}",
                ],
            ]
...
class SimplePDFGenerator:
    ...
    def __init__(self):
        logger.warning(
            "Using simple PDF generator. Install reportlab for advanced features."
        )
