...
class EnhancedCompressionMiddleware(BaseHTTPMiddleware):
    ...
    def _should_compress(self, request: Request, response: Response, accept_encoding: str) -> bool:
        """Determine if response should be compressed."""
        # Skip if client doesn't accept compression
        if not any(encoding in accept_encoding.lower() for encoding in ["gzip", "deflate"]):
            return False
        ...
        if any(path in request.url.path for path in ["/download", "/export", "/stream"]):
            return False
        
        return True
    
    def _compress_body(self, body: bytes, accept_encoding: str) -> tuple[Optional[bytes], Optional[str]]:
...
def get_compression_stats(responses: List[dict]) -> dict:
    ...
    total_compressed_size = sum(r.get("compressed_size", r.get("original_size", 0)) for r in responses)
    
    bytes_saved = total_original_size - total_compressed_size
    compression_ratio = (bytes_saved / total_original_size * 100) if total_original_size > 0 else 0
    
    return {
        "total_responses": total_responses,
        "compressed_responses": compressed_responses,
        "compression_percentage": round((compressed_responses / total_responses) * 100, 2),
