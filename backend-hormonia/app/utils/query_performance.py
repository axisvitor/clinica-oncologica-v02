...
class QueryPerformanceMonitor:
    ...
    def end_session(self, session_duration: float):
        ...
        with self.lock:
            ...
            if len(self.session_metrics["durations"]) > 1000:
                self.session_metrics["durations"] = self.session_metrics["durations"][
                    -1000:
                ]
                self.session_metrics["timestamps"] = self.session_metrics["timestamps"][
                    -1000:
                ]
        ...
        with self.lock:
            if query_hash not in self.metrics:
                ...
                self.metrics[query_hash] = QueryMetrics(
                    query_hash=query_hash, query_text=self._normalize_query(query)
                )
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            # Top slow queries
            top_slow = sorted(
                self.metrics.values(), key=lambda m: m.avg_time, reverse=True
            )[:10]
            ...
            return {
                ...
                "recent_slow_queries": [
                    {
                        "query_text": sq.query_text[:200] + "..."
                        if len(sq.query_text) > 200
                        else sq.query_text,
            ...
            for query_hash, metrics in self.metrics.items():
                if metrics.avg_time > 1.0:  # Slow queries
                    ...
                    if "order by" in query_lower and "limit" not in query_lower:
                        suggestion["suggestions"].append(
                            "Add LIMIT to ORDER BY queries"
                        )

                    if "join" in query_lower and "where" not in query_lower:
                        suggestion["suggestions"].append(
                            "Add WHERE clause to filter JOIN results"
                        )

                    if "like" in query_lower and query_lower.count("%") > 1:
                        suggestion["suggestions"].append(
                            "Consider full-text search instead of multiple LIKE operations"
                        )

                    if "in (" in query_lower:
                        suggestion["suggestions"].append(
                            "Consider using EXISTS or JOIN instead of large IN clauses"
                        )
            ...
            with self.engine.connect() as conn:
                ...
                return {
                    ...
                    "most_used_indexes": sorted(
                        stats, key=lambda x: x["scans"], reverse=True
                    )[:10],
                    "least_efficient_indexes": sorted(
                        [s for s in stats if s["scans"] > 0],
    ...
    def suggest_index_maintenance(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for unused_idx in usage_stats.get("unused_indexes", []):
                if not unused_idx["index"].endswith(
                    "_pkey"
                ):  # Don't suggest removing primary keys
                    suggestions.append(
                        {
                            "type": "remove_unused",
