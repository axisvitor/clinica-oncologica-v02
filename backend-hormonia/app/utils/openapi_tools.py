    ...
    for path, methods in openapi_schema.get("paths", {}).items():
        for method, operation in methods.items():
            if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                ...
                # Create request item
                request_item = _create_postman_request(
                    path, method, operation, openapi_schema
                )
...
def _create_postman_request(
    path: str, method: str, operation: Dict[str, Any], schema: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    if "security" in operation or any(
        tag in ["Patients", "Messages", "Reports"] for tag in operation.get("tags", [])
    ):
    ...
    if method.upper() in ["POST", "PUT", "PATCH"] and "requestBody" in operation:
        ...
        if "application/json" in content:
            ...
            if "properties" in form_schema:
                request["request"]["body"] = {"mode": "urlencoded", "urlencoded": []}
...
def _generate_example_from_schema(
    schema: Dict[str, Any], openapi_schema: Dict[str, Any]
) -> Any:
    ...
    if schema_type == "object":
        ...
        for prop_name, prop_schema in properties.items():
            ...
            if prop_name in required or len(result) < 5:
                result[prop_name] = _generate_example_from_schema(
                    prop_schema, openapi_schema
                )
...
def save_postman_collection(
    app: FastAPI, filename: str = "hormonia_api_collection.json"
):
    """
...
def validate_openapi_spec(app: FastAPI) -> Dict[str, Any]:
    ...
    try:
        ...
        for field in required_info_fields:
            if field not in info:
                validation_results["errors"].append(
                    f"Missing required info field: {field}"
                )
                validation_results["valid"] = False
            ...
            for method, operation in methods.items():
                if method.upper() in ["GET", "POST", "PUT", "DELETE", "PATCH"]:
                    ...
                    for status, response in responses.items():
                        ...
                        if any(
                            "example" in media_type for media_type in content.values()
                        ):
        ...
        if (
            endpoints_with_examples < endpoint_count * 0.5
