...
class LogAnalyzer:
        ...
        try:
            ...
            for key in keys:
                ...
                for log_json in logs_json:
                    ...
                    if pattern['first_seen'] is None or log_time < datetime.fromisoformat(pattern['first_seen']):
                        pattern['first_seen'] = log_time.isoformat()

                    if pattern['last_seen'] is None or log_time > datetime.fromisoformat(pattern['last_seen']):
    ...
    async def analyze_performance(
        ...
        db_performance = defaultdict(lambda: {
            'count': 0,
            'total_duration': 0,
            'min_duration': float('inf'),
            'max_duration': 0
        ...
        try:
            ...
            for key in keys:
                ...
                for log_json in logs_json:
                    ...
                    # API performance
                    if log.get('api_endpoint') and log.get('context', {}).get('duration_ms'):
        ...
        try:
            ...
            for key in keys:
                ...
                for log_json in logs_json:
                    ...
                    if 'unauthorized' in event_type.lower() or 'forbidden' in event_type.lower():
    ...
    async def correlation_analysis(
        ...
        try:
            ...
            return {
                ...
                'errors': [log for log in correlated_logs if log['level'] in ['ERROR', 'CRITICAL']],
                'db_queries': [log for log in correlated_logs if log.get('db_operation')]
            }
    ...
    def _calculate_request_duration(self, logs: List[Dict[str, Any]]) -> Optional[float]:
