...
class SentryConfig:
    ...
    def set_request_context(request: Request, user_id: Optional[str] = None) -> None:
        ...
        with sentry_sdk.configure_scope() as scope:
            ...
            scope.set_tag("ip_address", request.client.host if request.client else "unknown")
...
class SentryMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        if scope["type"] == "http":
            ...
            sentry_sdk.set_tag("transaction_name", f"{request.method} {request.url.path}")
...
def monitor_quiz_session(session_id: str, user_id: str, quiz_type: str) -> None:
    ...
    with sentry_sdk.configure_scope() as scope:
        ...
        scope.set_context("quiz_context", {
...
def monitor_patient_interaction(patient_id: str, interaction_type: str, metadata: Dict[str, Any] = None) -> None:
    """Monitor patient-related interactions."""
    with sentry_sdk.configure_scope() as scope:
        scope.set_tag("patient_interaction", interaction_type)
        scope.set_context("patient_context", {
...
def monitor_clinical_data_access(data_type: str, access_level: str, user_role: str) -> None:
...
def monitor_performance(operation_name: str):
    ...
    def decorator(func):
        async def async_wrapper(*args, **kwargs):
            with sentry_sdk.start_transaction(op="function", name=f"{operation_name}:{func.__name__}"):
