...
class StatusQuery:
    ...
    def _check_patient_exists_fast(self, patient_id: str) -> bool:
        ...
        if not self.redis_client:
            ...
            return (
                self.db.query(Patient).filter(Patient.id == UUID(patient_id)).first()
                is not None
            )
        ...
        try:
            ...
            self.redis_client.setex(
                cache_key, 300, "1" if exists else "0"
            )  # Cache for 5 minutes
    ...
    async def get_link_status(self, session_id: UUID) -> MonthlyQuizLinkResponse:
        ...
        status = QuizLinkStatus(
            metadata.get("link_status", QuizLinkStatus.ACTIVE.value)
        )
    ...
    async def get_patient_latest_status(
        ...
        if not self._check_patient_exists_fast(str(patient_id)):
            ...
            logger.info(
                f"Fast 404 for patient {str(patient_id)[:8]}... ({elapsed:.1f}ms)"
            )
            ...
            if expires_at_str:
                try:
                    ...
                    if (
                        ...
                        and metadata.get("link_status") != "cancelled"
                        ...
                        try:
                            ...
                            logger.error(
                                f"Error getting status for session {session.id}: {str(e)}"
                            )
                            continue
                except ValueError:
                    continue
    ...
    async def get_active_links_with_details(
        ...
        query = (
            ...
            .join(QuizTemplate, QuizSession.quiz_template_id == QuizTemplate.id)
            .filter(
                QuizSession.status != "completed",
                QuizSession.session_metadata.isnot(None),
            )
