...
class QuizFlowIntegrationService:
    ...
    async def trigger_monthly_quizzes(self, limit: int = 50) -> Dict[str, Any]:
        ...
        try:
            ...
            results = await self.quiz_trigger_service.check_and_trigger_monthly_quizzes(
                limit
            )
    ...
    async def process_quiz_response(
        self, patient_id: UUID, message_content: str, message_id: Optional[UUID] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            result = await self.conversational_service.process_quiz_response(
                ...
                message_metadata={"message_id": str(message_id)}
                if message_id
                else None,  # FIX: Change message_id= to message_metadata=
            )
    ...
    async def get_patient_quiz_status(self, patient_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            if not active_session:
                ...
                if flow_state:
                    (
                        is_due,
                        quiz_info,
                    ) = await self.quiz_trigger_service._is_patient_due_for_quiz(
                        flow_state
                    )
            ...
            return {"error": str(e), "has_active_quiz": False}
    ...
    async def cancel_active_quiz(self, patient_id: UUID) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Cancelled quiz session {active_session.id} for patient {patient_id}"
            )
            return True
    ...
    async def restart_quiz_session(self, patient_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            (
                is_due,
                quiz_info,
            ) = await self.quiz_trigger_service._is_patient_due_for_quiz(flow_state)
            ...
            # Trigger new quiz
            result = await self.quiz_trigger_service._trigger_patient_quiz(
