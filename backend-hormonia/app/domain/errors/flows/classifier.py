...
from app.exceptions import FlowStateError, FlowOperationError, ValidationError
...
class ErrorHandlerConfig:
    ...
    max_retry_attempts: dict[ErrorCategory, int] = field(
    ...
    retry_delays: dict[RecoveryStrategy, List[int]] = field(
        default_factory=lambda: {
...
class ErrorClassifier:
    ...
    def classify_error(self, error: Exception) -> tuple[ErrorCategory, ErrorSeverity]:
        ...
        if any(
            ...
            for keyword in ErrorHandlerConstants.MESSAGE_KEYWORDS
        ):
            if any(
                keyword in error_message
                for keyword in ErrorHandlerConstants.TIMEOUT_KEYWORDS
            ):
                return ErrorCategory.MESSAGE_DELIVERY, ErrorSeverity.MEDIUM
            elif any(
                keyword in error_message
                for keyword in ErrorHandlerConstants.RATE_LIMIT_KEYWORDS
            ):
        ...
        # External service errors
            ...
            keyword in error_message
            for keyword in ErrorHandlerConstants.EXTERNAL_SERVICE_KEYWORDS
        ):
            if any(
                keyword in error_message
                for keyword in ErrorHandlerConstants.TIMEOUT_KEYWORDS
        ...
        elif isinstance(error, SQLAlchemyError) or any(
            keyword in error_message
            for keyword in ErrorHandlerConstants.DATABASE_KEYWORDS
        ):
        ...
        # System errors
            ...
            if any(
                keyword in error_message
                for keyword in ErrorHandlerConstants.SYSTEM_RESOURCE_KEYWORDS
            ):
