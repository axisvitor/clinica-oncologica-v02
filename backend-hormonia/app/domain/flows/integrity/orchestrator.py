...
class FlowDataIntegrityChecker:
    ...
    async def run_comprehensive_check(
        ...
        try:
            logger.info(
                f"Starting comprehensive integrity check for patient {patient_id or 'all'}"
            )

            if check_flow_states:
                flow_issues, flow_count = await self.flow_state_checker.check_integrity(
                    patient_id
                )
            ...
            if check_messages:
                (
                    message_issues,
                    message_count,
                ) = await self.message_checker.check_integrity(patient_id)
                issues.extend(message_issues)
                total_records += message_count

            if check_references:
                ref_issues, ref_count = await self.reference_checker.check_integrity(
                    patient_id
                )
            ...
            logger.info(
                f"Integrity check completed: {len(issues)} issues found in {duration:.2f}s"
            )
            return result
    ...
    async def fix_issue(
        self, issue_id: str, create_backup: bool = True
    ) -> CorrectionResult:
        """
        ...
        try:
            ...
            if "invalid_flow_type_" in issue_id:
                return await self.flow_state_corrector.fix_invalid_flow_type(
                    issue_id, create_backup
                )
            elif "invalid_step_" in issue_id:
                return await self.flow_state_corrector.fix_invalid_step(
                    issue_id, create_backup
                )
            elif "corrupted_json_" in issue_id and "flow_state" in issue_id:
                return await self.flow_state_corrector.fix_corrupted_json(
                    issue_id, create_backup
                )
            elif "corrupted_metadata_" in issue_id:
                return await self.message_corrector.fix_corrupted_metadata(
                    issue_id, create_backup
                )
            elif "inconsistent_status_" in issue_id:
                return await self.message_corrector.fix_inconsistent_status(
                    issue_id, create_backup
                )
            elif "invalid_message_dates_" in issue_id:
                return await self.message_corrector.fix_invalid_dates(
                    issue_id, create_backup
                )
            elif "duplicate_active_flows_" in issue_id:
                return await self.flow_state_corrector.fix_duplicate_active_flows(
                    issue_id, create_backup
                )
            elif "orphaned_flow_message_" in issue_id:
                return await self.flow_state_corrector.fix_orphaned_flow_message(
                    issue_id, create_backup
                )
            else:
    ...
    def _calculate_corruption_score(
        self, issues: list[CorruptionIssue], total_records: int
    ) -> float:
    ...
    def _generate_recommendations(self, issues: list[CorruptionIssue]) -> list[str]:
        ...
        critical_count = len(
            [i for i in issues if i.severity == CorruptionSeverity.CRITICAL]
        )
        ...
        if CorruptionType.ORPHANED_DATA in corruption_types:
            recommendations.append(
                "Clean up orphaned data to improve system performance"
            )

        if CorruptionType.DUPLICATE_RECORDS in corruption_types:
            recommendations.append(
                "Resolve duplicate records to prevent flow conflicts"
            )

        if CorruptionType.CORRUPTED_JSON in corruption_types:
            recommendations.append(
                "Fix corrupted JSON data to prevent processing errors"
            )

        if not recommendations:
            recommendations.append(
                "No significant issues found - system integrity is good"
            )
