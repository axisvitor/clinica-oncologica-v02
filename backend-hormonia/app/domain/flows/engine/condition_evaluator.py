...
class ConditionEvaluator:
    ...
    async def humanize_message_content(
        ...
        if not should_humanize_message(content):
            logger.info(
                f"Message contains critical keywords, skipping AI humanization: {content[:100]}..."
            )
            return content
        ...
        try:
            ...
            custom_fields = (
                ...
                if isinstance(metadata.get("custom_fields"), dict)
                else {}
            )
            if metadata.get("no_ai_messages", False) or custom_fields.get(
                "no_ai_messages", False
            ):
                logger.info(
                    f"Patient {patient_id} has AI restriction (no_ai_messages) - skipping humanization"
                )
                return content
            if metadata.get("critical_condition", False) or custom_fields.get(
                "critical_condition", False
            ):
                logger.info(
                    f"Patient {patient_id} in critical condition - skipping AI humanization"
                )
                return content
            ...
            patient_context = await self.ai_context_builder.build_patient_context(
                ...
                patient_data={
                    ...
                    if hasattr(patient, "treatment_start_date")
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    humanized_response = await asyncio.wait_for(
                        humanization_task, timeout=timeout
                    )
                    ...
                    if not should_humanize_message(humanized_content):
                        logger.warning(
                            f"AI output contains critical keywords - using original content for patient {patient_id}"
                        )
                        return content

                    # Log successful humanization
                    logger.info(
                        f"Message successfully humanized for patient {patient_id} (attempt {attempt + 1})"
                    )
                    ...
                    # Store humanization metadata
                    {
                        ...
                        "personalization_notes": getattr(
                            humanized_response, "personalization_notes", []
                        ),
                    }
                    ...
                    if (
                        cache_key
                        and hasattr(self, "redis_client")
                        and self.redis_client
                    ):
                        try:
                            await self.redis_client.setex(
                                cache_key, 86400, humanized_content
                            )  # 24h TTL
                    ...
                    logger.warning(
                        f"AI humanization timeout on attempt {attempt + 1} for patient {patient_id}"
                    )
                    ...
                    logger.warning(
                        f"AI humanization failed on attempt {attempt + 1} for patient {patient_id}: {e}"
                    )
                    if attempt == max_retries:
                        break
                    await asyncio.sleep(0.5 * (attempt + 1))  # Progressive delay
            ...
            logger.error(
                f"AI humanization failed after {max_retries + 1} attempts for patient {patient_id}"
            )

        except Exception as e:
            logger.error(
                f"Critical error in AI humanization for patient {patient_id}: {e}"
            )
        ...
        if self.humanization_config["fallback_enabled"]:
            ...
            logger.warning(
                f"Fallback disabled, returning empty content for patient {patient_id}"
            )
    ...
    def determine_question_type(self, step: FlowStep) -> str:
        ...
        if hasattr(step, "metadata") and step.metadata:
            ...
            if any(
                key in str(step.metadata).lower()
                for key in ["medication", "dosage", "emergency", "consent"]
            ):
        ...
        if any(
            ...
            for word in ["medicação", "medicamento", "dose", "mg", "ml", "emergência"]
        ):
            return "medication_verification"

        if any(
            word in content_lower
            for word in ["cirurgia", "procedimento", "exame", "jejum"]
        ):
            return "surgery_preparation"

        if any(
            word in content_lower for word in ["consentimento", "autorizo", "concordo"]
        ):
        ...
        if any(
            word in content_lower
            for word in ["como você está", "como se sente", "sentindo"]
        ):
            return "daily_checkin"

        if any(
            word in content_lower
            for word in ["humor", "ânimo", "emocional", "ansiedade"]
        ):
            return "mood_assessment"

        if any(
            word in content_lower
            for word in ["sintoma", "dor", "desconforto", "náusea"]
        ):
