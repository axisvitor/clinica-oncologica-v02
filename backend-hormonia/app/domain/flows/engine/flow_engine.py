...
from app.core.event_loop_manager import AsyncFlowEngineBase
...
class FlowEngine(AsyncFlowEngineBase):
    ...
    def _get_flow_type_from_state(self, flow_state: PatientFlowState) -> str:
        ...
        if not template_version:
            raise ValidationError(
                f"Template version not found for flow state {flow_state.id}"
            )
    ...
    def _validate_patient_data(self, patient: Patient) -> dict[str, Any]:
        ...
        critical_fields = {
            ...
            "treatment_type": (
                ...
                "Tipo de tratamento não informado",
            ),
        ...
        for field_name, (field_value, error_msg) in critical_fields.items():
            if not field_value or (
                ...
                errors.append(
                    {"field": field_name, "message": error_msg, "severity": "error"}
                )
        ...
        recommended_fields = {
            ...
            "treatment_start_date": (
                ...
                "Data de início do tratamento não informada",
            ),
        ...
        for field_name, (field_value, warning_msg) in recommended_fields.items():
            if not field_value or (
                ...
                warnings.append(
                    {"field": field_name, "message": warning_msg, "severity": "warning"}
                )
        ...
        if warnings:
            warning_messages = [
                f"{warn['field']}: {warn['message']}" for warn in warnings
            ]
            logger.warning(
    ...
    def start_flow(
        ...
        if not template_data:
            logger.error(
                f"No suitable template found for flow_type '{flow_type}' and patient {patient_id}"
            )
            if not fallback_to_default:
                raise NotFoundError(f"Flow template '{flow_type}' not found")
            else:
                raise NotFoundError(
                    f"Flow template '{flow_type}' not found and no default template available"
                )

        # Log if we're using a fallback template
        if template_data.flow_type != flow_type:
            logger.warning(
                f"Using fallback template '{template_data.flow_type}' instead of requested '{flow_type}' for patient {patient_id}"
            )
        ...
        if validation_errors:
            raise ValidationError(
                f"Flow template validation failed: {validation_errors}"
            )
        ...
        flow_kind = (
            self.db.query(FlowKind)
            .filter(FlowKind.flow_type == template_data.flow_type)
            .first()
        )

        if not flow_kind:
            raise ValidationError(
                f"Flow kind not found for flow type: {template_data.flow_type}"
            )
        ...
        current_version = template_version_repo.get_current_version_by_flow_type(
            template_data.flow_type
        )

        if not current_version:
            raise ValidationError(
                f"No current version found for flow type: {template_data.flow_type}"
            )

        # Determine the first step (entry point) from the template
        entry_step = (
            min(template_data.steps, key=lambda s: s.id).id
        ...
        safe_create_task(
            self.step_executor.schedule_step_actions(
                patient,
                created_flow.current_step,
                state_machine,
            ...
            context={
                ...
                "step": created_flow.current_step,
            },
        )
        ...
        if first_step:
            safe_create_task(
                self.step_executor.schedule_step(
                    created_flow,
                    first_step,
                    created_flow.started_at,
                    self.condition_evaluator,
                ),
    ...
    def _get_template_with_fallback(
        self, flow_type: str, enable_fallback: bool = True
    ) -> Optional[Any]:
        """
        ...
        fallback_hierarchy = {
            ...
            "hormonia_fluxo_prostata": [
                "hormonia_fluxo_hormonal",
                "hormonia_fluxo_padrao",
            ],
        ...
        for fallback_type in fallbacks:
            ...
            if template_data:
                logger.info(
                    f"Using fallback template '{fallback_type}' for requested '{flow_type}'"
                )
                return template_data
        ...
        try:
            ...
            if available_templates:
                ...
                template_data = self.template_service.get_template_data(
                    fallback_template.flow_type
                )
                if template_data:
                    logger.warning(
                        f"Using emergency fallback template '{fallback_template.flow_type}' for requested '{flow_type}'"
                    )
                    return template_data
    ...
    async def process_patient_day_async(
        ...
        return await self._process_patient_day_internal(
            patient_id, force_transition, additional_context
        )
    ...
    def process_patient_day(
        ...
        try:
            return asyncio.run(
                self._process_patient_day_internal(
                    patient_id, force_transition, additional_context
                )
            )
        except RuntimeError:
    ...
    async def _process_patient_day_internal(
        ...
        context = self.context_builder.build_context(
            patient, active_flow, additional_context
        )
    ...
    async def advance_flow(
        self, patient_id: UUID, to_step: Optional[int] = None, force: bool = False
    ) -> dict[str, Any]:
        ...
        state_machine = StateMachine(template_data)
        self.context_builder.build_context(patient, active_flow)
        ...
        if to_step is not None:
            ...
            return await self.process_patient_day_async(
                patient_id, force_transition=force
            )

    @with_db_retry(max_retries=3)
    def reset_flow(
        self, patient_id: UUID, to_step: int = 0, preserve_data: bool = True
    ) -> dict[str, Any]:
        ...
        if preserve_data:
            active_flow.state_data = active_flow.state_data or {}
            active_flow.state_data["reset_history"] = active_flow.state_data.get(
                "reset_history", []
            )
    ...
    def get_flow_status(self, patient_id: UUID) -> dict[str, Any]:
        ...
        template_version = (
            self.db.query(FlowTemplateVersion)
            .filter(FlowTemplateVersion.id == active_flow.template_version_id)
            .first()
        )

        if not template_version:
            raise ValidationError("Template version not found for flow state")
