...
class FlowScheduler:
    ...
    async def should_skip_patient_flow(
        ...
        try:
            ...
            if (
                ...
                and flow_state.state_data.get("status") == "completed"
            ):
    ...
    def calculate_processing_batch_size(
        self, total_flows: int, time_window_hours: int = 24
    ) -> int:
        """
    ...
    async def validate_send_time(
        self, send_time: datetime, patient: Patient
    ) -> tuple[bool, Optional[str]]:
        """
        ...
        try:
            ...
            if hour < 8 or hour > 22:
                logger.warning(
                    f"Send time {send_time} is outside reasonable hours (8-22)"
                )
    ...
    async def reschedule_failed_flow(
        self, flow_state: PatientFlowState, retry_delay_hours: int = 1
    ) -> datetime:
        """
