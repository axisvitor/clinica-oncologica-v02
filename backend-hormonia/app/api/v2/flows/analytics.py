...
async def get_dashboard_overview(
    ...
    try:
        return await _get_cached_or_compute(
            cache_key, compute, redis_cache, CACHE_TTL_DASHBOARD
        )
...
async def get_flow_metrics(
    ...
    async def compute():
        ...
        metrics_data = await dashboard_service.get_flow_metrics(
            flow_type=flow_type, timeframe=timeframe
        )
        return {
            "success": True,
            "flow_type": flow_type,
...
async def get_patient_engagement_metrics(
    ...
    async def compute():
        dashboard_service = get_flow_dashboard_service(db)
        engagement_data = await dashboard_service.get_patient_engagement_metrics(
            timeframe
        )
        return {
            "success": True,
...
async def get_risk_assessment(
    ...
    try:
        return await _get_cached_or_compute(
            cache_key, compute, redis_cache, CACHE_TTL_RISK
        )
    except Exception as e:
...
async def get_flow_performance_analytics(
    ...
    cache_key = (
        f"flow:performance:{flow_type or 'all'}:{start_str}:{end_str}:{current_user.id}"
    )
...
async def get_patient_journey_analytics(
    ...
    try:
        ...
        journey_data = await analytics_service.analyze_patient_journeys(
            patient_id=patient_id, flow_type=flow_type, timeframe=timeframe
        )
        return {
            "success": True,
...
async def generate_flow_insights(
    ...
    try:
        ...
        insights = await analytics_service.generate_ai_insights(
            flow_type=flow_type, analysis_depth=analysis_depth
        )
        return {
            "success": True,
            "flow_type": flow_type,
