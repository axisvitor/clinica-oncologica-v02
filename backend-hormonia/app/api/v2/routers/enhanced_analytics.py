...
async def get_enhanced_dashboard(
    ...
    return await service.get_enhanced_dashboard(time_range, include_predictions, fields, role, user_uuid)
...
async def get_cohort_analysis(
    ...
    role, user_uuid = _extract_user_context(current_user)
    return await service.get_cohort_analysis(cohort_filter, treatment_type, min_age, max_age, time_range, cursor, limit, role, user_uuid)
...
async def get_engagement_funnel(
    ...
    role, user_uuid = _extract_user_context(current_user)
    return await service.get_engagement_funnel(time_range, treatment_type, role, user_uuid)
...
async def get_predictive_analytics(
    ...
    role, user_uuid = _extract_user_context(current_user)
    return await service.get_predictive_analytics(metric_type, forecast_days, confidence_threshold, role, user_uuid)
...
@router.get("/export") # Response model handled by service returning StreamingResponse or similar
...
async def get_comparative_analytics(
    ...
    role, user_uuid = _extract_user_context(current_user)
    return await service.get_comparative_analytics(metric_type, current_start, current_end, compare_start, compare_end, role, user_uuid)
