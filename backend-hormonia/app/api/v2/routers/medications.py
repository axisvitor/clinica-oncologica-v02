...
from app.api.v2.patients_utils import _get_current_user_simple, _extract_user_context, _ensure_uuid
...
def _ensure_medication_access(current_user, medication_prescribed_by_id, patient_doctor_id):
    ...
    if user_uuid is None: raise HTTPException(status_code=403, detail="No permissions")
...
def _serialize_medication(medication) -> Optional[dict]:
    if medication is None: return None
    return {
...
async def list_medications(
    ...
    if include:
        if "patient" in include: query = query.options(joinedload(Medication.patient))
        if "prescribed_by" in include: query = query.options(joinedload(Medication.prescribed_by))
        if "treatment" in include: query = query.options(joinedload(Medication.treatment))
    ...
    if has_more: medications = medications[:limit]

    next_cursor = None
    if has_more and medications:
        cd = {"id": str(medications[-1].id), "created_at": medications[-1].created_at.isoformat()}
        import base64
    ...
    for m in medications:
        ...
        if include:
            if "patient" in include and m.patient:
                md["patient"] = {"id": str(m.patient.id), "name": m.patient.name, "email": m.patient.email}
            if "prescribed_by" in include and m.prescribed_by:
                md["prescribed_by"] = {"id": str(m.prescribed_by.id), "name": m.prescribed_by.full_name, "email": m.prescribed_by.email}
            if "treatment" in include and m.treatment:
                md["treatment"] = {"id": str(m.treatment.id), "treatment_type": m.treatment.treatment_type, "status": m.treatment.status, "start_date": m.treatment.start_date}
        if fields: md = apply_field_selection(md, fields)
        resp_data.append(md)
...
async def list_active_medications(
    ...
    filters = [Medication.deleted_at.is_(None), Medication.is_active == True]
    current_user_uuid = _ensure_uuid(user_id)
    
    if role_enum != UserRole.ADMIN:
        if not current_user_uuid: raise HTTPException(status_code=403)
        filters.append(or_(Medication.prescribed_by_id == current_user_uuid, Medication.patient.has(Patient.doctor_id == current_user_uuid)))
    ...
    return {"data": resp_data, "has_more": has_more, "total": None, "next_cursor": None} # Simplified
...
async def search_medications(
    ...
    if role_enum != UserRole.ADMIN:
        if not current_user_uuid: raise HTTPException(status_code=403)
        query = query.filter(or_(Medication.prescribed_by_id == current_user_uuid, Medication.patient.has(Patient.doctor_id == current_user_uuid)))
        
    medications = query.filter(Medication.name.ilike(f"%{q}%")).order_by(Medication.created_at.desc()).limit(limit).all()
...
async def get_medication(
    ...
    medication = query.filter(Medication.id == mid, Medication.deleted_at.is_(None)).first()
    if not medication: raise HTTPException(status_code=404)
...
async def create_medication(
    ...
    current_user_uuid = _ensure_uuid(user_id)
    if role_enum != UserRole.ADMIN:
        if not current_user_uuid or current_user_uuid != patient.doctor_id: raise HTTPException(status_code=403)
