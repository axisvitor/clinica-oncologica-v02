...
def _serialize_treatment(treatment) -> Optional[dict]:
    if treatment is None: return None
    status_val = treatment.status.value if isinstance(treatment.status, TreatmentStatus) else treatment.status
    type_val = treatment.treatment_type.value if isinstance(treatment.treatment_type, TreatmentType) else treatment.treatment_type
...
async def list_treatments(
    ...
    if include:
        if "patient" in include: query = query.options(joinedload(Treatment.patient))
        if "doctor" in include: query = query.options(joinedload(Treatment.doctor))
        if "medications" in include: query = query.options(joinedload(Treatment.medications))

    filters = [Treatment.is_active == True]
    ...
    if has_more: treatments = treatments[:limit]

    next_cursor = None
    if has_more and treatments:
        cd = {"id": str(treatments[-1].id), "created_at": treatments[-1].created_at.isoformat()}
        import base64
    ...
    for t in treatments:
        ...
        if include:
            if "patient" in include and t.patient:
                td["patient"] = {"id": str(t.patient.id), "name": t.patient.name, "email": t.patient.email}
            if "doctor" in include and t.doctor:
                td["doctor"] = {"id": str(t.doctor.id), "name": t.doctor.full_name, "email": t.doctor.email}
            if "medications" in include and hasattr(t, "medications"):
                td["medications"] = [{"id": str(m.id), "name": m.name, "dosage": m.dosage} for m in t.medications]
        if fields: td = apply_field_selection(td, fields)
        resp_data.append(td)
...
async def get_treatment_statistics(
    ...
    by_status = {s.value: base_query.filter(Treatment.status == s).count() for s in TreatmentStatus}
    by_type = {t.value: base_query.filter(Treatment.treatment_type == t).count() for t in TreatmentType}

    return {
        "total_treatments": total, "active_treatments": active,
        "completed_treatments": completed, "planned_treatments": planned,
        "by_status": by_status, "by_type": by_type, "completion_rate": rate
    }
...
async def get_treatment(
    ...
    treatment = query.filter(Treatment.id == tid, Treatment.is_active == True).first()
    if not treatment: raise HTTPException(status_code=404)
...
async def create_treatment(
    ...
    from app.repositories.patient import PatientRepository # Assuming we verify patient here or service
    ...
    role_enum, user_id = _extract_user_context(current_user)
    current_user_uuid = _ensure_uuid(user_id)
    if not did: did = current_user_uuid
...
async def update_treatment(
    ...
    if not t or not t.is_active: raise HTTPException(status_code=404)
