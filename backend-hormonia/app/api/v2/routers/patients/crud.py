...
async def list_patients(
    ...
    try:
        ...
        patients, has_more, next_cursor, total = await _run_sync(
            db,
            lambda sync_db: PatientRepository(sync_db).list_v2(
                filters=filters,
...
async def get_patient(
    ...
    try:
        patient = await _run_sync(
            db,
            lambda sync_db: PatientRepository(sync_db).get_by_id(
                patient_uuid,
                eager_load=True,
            )
        )
...
async def create_patient(
    ...
    if x_idempotency_key:
        ...
        legacy_existing = await _run_sync(
            db,
            lambda sync_db: PatientRepository(sync_db).get_by_idempotency_key(
                x_idempotency_key
            ),
        )
        if legacy_existing:
            await ensure_patient_access(current_user, legacy_existing.doctor_id)
            duration_seconds = time.monotonic() - start_time
            patient_create_idempotency_hits.labels(source="db").inc()
            patient_create_duration.labels(idempotent="true").observe(duration_seconds)
            response.status_code = status.HTTP_200_OK
            return await serialize_patient(legacy_existing)
...
async def update_patient(
    ...
    if update_dict:
        try:
            validated = await _run_sync(
                db,
                lambda sync_db: PatientIntegrityService(
                    sync_db,
    ...
    updated_patient = await _run_sync(
        db,
        lambda sync_db: PatientCRUDService(
            sync_db,
            PatientRepository(sync_db),
        ).update_patient(patient_uuid, domain_update)
    )
...
async def delete_patient(
    ...
    patient = await _run_sync(
        db,
        lambda sync_db: PatientRepository(sync_db).get_by_id(pid, eager_load=False)
    )
    if not patient:
