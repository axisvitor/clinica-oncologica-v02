...
def _serialize_appointment(appointment: Appointment) -> Optional[dict]:
    ...
    status_value = appointment.status.value if isinstance(appointment.status, AppointmentStatus) else appointment.status
    type_value = appointment.appointment_type.value if isinstance(appointment.appointment_type, AppointmentType) else appointment.appointment_type
...
async def list_appointments(
    ...
    if include:
        if "patient" in include: query = query.options(joinedload(Appointment.patient))
        if "practitioner" in include: query = query.options(joinedload(Appointment.practitioner))
    ...
    if has_more: appointments = appointments[:limit]

    next_cursor = None
    if has_more and appointments:
        cd = {"id": str(appointments[-1].id), "created_at": appointments[-1].created_at.isoformat()}
    ...
    for appt in appointments:
        ...
        if include:
            if "patient" in include and appt.patient:
                ad["patient"] = {"id": str(appt.patient.id), "name": appt.patient.name, "email": appt.patient.email, "phone": appt.patient.phone}
            if "practitioner" in include and appt.practitioner:
                ad["practitioner"] = {"id": str(appt.practitioner.id), "name": appt.practitioner.full_name, "email": appt.practitioner.email}
        if fields: ad = apply_field_selection(ad, fields)
        resp_data.append(ad)
...
async def check_conflicts(
    ...
    for appt in conflicts_list:
        conflicts.append({
            "id": str(appt.id), "patient_id": str(appt.patient_id),
            "scheduled_at": appt.scheduled_at.isoformat(),
            "duration_minutes": appt.duration_minutes, "status": appt.status
        })
...
async def complete_appointment(
    ...
    if post_appointment_notes: appt.post_appointment_notes = post_appointment_notes
    db.commit()
    return _serialize_appointment(appt)
