...
async def list_flow_templates(
    ...
    try:
        ...
        if is_active is not None: query = query.filter(FlowTemplateVersion.is_active == is_active)
        if is_draft is not None: query = query.filter(FlowTemplateVersion.is_draft == is_draft)
        if kind_key: query = query.filter(FlowKind.kind_key == kind_key)
...
async def create_flow_template(
    ...
    try:
        ...
        if template.flow_kind_id:
            ...
            if not flow_kind: raise HTTPException(status_code=404, detail="Flow kind not found")
            ...
            raise HTTPException(status_code=400, detail="flow_kind_id or kind_key required")
        ...
        if existing: raise HTTPException(status_code=409, detail="Version exists")
...
async def update_flow_template(
    ...
    try:
        ...
        if updates.template_name is not None: template.template_name = updates.template_name
        if updates.description is not None: template.description = updates.description
        if updates.steps is not None: template.messages = updates.steps
        if updates.metadata is not None: template.template_metadata = updates.metadata
        if updates.is_active is not None: template.is_active = updates.is_active
        if updates.is_draft is not None:
            if template.is_draft and not updates.is_draft: template.published_at = datetime.utcnow()
        ...
        template = db.query(FlowTemplateVersion).options(joinedload(FlowTemplateVersion.kind)).filter(FlowTemplateVersion.id == template_id).first()
...
@router.post("/flows/{template_id}/duplicate", response_model=FlowTemplateV2Response, status_code=201)
@limiter.limit(RATE_LIMIT_WRITE)
async def duplicate_flow_template(
    ...
    try:
        _check_write_permission(current_user)
        role, user_uuid = _extract_user_context(current_user)
        source = db.query(FlowTemplateVersion).filter(FlowTemplateVersion.id == template_id).first()
        ...
        new_template = FlowTemplateVersion(
            ...
            is_active=False, is_draft=True, published_at=None,
            created_by=user_uuid
        )
        db.add(new_template)
        db.commit()
        ...
        new_template = db.query(FlowTemplateVersion).options(joinedload(FlowTemplateVersion.kind)).filter(FlowTemplateVersion.id == new_template.id).first()
...
async def list_flow_kinds(
    ...
    try:
        ...
        if is_active is not None: query = query.filter(FlowKind.is_active == is_active)
...
async def create_flow_kind(
    ...
    try:
        ...
        if existing: raise HTTPException(status_code=409, detail="Kind exists")
