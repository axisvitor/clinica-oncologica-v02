...
class BaseAgent(ABC):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.logger.info(f"Agent {self.agent_id} initialized with specialization: {specialization}")
    ...
    async def receive_message(self, message: AgentMessage):
        ...
        try:
            ...
            self.logger.debug(f"Received message {message.id} from {message.from_agent}")
    ...
    async def _execute_task(self, task_id: str, task_data: Dict[str, Any]):
        ...
        try:
            ...
            # Run Claude-Flow pre-task hook
            await self._run_pre_task_hook("task_execution", {
                "task_id": task_id,
                "task_data": task_data
            })
            ...
            # Run Claude-Flow post-task hook
            await self._run_post_task_hook("task_execution", {
                "task_id": task_id,
                "result": result,
    ...
    def _update_average_response_time(self, execution_time: float):
        ...
        if self.metrics.tasks_completed == 1:
            ...
            self.metrics.average_response_time = (current_total + execution_time) / total_tasks
