...
class CircuitBreaker:
        ...
        if self.state == CircuitState.HALF_OPEN:
            if self.stats.consecutive_successes >= self.success_threshold:
                ...
                self.stats.state_changes.append(
                    (CircuitState.CLOSED, now_sao_paulo())
                )
    ...
    def record_failure(self):
        ...
        self.stats.last_failure_time = now_sao_paulo()
        self.stats.consecutive_failures += 1
        self.stats.consecutive_successes = 0

        if self.state == CircuitState.HALF_OPEN:
            self.state = CircuitState.OPEN
            self.stats.state_changes.append((CircuitState.OPEN, now_sao_paulo()))
    ...
    async def call(
        self, func: Callable, *args, fallback: Optional[Any] = None, **kwargs
    ) -> Any:
        """
        ...
        Args:
            ...
            *args: Function arguments
            fallback: Optional fallback function or static fallback value
        ...
        async with self._lock:
            ...
            if self.state == CircuitState.OPEN:
                if self._should_attempt_reset():
                    ...
                    self.stats.state_changes.append(
                        (CircuitState.HALF_OPEN, now_sao_paulo())
                    )
        ...
        async with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                if self.stats.consecutive_failures >= self.failure_threshold:
                    self.state = CircuitState.OPEN
                    self.stats.state_changes.append(
                        (CircuitState.OPEN, now_sao_paulo())
                    )
                    logger.error(
                        f"Circuit {self.name} opened after {self.failure_threshold} failures"
                    )
    ...
    def _should_attempt_reset(self) -> bool:
        ...
        time_since_failure = now_sao_paulo() - self.stats.last_failure_time
        return time_since_failure.total_seconds() >= self.recovery_timeout

    async def _execute_fallback(self, fallback: Any, *args, **kwargs) -> Any:
        """Execute fallback callable or return fallback literal value."""
        try:
...
class AIServiceCircuitBreaker:
    ...
    async def call_gemini(
        ...
        **kwargs,
        ...
        Returns:
            Tuple of (Gemini response or fallback, used_fallback flag)
        """
        used_fallback = False
        ...
        response = await self.breakers["gemini"].call(
