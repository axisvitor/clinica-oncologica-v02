...
from .rate_limit import RateLimiter, RateLimitContext
...
class FastAPIResilienceManager:
    ...
    def _init_rate_limiting(self):
        ...
        self.rate_limiter = RateLimiter(
            config=self.config.rate_limit, name="fastapi_main"
        )
    ...
    def _resolve_user_id_from_object(user: Any) -> Optional[str]:
        ...
        if isinstance(user, dict):
            for key in ("id", "user_id", "uid"):
                ...
                normalized = FastAPIResilienceManager._normalize_user_identifier(
                    candidate
                )
                if normalized:
                    return normalized
    ...
    def _should_bypass_route(self, path: str) -> bool:
        ...
        bypass_patterns = ["/health", "/metrics", "/docs", "/redoc", "/openapi.json"]
