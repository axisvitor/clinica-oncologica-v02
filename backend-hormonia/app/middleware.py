...
from app.utils.rate_limiting import check_rate_limit, RateLimitType, get_client_identifier
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        request_logger.info(
            ...
            extra={
                ...
                'request_body': request_data.get('body') if self.log_request_body else None
            }
        )
        ...
        try:
            ...
            response_data = await self._extract_response_data(response) if self.log_response_body else {}
            ...
            request_logger.info(
                ...
                extra={
                    ...
                    'response_body': response_data.get('body') if self.log_response_body else None
                }
            )
            ...
            if isinstance(e, HTTPException):
                if e.status_code in [401, 403, 429]:
                    log_security_event(
                        'authentication_failure' if e.status_code == 401 else 
                        'authorization_failure' if e.status_code == 403 else 'rate_limit_exceeded',
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if any(path in str(request.url.path) for path in ['/auth/', '/patients/', '/reports/']):
...
class InputSanitizationMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if request.method in ['POST', 'PUT', 'PATCH'] and self._should_sanitize(request):
            try:
                ...
                if body:
                    ...
                    if 'application/json' in content_type:
                        try:
                            ...
                            # Sanitize the data
                            sanitized_data = self._sanitize_request_data(data, request.url.path)
