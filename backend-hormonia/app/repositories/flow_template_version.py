...
class FlowTemplateVersionRepository(BaseRepository):
    ...
    def get_latest_published_by_kind(
        ...
        return (
            ...
            .order_by(desc(FlowTemplateVersion.published_at))
            .first()
        )

    def get_by_flow_type_and_version(
        self, flow_type: str, version: str = None
    ) -> Optional[FlowTemplateVersion]:
        ...
        result = self.db.execute(
            query, {"flow_type": flow_type, "version": version}
        ).fetchone()

        if result:
            return (
                self.db.query(FlowTemplateVersion)
                .filter(FlowTemplateVersion.id == result.id)
                .first()
            )
        return None
    ...
    def list_versions_by_kind(
        self, kind_id: UUID, status: str = None
        ...
        try:
            result = self.db.execute(
                ...
                """),
                {
                    ...
                    "approved_by": str(published_by) if published_by else None,
                },
            )
    ...
    def update_messages(
        ...
        try:
            updates = {"messages": messages, "version_id": str(version_id)}
    ...
    def get_message_for_day(
        self, flow_type: str, day: int, version: str = None
    ...
    def search_templates(
        self, search_term: str = None, status: str = None, flow_type: str = None
    ) -> List[Dict[str, Any]]:
