...
class MessageRepository(BaseRepository[Message]):
    ...
    async def create_with_integrity_check(self, message_data: Dict) -> Message:
        ...
        try:
            ...
            message_data['integrity_checksum'] = self.integrity_service.generate_message_checksum(message_data)
            ...
            self.db.rollback()
            raise ValidationError("Message creation failed due to integrity constraints")
    ...
    async def validate_conversation_integrity(self, patient_id: UUID) -> Dict[str, any]:
        ...
        try:
            return await self.integrity_service.validate_conversation_integrity(patient_id)
        except Exception as e:
...
class MessageIntegrityService:
    ...
    async def validate_message_creation(self, message_data: Dict) -> None:
        ...
        try:
            ...
            if len(content) > 4096:  # WhatsApp message limit
                raise ValidationError("Message content exceeds maximum length (4096 characters)")
            ...
            if direction == MessageDirection.INBOUND and status == MessageStatus.PENDING:
            ...
            logger.info(f"Message validation passed for patient {message_data['patient_id']}")
    ...
    async def _validate_chronological_order(self, message_data: Dict) -> None:
        ...
        try:
            ...
            if latest_message:
                ...
                time_diff = abs((message_timestamp - latest_message.created_at).total_seconds())
    ...
    def generate_message_checksum(self, message_data: Dict) -> str:
        ...
        try:
            ...
            checksum_string = '|'.join(f"{k}:{v}" for k, v in sorted(checksum_fields.items()))
    ...
    async def validate_conversation_integrity(self, patient_id: UUID) -> Dict[str, any]:
        ...
        try:
            ...
            for message in messages:
                if hasattr(message, 'integrity_checksum') and message.integrity_checksum:
            ...
            if not patient:
                ...
                validation_result['issues'].append(f"All {len(messages)} messages are orphaned (patient not found)")
                validation_result['overall_integrity'] = False
            ...
            if validation_result['overall_integrity']:
                logger.info(f"Conversation integrity validation passed for patient {patient_id}")
            else:
                logger.warning(f"Conversation integrity issues found for patient {patient_id}: {validation_result['issues']}")
    ...
    async def _validate_message_flow_consistency(self, messages: List[Message]) -> List[str]:
        ...
        try:
            ...
            inbound_count = sum(1 for msg in messages if msg.direction == MessageDirection.INBOUND)
            outbound_count = sum(1 for msg in messages if msg.direction == MessageDirection.OUTBOUND)
            ...
            for message in messages:
                if message.direction == MessageDirection.OUTBOUND:
                    ...
                    if consecutive_outbound > 5:  # More than 5 consecutive outbound messages
                        issues.append(f"Too many consecutive outbound messages detected (>{consecutive_outbound})")
                        break
                else:
                    consecutive_outbound = 0
            ...
            for message in messages:
                if message.direction == MessageDirection.OUTBOUND:
                    ...
                    if not metadata or 'flow_context' not in metadata:
                        issues.append(f"Missing flow context for outbound message {message.id}")
        ...
        try:
            ...
            if validation_result['checksum_mismatches'] > 0:
                ...
                for message in messages:
                    ...
                    if hasattr(message, 'integrity_checksum') and message.integrity_checksum != correct_checksum:
                        message.integrity_checksum = correct_checksum
                        repairs_made.append(f"Updated checksum for message {message.id}")

                self.db.commit()

            logger.info(f"Conversation integrity repair completed for patient {patient_id}: {len(repairs_made)} repairs made")
