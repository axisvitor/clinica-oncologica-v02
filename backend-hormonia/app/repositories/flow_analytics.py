...
class FlowAnalyticsRepository(BaseRepository[FlowAnalytics]):
    ...
    def get_by_patient_and_date_range(self,
                                     patient_id: UUID,
                                     start_date: datetime,
                                     end_date: datetime) -> List[FlowAnalytics]:
        """Get analytics records for patient within date range."""
        return self.db.query(FlowAnalytics).filter(
            and_(
    ...
    def get_by_flow_type_and_date_range(self,
                                       flow_type: str,
                                       start_date: datetime,
                                       end_date: datetime) -> List[FlowAnalytics]:
    ...
    def get_daily_metrics(self,
        ...
        query = self.db.query(
            ...
            func.date(FlowAnalytics.timestamp),
            FlowAnalytics.event_type
    ...
    def get_patient_engagement_scores(self,
                                    patient_ids: List[UUID],
                                    start_date: datetime,
                                    end_date: datetime) -> Dict[UUID, float]:
        ...
        results = query.all()
        return {patient_id: float(avg_engagement) for patient_id, avg_engagement in results}
...
class FlowMessageRepository(BaseRepository[FlowMessage]):
    ...
    def get_by_flow_state(self, flow_state_id: UUID) -> List[FlowMessage]:
        ...
        return self.db.query(FlowMessage).filter(
            FlowMessage.flow_state_id == flow_state_id
        ).order_by(asc(FlowMessage.flow_day)).all()
    ...
    def get_delivery_metrics(self,
        ...
        delivery_times = query.filter(
            ...
            (func.extract('epoch', FlowMessage.delivered_at) - 
             func.extract('epoch', FlowMessage.sent_at)).label('delivery_time')
        ).all()
    ...
    def get_response_correlation(self,
                               start_date: datetime,
                               end_date: datetime) -> List[Dict[str, Any]]:
