...
from app.monitoring.agent_health_monitor import SystemHealthMonitor, get_system_health_monitor
...
class AgentHealth:
    ...
    def is_healthy(self, max_response_time: float = 5.0, min_success_rate: float = 0.8) -> bool:
        ...
        return (
            ...
            time_since_heartbeat < 60 and  # Heartbeat within last minute
...
class SwarmManager:
    ...
    def __init__(self, db_session: Session):
        ...
        self.health_check_interval = settings.get('SWARM_HEALTH_CHECK_INTERVAL', 30)  # 30 seconds
    ...
    async def start(self):
        ...
        try:
            ...
            self.background_tasks = [
                ...
                self._create_background_task(self._metrics_collector(), "metrics_collector")
            ]
    ...
    async def register_agent(self, agent: BaseAgent) -> bool:
        ...
        try:
            if len(self.agents) >= self.max_agents:
                self.logger.warning(f"Cannot register agent {agent.agent_id}: swarm at capacity")
                return False
            ...
            self.logger.info(f"Registered agent {agent.agent_id} ({agent.specialization})")
            return True
    ...
    async def broadcast_message(
        ...
        if filter_capabilities:
            ...
            target_agents = [
                ...
                if any(cap in self.agent_capabilities.get(agent_id, []) 
        ...
        while self.status != SwarmStatus.SHUTDOWN:
            try:
                ...
                if suitable_agent:
                    ...
                    self.logger.warning(f"No suitable agent for task {task.task_id}, requeueing")
    ...
    async def _assign_task_to_agent(self, task: SwarmTask, agent_id: str):
        ...
        try:
            ...
            self.logger.error(f"Failed to assign task {task.task_id} to agent {agent_id}: {e}")
        ...
        if agent_id in self.agents:
            try:
                ...
                if health and not health.is_healthy():
                    self.logger.error(f"Agent {agent_id} failed recovery, unregistering")
        ...
        while self.status != SwarmStatus.SHUTDOWN:
            try:
                ...
                for agent_id in list(self.agents.keys()):
                    ...
                    while not queue.empty() and messages_processed < 10:  # Limit per iteration
                        try:
                            ...
                            self.logger.error(f"Message routing error for agent {agent_id}: {e}")
            ...
            try:
                ...
                for agent_id, agent in self.agents.items():
                    try:
                        ...
                        await self.send_message_to_agent(agent_id, "metrics_request", {})
                        ...
                        if self.health_monitor and agent_id in self.agent_health:
                            ...
                            await self.health_monitor.update_agent_metrics(
                                agent_id=agent_id,
                                response_time_ms=health.response_time * 1000,  # Convert to ms
    ...
    async def _reassign_agent_tasks(self, agent_id: str):
        ...
        for task in tasks_to_reassign:
            ...
            self.logger.info(f"Reassigned task {task.task_id} from failed agent {agent_id}")
        ...
        return [
            {
                ...
                "health": asdict(self.agent_health[agent_id]) if agent_id in self.agent_health else None
            }
            for agent_id, agent in self.agents.items()
        ]

    async def complete_task(self, task_id: str, result: Optional[Dict[str, Any]] = None, error: Optional[str] = None) -> bool:
        ...
        if task.assigned_agent and task.assigned_agent in self.agent_health:
            ...
            if error:
                ...
                health.success_rate = successful_tasks / total_tasks if total_tasks > 0 else 1.0
        ...
        self.logger.info(f"Task {task_id} {'failed' if error else 'completed'} by agent {task.assigned_agent}")
        return True
