...
def upgrade() -> None:
    ...
    result = conn.execute(
        sa.text(
            "SELECT id FROM flow_template_versions WHERE flow_kind_id = :flow_kind_id AND version_number = :version_number"
        ),
    ...
    if not result:
        ...
        print(
            f"✅ Created template version: Onboarding v1.0 (ID: {ONBOARDING_TEMPLATE_VERSION_ID})"
        )
    else:
