...
| Auth mode transition (RETIRED) | `backend-hormonia/app/dependencies/auth_dependencies.py` | ~~Session auth is primary; Firebase bearer-token auth remains compatibility-only.~~ **Retired:** Cookie-session is the sole auth contract (S01/M004). | Backend API Team | — | — | — | — |
...
> **Status: RETIRED (S03/M006).** The canonical auth contract is cookie-session only (`HttpOnly`, `Secure`, `SameSite`). The `X-Session-ID` header fallback, `Authorization: Bearer` Firebase token path, and multi-transport session resolver were removed or made rejection-only in S01/M004. The entries below are retained for historical reference; no compatibility code for these transports remains active.

| layer | location | compatibility contract | owner | consumer | sunset trigger | target removal wave/date placeholder | metric to monitor | **status** |
|---|---|---|---|---|---|---|---|---|
| Backend session resolver shim | `backend-hormonia/app/dependencies/auth_dependencies.py` | ~~Resolve session from cookie, `X-Session-ID`, or `Authorization` fallback.~~ | Backend API Team | — | — | — | — | **RETIRED** — cookie-only contract (S01/M004) |
| Backend session endpoints shim | `backend-hormonia/app/routers/auth_session.py` | ~~`/session/validate` and `/session/logout` accept cookie primary + `X-Session-ID` fallback.~~ | Backend API Team | — | — | — | — | **RETIRED** — cookie-only (S01/M004) |
| Backend legacy token auth shim | `backend-hormonia/app/dependencies/auth_dependencies.py` | ~~Keep deprecated Firebase token path (`get_current_user`).~~ | Backend API Team | — | — | — | — | **RETIRED** — Firebase bearer path removed (S01/M004) |
| Frontend medico auth shim | `frontend-hormonia/src/app/providers/MedicoAuthContext.tsx` | Preserve legacy `{ state, signIn, signOut }` contract over new auth provider. | Frontend Dashboard Team | Legacy medico screens/hooks | No imports of `useMedicoAuth` and legacy `state` usage | Wave-Compat-3 (YYYY-MM-DD) | Import/reference count for `useMedicoAuth` | **ACTIVE** — frontend-only, unrelated to backend transport |
| Quiz client session alias | `quiz-mensal-interface/lib/api-client.ts` | Keep `getSessionStatus()` as alias to `recoverSession()` for older call sites. | Quiz Interface Team | Legacy quiz hooks/components | No calls to `getSessionStatus()` for 2 release cycles | Wave-Compat-3 (YYYY-MM-DD) | Call-site count for `getSessionStatus` | **ACTIVE** |
