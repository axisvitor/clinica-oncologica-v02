...
- [x] **LGPD-03**: AI event types (`AI_QUERY`, `AI_HUMANIZATION`, `AI_SENTIMENT`, `AI_FOLLOW_UP`) adicionados ao `AuditEventType` enum + Alembic migration
...
- [x] **AI-02**: Converter fallbacks `None` de LangGraph para `FeatureNotAvailableError` explícito (sem silent degradation)
...
- [x] **FLOW-03**: Testes de integração cobrindo flow system unificado + alert pipeline end-to-end
