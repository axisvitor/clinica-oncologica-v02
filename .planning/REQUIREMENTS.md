...
- [x] **AGENT-02**: HumanizeAgent transforms flow templates into natural conversation messages preserving question count and placeholders
- [x] **AGENT-03**: VariationAgent generates question variations avoiding 88% word overlap with recent interactions
- [x] **AGENT-04**: EmpathyAgent generates empathetic follow-up messages based on sentiment analysis
...
- [x] **AGENT-07**: GeminiDomainClient methods delegate to new Pydantic AI agents via shim pattern (zero breaking changes to callers)
- [x] **AGENT-08**: 50-scenario output regression test suite passes comparing old GeminiClient vs new agent outputs
...
- [x] **FLOW-03**: LangGraph, langchain-core packages removed from requirements.txt
- [x] **FLOW-04**: Redis LangGraph checkpoint keys purged via migration script (PHI data, LGPD compliance)
- [x] **FLOW-05**: app/ai/langgraph/ directory tombstoned (runtime.py, nodes.py, state.py, graphs.py, _invoke.py, prompts.py)
...
- [x] **SDK-02**: langchain-google-genai package removed from requirements.txt (last LangChain dependency)
- [x] **SDK-03**: All Celery tasks calling AI agents use agent.run_sync() (not async_to_sync wrapper) to avoid event loop closure errors
