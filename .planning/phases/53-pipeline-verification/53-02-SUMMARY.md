...
plan: 02
subsystem: testing
tags: [recovery, retry, celery, stuck-flow, followup]
requires:
  - phase: 50-pipeline-reliability
    provides: failed-send retry task mechanics and outbound message persistence
  - phase: 51-flow-recovery
    provides: stuck-flow detection and bounded recovery behavior
provides:
...
affects: [phase-53-verification, regression-coverage, flow-recovery]
tech-stack:
  added: []
  patterns: [contextmanager-patched task sessions, real task.run coverage with async_to_sync bridge helpers]
key-files:
  created:
