...
- Issue: SQLAlchemy sync `Session` is called inside `async def` functions, which blocks the event loop and defeats FastAPI/Uvicorn's concurrency model. Annotated with `# TODO(async-migration)` markers.
- Files:
