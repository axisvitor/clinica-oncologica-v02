module.exports = {
  ...
  plugins: ["@typescript-eslint", "react", "react-hooks", "testing-library", "vitest"],
  settings: {
    react: {
      version: "detect",
    },
  ...
  rules: {
    ...
    // General rules
    // QUALITY FIX #7: Enforce no console.log in production code
    ...
    // Custom rule: Suggest logger usage
  ...
  overrides: [
    ...
    {
      files: ["*.config.{ts,js}", "*.setup.{ts,js}", "scripts/**/*.{ts,js}"],
      rules: {
        "@typescript-eslint/no-var-requires": "off",
        "no-console": "off",
        "no-restricted-syntax": "off",
