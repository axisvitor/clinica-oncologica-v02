...
export class SupabaseIntegrationTester {
  ...
  private async testDatabaseOperations() {
    try {
      ...
      const patientsResult = await database.patients.list({ limit: 1 })
      this.addResult('Database Read (Patients)', true, 
        `Successfully queried patients table. Total: ${(patientsResult as any).total}`, 
        { total: (patientsResult as any).total, hasData: (patientsResult as any).data.length > 0 })
