// WebSocket configuration - will be properly configured based on your backend setup
...
class WebSocketManager {
  ...
  private handleMessage(message: WebSocketMessage) {
    ...
    if (message.event.startsWith('patient:')) {
      const safeData = isObject(message.data) ? message.data : {}
      this.emit(message.event, { ...safeData, patient_id: message.patient_id })
    }

    // Handle quiz events with session_id
    if (message.event.startsWith('quiz:')) {
      const safeData = isObject(message.data) ? message.data : {}
      this.emit(message.event, { ...safeData, session_id: message.session_id })
    }
