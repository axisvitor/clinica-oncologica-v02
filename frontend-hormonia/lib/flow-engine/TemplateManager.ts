...
export class TemplateManager {
  ...
  async createTemplate(template: FlowTemplate): Promise<FlowTemplate> {
    ...
    try {
      ...
      const apiTemplate = {
        ...
        steps: [], // Required by API but not used in our internal format
        ...template.metadata
      }
      const createdTemplate = await apiClient.flows.createTemplate(apiTemplate)
      // Convert back to internal format and store
      const internalTemplate: FlowTemplate = {
        ...template,
        id: createdTemplate.id || template.id
      }
      this.templates.set(template.flow_type, internalTemplate)
  ...
  async updateTemplate(template: FlowTemplate): Promise<FlowTemplate> {
    ...
    try {
      ...
      await apiClient.flows.updateTemplate(template.id, apiTemplate)
      // Keep internal format
      this.templates.set(template.flow_type, template)
      return template
    } catch (error) {
