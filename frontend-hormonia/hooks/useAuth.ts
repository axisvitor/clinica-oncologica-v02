...
export function useAuth({
  ...
  }, [preferSupabase, supabaseAuth, apiAuth, sessionManagement, resetRetryState])
  ...
  }, [preferSupabase, supabaseAuth, apiAuth, sessionManagement])
  ...
  const refreshAuth = useCallback(async () => {
    return await executeWithRetry(async () => {
      if (preferSupabase && supabaseAuth.session) {
        await supabaseAuth.refreshSession()
        if (supabaseAuth.session?.expires_at) {
          const expiresIn = supabaseAuth.session.expires_at - Math.floor(Date.now() / 1000)
          sessionManagement.setupSession(expiresIn)
        }
        return { session: supabaseAuth.session, error: null }
      } else if (apiAuth.refreshToken) {
  ...
  }, [preferSupabase, supabaseAuth, apiAuth, sessionManagement, executeWithRetry])
