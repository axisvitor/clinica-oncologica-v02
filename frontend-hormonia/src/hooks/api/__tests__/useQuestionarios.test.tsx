...
describe('useQuestionarios', () => {
  ...
  it('should filter by search term', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ search: 'wellness' }),
      { wrapper: createWrapper() }
    )
    ...
    expect(result.current.data?.data[0].name).toContain('Wellness')
    expect(result.current.data?.total).toBe(2)
  })

  it('should filter by type - medical', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ type: 'medical' }),
      { wrapper: createWrapper() }
    )
    ...
    expect(result.current.data?.data).toHaveLength(2)
    expect(result.current.data?.total).toBe(2)
    result.current.data?.data.forEach(template => {
  ...
  it('should filter by type - wellness', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ type: 'wellness' }),
      { wrapper: createWrapper() }
    )
  ...
  it('should filter by status - active', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ status: 'active' }),
      { wrapper: createWrapper() }
    )
  ...
  it('should filter by status - inactive', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ status: 'inactive' }),
      { wrapper: createWrapper() }
    )
    ...
    expect(result.current.data?.data[0].is_active).toBe(false)
  })

  it('should sort by name ascending', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ sortBy: 'name', sortOrder: 'asc' }),
      { wrapper: createWrapper() }
    )

    await waitFor(() => expect(result.current.isSuccess).toBe(true))

    const names = result.current.data?.data.map(t => t.name) || []
  ...
  it('should sort by name descending', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ sortBy: 'name', sortOrder: 'desc' }),
      { wrapper: createWrapper() }
    )
  ...
  it('should sort by created_at descending (default)', async () => {
    ...
    const dates = result.current.data?.data.map(t => new Date(t.created_at)) || []
  ...
  it('should sort by created_at ascending', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ sortBy: 'created_at', sortOrder: 'asc' }),
      { wrapper: createWrapper() }
    )
  ...
  it('should paginate results', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ page: 1, size: 2 }),
      { wrapper: createWrapper() }
    )
  ...
  it('should return second page of results', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ page: 2, size: 2 }),
      { wrapper: createWrapper() }
    )
  ...
  it('should combine multiple filters', async () => {
    ...
    expect(result.current.data?.data).toHaveLength(1)
    expect(result.current.data?.data[0].name).toBe('Medical Oncology Assessment')
  ...
  it('should update query key when filters change', async () => {
    const { result, rerender } = renderHook(
      ...
      {
        wrapper: createWrapper(),
        initialProps: { options: { search: 'medical' } }
      }
    )
    ...
    rerender({ options: { search: 'wellness' } })
  ...
  it('should handle empty results', async () => {
    const { result } = renderHook(
      () => useQuestionarios({ search: 'nonexistent' }),
      { wrapper: createWrapper() }
    )
  ...
  it('should sort by responses count', async () => {
    ...
    const { result } = renderHook(
      () => useQuestionarios({ sortBy: 'responses', sortOrder: 'desc' }),
      { wrapper: createWrapper() }
    )

    await waitFor(() => expect(result.current.isSuccess).toBe(true))

    const responses = result.current.data?.data.map(t => t.analytics?.total_responses || 0) || []
