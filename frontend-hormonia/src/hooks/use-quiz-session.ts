...
import { quizApiClient, QuizSession, QuizApiError } from '../../lib/api-client'
...
function getSessionTokenFromCookie(): string | null {
  try {
    ...
    if (sessionCookie) {
      ...
      logger.debug(
        '[useQuizSession] Found session token in cookie:',
        token?.substring(0, 8) + '...'
      )
      return token || null
    }
...
export function useQuizSession(options: UseQuizSessionOptions = {}): UseQuizSessionState {
  const { autoFetch = true, pollingInterval = 0, onSessionLoaded, onError } = options
  ...
  const resolveApiError = useCallback(
    (err: unknown): QuizApiError =>
      err instanceof QuizApiError ? err : new QuizApiError(0, 'Unknown Error', err),
    []
  )

  const reportMutationError = useCallback(
    (message: string, err: unknown): QuizApiError | null => {
  ...
  const submitResponse = useCallback(
    async (questionId: string, answer: string | string[]): Promise<void> => {
      ...
      try {
        ...
        const updatedSession = await quizApiClient.submitResponse(sessionToken, questionId, answer)
