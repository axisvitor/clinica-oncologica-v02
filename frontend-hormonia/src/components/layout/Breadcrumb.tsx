...
export function Breadcrumb({
  ...
  const breadcrumbItems = React.useMemo(() => {
    ...
    if (generatedItems.length > maxItems) {
      ...
      return [firstItem, { label: '...', href: undefined, isActive: false }, ...lastItems]
    }
...
export function BreadcrumbWithDropdown({
  ...
  if (allItems.length <= maxVisibleItems) {
    return (
      <Breadcrumb {...(className ? { className } : {})} items={allItems} showIcons={showIcons} />
    )
  }

  const visibleItems = [allItems[0], ...allItems.slice(-2)]
