...
export interface HiveMindApi {
  ...
  alerts: (
    activeOnly?: boolean,
    severity?: string
  ) => Promise<{ alerts: unknown[]; count: number; active_count: number; timestamp: string }>
  integration: {
    getStatus: () => Promise<IntegrationStatus>
    setMode: (mode: string) => Promise<{ success: boolean; new_mode: string; timestamp: string }>
    setMigrationPercentage: (
      percentage: number
    ) => Promise<{ success: boolean; new_percentage: number; timestamp: string }>
  }
  swarm: {
  ...
  return {
    ...
    tasks: {
      ...
      conductQuiz: (patientId: string, quizType = 'monthly_checkup') =>
        client.post(`/api/v2/hive-mind/tasks/conduct-quiz/${patientId}`, undefined, {
          quiz_type: quizType,
        }),
    },
