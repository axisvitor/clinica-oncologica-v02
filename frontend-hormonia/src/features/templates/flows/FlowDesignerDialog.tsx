...
export const FlowDesignerDialog = memo<FlowDesignerDialogProps>(
  ({ open, onOpenChange, template, createNewVersion, onSuccess }) => {
    ...
    const handleSave = async (design: FlowDesign) => {
      try {
        ...
        const templateDescription = createNewVersion ? changelog.trim() : design.description
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-[95vw] max-h-[95vh] p-0">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="1"
                />
                <p className="text-xs text-muted-foreground">Versão do template (ex: 1, 2, 3...)</p>
              </div>
          ...
          <div className="h-[65vh] p-6">
            <FlowDesigner initialDesign={initialDesign} onSave={handleSave} className="h-full" />
          </div>
        </DialogContent>
      </Dialog>
    )
