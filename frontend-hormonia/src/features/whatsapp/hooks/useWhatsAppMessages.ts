...
export function useWhatsAppMessages(selectedInstance: string) {
  ...
  const { data: messageStats } = useQuery<MessageStats>({
    ...
    queryFn: () => apiClient.request(`/api/v2/whatsapp/messages/stats?instance=${selectedInstance}`),
  ...
  const { data: recentMessages = [] } = useQuery<WhatsAppMessage[]>({
    ...
    queryFn: () => apiClient.request(`/api/v2/whatsapp/messages?instance=${selectedInstance}&limit=20`),
    enabled: !!selectedInstance,
