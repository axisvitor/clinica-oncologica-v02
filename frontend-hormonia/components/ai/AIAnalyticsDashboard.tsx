    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{data.metrics.total_conversations}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(data.metrics.avg_sentiment * 100).toFixed(1)}%
            </div>
            <Progress value={data.metrics.avg_sentiment * 100} className="mt-2" />
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(data.metrics.response_accuracy * 100).toFixed(1)}%
            </div>
            <p className="text-xs text-muted-foreground">
              {data.metrics.response_accuracy > 0.9 ? 'Excelente' : 'Bom'}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {(data.metrics.human_handoff_rate * 100).toFixed(1)}%
            </div>
            <p className="text-xs text-muted-foreground">
              {data.metrics.human_handoff_rate < 0.1 ? 'Baixa' : 'Moderada'}
            </p>
          </CardContent>
        </Card>
      </div>
...
function getMockAnalyticsData(): AIAnalyticsData {
  return {
    ...
    trends: [
      { name: 'User Engagement', value: 85, change: 8, trend: 'up' as const, period: 'month' },
      { name: 'Response Accuracy', value: 92, change: 3, trend: 'up' as const, period: 'month' }
    ],
    overview: {
      total_conversations: 1247,
      avg_sentiment: 0.78,
    ...
    engagement_metrics: [
      {
        ...
        sentiment_trend: [
          { date: '2025-01-20', value: 0.8 },
          { date: '2025-01-19', value: 0.75 }
        ],
        engagement_score: 88,
