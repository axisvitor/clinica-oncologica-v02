...
import { Send, Image, FileText, Paperclip, X, CircleCheck as CheckCircle, CircleAlert as AlertCircle, Upload, MessageSquare } from 'lucide-react';
import { whatsappService as whatsAppService, MessageRequest, MessageResponse } from '../../services/whatsapp/WhatsAppService';
...
export const WhatsAppMessageSender: React.FC<WhatsAppMessageSenderProps> = ({
  ...
  const validateAndFormatNumber = async (phoneNumber: string) => {
    ...
    try {
      ...
      const validation = await whatsAppService.validatePhoneNumber(phoneNumber);

      if (validation.isValid) {
        // Check if number is on WhatsApp
        const response = await whatsAppService.checkWhatsAppNumber(phoneNumber);
        setIsValidNumber(response.exists);
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (messageType === 'text') {
        ...
        request.content = textMessage;
      } else {
        // Upload media file first
        const mediaResponse = await whatsAppService.uploadMedia(mediaFile!);
        request.mediaUrl = mediaResponse.mediaUrl;
        request.mediaCaption = mediaCaption;
        request.content = mediaCaption || '';
