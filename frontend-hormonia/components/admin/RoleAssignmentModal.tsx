...
const ROLE_TEMPLATES = {
  admin: {
    name: 'Administrador',
    description: 'Acesso padrão para administradores',
    permissions: [
      'admin.users.read',
      'admin.patients.read',
  ...
  doctor: {
    ...
    permissions: PERMISSION_GROUPS.flatMap(group => group.permissions.filter(p => !p.id.includes('admin')).map(p => p.id))
  },
}
...
export function RoleAssignmentModal({ open, onOpenChange, user }: RoleAssignmentModalProps) {
  ...
  const [selectedRole, setSelectedRole] = useState<any>('doctor')
  ...
  const handleRoleChange = (role: 'doctor' | 'admin') => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(ROLE_TEMPLATES).map(([roleKey, template]) => (
                  <Card
                    ...
                    onClick={() => handleRoleChange(roleKey as 'doctor' | 'admin')}
                  >
      ...
      // @ts-expect-error TODO: fix role indexing
      // @ts-expect-error TODO: fix role indexing
